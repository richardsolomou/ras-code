...
const resolveMockUpdateServerConfig = Effect.gen(function* () {
  ...
  const config = yield* Config.all({
    port: Config.port("RAS_CODE_DESKTOP_MOCK_UPDATE_SERVER_PORT").pipe(Config.withDefault(3000)),
    root: Config.string("RAS_CODE_DESKTOP_MOCK_UPDATE_SERVER_ROOT").pipe(
