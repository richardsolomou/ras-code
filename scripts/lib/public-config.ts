...
export interface RasCodePublicConfig {
...
export function loadRepoEnv({
  ...
  return {
    ...
    ...(config.clerkPublishableKey
      ? {
          RAS_CODE_CLERK_PUBLISHABLE_KEY: config.clerkPublishableKey,
    ...
    ...(config.clerkJwtTemplate
      ? {
          RAS_CODE_CLERK_JWT_TEMPLATE: config.clerkJwtTemplate,
    ...
    ...(config.clerkCliOAuthClientId
      ? {
          RAS_CODE_CLERK_CLI_OAUTH_CLIENT_ID: config.clerkCliOAuthClientId,
    ...
    ...(config.relayUrl
      ? {
          ...
          VITE_RAS_CODE_RELAY_URL: config.relayUrl,
        }
      : {}),
    ...(config.mobileOtlpTracesUrl
      ? {
          RAS_CODE_MOBILE_OTLP_TRACES_URL: config.mobileOtlpTracesUrl,
    ...
    ...(config.relayClientOtlpTracesToken
      ? {
          RAS_CODE_RELAY_CLIENT_OTLP_TRACES_TOKEN: config.relayClientOtlpTracesToken,
...
export function resolvePublicConfig(...sources: readonly Environment[]): RasCodePublicConfig {
  return {
    ...
    relayUrl: firstNonEmpty(sources, "RAS_CODE_RELAY_URL", "VITE_RAS_CODE_RELAY_URL"),
    ...
    relayClientOtlpTracesToken: firstNonEmpty(
      sources,
      "RAS_CODE_RELAY_CLIENT_OTLP_TRACES_TOKEN",
