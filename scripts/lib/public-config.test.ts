...
describe("loadRepoEnv", () => {
  it("does not project cloud configuration for an unconfigured clone", () => {
    ...
    expect(env.RAS_CODE_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.RAS_CODE_CLERK_CLI_OAUTH_CLIENT_ID).toBeUndefined();
    expect(env.VITE_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.RAS_CODE_CLERK_JWT_TEMPLATE).toBeUndefined();
  ...
  it("applies process, root local, and root precedence in that order", () => {
    ...
    NodeFS.writeFileSync(
      ...
      "RAS_CODE_CLERK_PUBLISHABLE_KEY=pk_root\nRAS_CODE_CLERK_JWT_TEMPLATE=template_root\nRAS_CODE_CLERK_CLI_OAUTH_CLIENT_ID=oauth_root\nRAS_CODE_RELAY_URL=https://root.example.test\n",
    );
    NodeFS.writeFileSync(
      NodePath.join(repoRoot, ".env.local"),
      "RAS_CODE_CLERK_PUBLISHABLE_KEY=pk_local\nRAS_CODE_CLERK_JWT_TEMPLATE=template_local\nRAS_CODE_CLERK_CLI_OAUTH_CLIENT_ID=oauth_local\nRAS_CODE_RELAY_URL=https://local.example.test\n",
    );

    expect(loadRepoEnv({ baseEnv: {}, repoRoot }).RAS_CODE_RELAY_URL).toBe(
      "https://local.example.test",
    );
    expect(
      loadRepoEnv({
        baseEnv: {
          RAS_CODE_CLERK_PUBLISHABLE_KEY: "pk_ci",
          RAS_CODE_CLERK_JWT_TEMPLATE: "template_ci",
          RAS_CODE_CLERK_CLI_OAUTH_CLIENT_ID: "oauth_ci",
          RAS_CODE_RELAY_URL: "https://ci.example.test",
        },
        repoRoot,
      }),
    ).toMatchObject({
      ...
      RAS_CODE_RELAY_URL: "https://ci.example.test",
      VITE_RAS_CODE_RELAY_URL: "https://ci.example.test",
    });
  });
...
function makeTemporaryDirectory() {
  const directory = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "ras-code-public-config-"));
