...
it.effect("ignores tags that do not match the canary format", () =>
  Effect.gen(function* () {
    const previous = yield* resolvePreviousReleaseTag("canary", "v1.2.0-canary.20260620.2", [
      "nightly-v1.2.0-nightly.20260620.1",
      "v1.1.0-canary.20260619.9",
    ]);

    assert.equal(previous, "v1.1.0-canary.20260619.9");
  }),
);

it.effect("reports the invalid tag with its release channel", () =>
  Effect.gen(function* () {
    const error = yield* resolvePreviousReleaseTag("canary", "v1.2.0", []).pipe(Effect.flip);

    assert.equal(error._tag, "InvalidReleaseTagError");
    assert.equal(error.channel, "canary");
    assert.equal(error.currentTag, "v1.2.0");
    assert.equal(error.message, "Invalid canary release tag 'v1.2.0'.");
  }),
);
