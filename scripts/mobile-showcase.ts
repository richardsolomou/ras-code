...
const ANDROID_PACKAGE = "com.richardsolomou.ras_code";
const APP_SCHEME = "ras-code";
const IOS_READY_FILENAME = "RasCodeShowcaseReadyScene";
const SERVER_HOST = "0.0.0.0";
const IOS_SIMULATOR_ARCH = NodeProcess.arch === "arm64" ? "arm64" : "x86_64";
const IOS_APP_PATH = NodePath.join(
  MOBILE_ROOT,
  ".showcase/ios-derived-data/Build/Products/Debug-iphonesimulator/RASCode.app",
);
...
const MOBILE_BUILD_ENV = {
  ...
  RAS_CODE_SHOWCASE_CAPTURE_BUILD: "1",
  JAVA_HOME:
...
async function buildIos(): Promise<string> {
  ...
  await runCommand(
    ...
    [
      "-workspace",
      NodePath.join(MOBILE_ROOT, "ios/RASCode.xcworkspace"),
      "-scheme",
      "RASCode",
      "-configuration",
      "Debug",
...
async function captureIos(
  ...
  const scenePath = NodePath.join(
    ...
    "Library/Caches/RasCodeShowcaseScene",
  );
  const readyPath = NodePath.join(
    await iosAppContainer(simulator.udid),
    "Library/Caches",
...
async function writeAndroidShowcaseScene(serial: string, scene: ShowcaseScene): Promise<void> {
  await runAdb(serial, [
    "shell",
    `run-as ${ANDROID_PACKAGE} sh -c 'mkdir -p files && rm -f files/ras-code-showcase-ready && printf %s ${scene} > files/ras-code-showcase-scene'`,
  ]);
}
...
async function main(): Promise<void> {
  ...
  const showcaseRootDir = await NodeFSP.mkdtemp(
    NodePath.join(NodeOS.tmpdir(), "ras-code-mobile-showcase-"),
  );
