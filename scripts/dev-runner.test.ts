...
const devServerInput = {
  mode: "dev:server",
  rasCodeHome: "/tmp/ras-code-dev-runner",
  browser: undefined,
...
it.layer(NodeServices.layer)("dev-runner", (it) => {
  ...
  describe("resolveOffset", () => {
    it.effect("uses explicit RAS_CODE_PORT_OFFSET when provided", () =>
      Effect.gen(function* () {
        const result = yield* resolveOffset({ portOffset: 12, devInstance: undefined });
        assert.deepStrictEqual(result, {
          offset: 12,
          source: "RAS_CODE_PORT_OFFSET=12",
        });
      }),
    );
    ...
    it.effect("returns structured context for a negative port offset", () =>
      Effect.gen(function* () {
        ...
        assert.equal(error.configKey, "RAS_CODE_PORT_OFFSET");
  ...
  describe("createDevRunnerEnv", () => {
    it.effect("leaves the shared home implicit and disables browser auto-open", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.RAS_CODE_HOME, undefined);
        assert.equal(env.RAS_CODE_NO_BROWSER, "1");
      }),
    );
    ...
    it.effect("allows browser auto-open to be explicitly enabled", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.RAS_CODE_NO_BROWSER, "0");
      }),
    );
    ...
    it.effect("supports explicit typed overrides", () =>
      Effect.gen(function* () {
        ...
        const env = yield* createDevRunnerEnv({
          ...
          serverOffset: 0,
          webOffset: 0,
          rasCodeHome: "/tmp/custom-ras-code",
          browser: false,
        ...
        assert.equal(env.RAS_CODE_HOME, path.resolve("/tmp/custom-ras-code"));
        assert.equal(env.RAS_CODE_PORT, "4222");
        assert.equal(env.VITE_HTTP_URL, "http://localhost:4222");
        assert.equal(env.VITE_WS_URL, "ws://localhost:4222");
        assert.equal(env.RAS_CODE_NO_BROWSER, "1");
        assert.equal(env.RAS_CODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD, "0");
        assert.equal(env.RAS_CODE_LOG_WS_EVENTS, "1");
        assert.equal(env.RAS_CODE_HOST, "0.0.0.0");
    ...
    it.effect("strips inherited service-launcher context", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {
            RAS_CODE_SERVICE_LAUNCHER_CONTEXT: '{"childVersion":"9.9.9"}',
            RAS_CODE_BOOT_SERVICE_UNIT: "ras-code.service",
          },
          serverOffset: 0,
          webOffset: 0,
          rasCodeHome: undefined,
        ...
        assert.equal(env.RAS_CODE_SERVICE_LAUNCHER_CONTEXT, undefined);
        assert.equal(env.RAS_CODE_BOOT_SERVICE_UNIT, undefined);
      }),
    );
    ...
    it.effect("does not force websocket logging on in dev mode when unset", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {
            RAS_CODE_LOG_WS_EVENTS: "keep-me-out",
          },
          serverOffset: 0,
          webOffset: 0,
          rasCodeHome: undefined,
        ...
        assert.equal(env.RAS_CODE_MODE, "web");
        assert.equal(env.RAS_CODE_LOG_WS_EVENTS, undefined);
      }),
    );
    ...
    it.effect("forwards explicit websocket logging false without coercing it away", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.RAS_CODE_LOG_WS_EVENTS, "0");
      }),
    );

    it.effect("uses custom rasCodeHome when provided", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const env = yield* createDevRunnerEnv({
          mode: "dev",
        ...
        assert.equal(env.RAS_CODE_HOME, path.resolve("/tmp/my-ras-code"));
      }),
    );
      ...
      Effect.gen(function* () {
        ...
        const env = yield* createDevRunnerEnv({
          ...
          baseEnv: {
            RAS_CODE_PORT: "13773",
            RAS_CODE_MODE: "web",
            RAS_CODE_NO_BROWSER: "0",
            RAS_CODE_HOST: "0.0.0.0",
        ...
        assert.equal(env.RAS_CODE_MODE, undefined);
        assert.equal(env.RAS_CODE_NO_BROWSER, undefined);
        assert.equal(env.RAS_CODE_HOST, undefined);
    ...
    it.effect("defaults dev server mode to the higher backend port range", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.RAS_CODE_PORT, "13773");
        assert.equal(env.PORT, "5733");
      }),
    );
    ...
    for (const mode of ["dev", "dev:web"] as const) {
      it.effect(`leaves the client backend URLs unset in ${mode} mode`, () =>
        Effect.gen(function* () {
          ...
          assert.equal(env.RAS_CODE_SINGLE_ORIGIN_DEV, "1");
        }),
      );
    }
    ...
    it.effect("clears the single-origin marker in dev:desktop mode", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev:desktop",
          baseEnv: { RAS_CODE_SINGLE_ORIGIN_DEV: "1" },
          serverOffset: 0,
          webOffset: 0,
          rasCodeHome: undefined,
          browser: undefined,
        ...
        assert.equal(env.RAS_CODE_SINGLE_ORIGIN_DEV, undefined);
    ...
    // --host configures the *backend* (RAS_CODE_HOST). It must not become Vite's
  ...
  describe("devPortProbeHosts", () => {
    ...
    // actually bind, so --host/RAS_CODE_HOST has to be probed as well.
    ...
    // Only the backend honours --host/RAS_CODE_HOST. Vite reads HOST (set for
  ...
  describe("runDevRunnerWithInput", () => {
    it.effect("preserves invalid configuration as the exact cause", () =>
      Effect.gen(function* () {
          ...
          Effect.provide(
            Layer.merge(
              ...
              ConfigProvider.layer(
                ConfigProvider.fromEnv({ env: { RAS_CODE_PORT_OFFSET: "not-an-integer" } }),
              ),
            ),
          ),
          Effect.flip,
        ...
        assert.deepStrictEqual(error.configKeys, ["RAS_CODE_PORT_OFFSET", "RAS_CODE_DEV_INSTANCE"]);
    ...
    // Base-dir precedence (--home-dir > worktree .ras-code > ambient RAS_CODE_HOME)
    ...
    it.effect("ignores an ambient RAS_CODE_HOME when no home is resolved", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: { RAS_CODE_HOME: "/home/user/.ras-code" },
          serverOffset: 0,
          webOffset: 0,
          rasCodeHome: undefined,
          browser: undefined,
    ...
    // defaults: an explicit RAS_CODE_BUNDLED_DEV (even "0") must pass through.
    describe("--share bundled dev default", () => {
          ...
          yield* runDevRunnerWithInput({
            ...
            Effect.provideService(
              ...
              input.ambientBundledDev === undefined
                ? {}
                : { RAS_CODE_BUNDLED_DEV: input.ambientBundledDev },
            ),
          );
      ...
      it.effect("defaults RAS_CODE_BUNDLED_DEV=1 for a shared run", () =>
        Effect.gen(function* () {
          const env = yield* shareSpawnedEnv({ ambientBundledDev: undefined });
          assert.equal(env?.RAS_CODE_BUNDLED_DEV, "1");
        }),
      );

      it.effect("keeps an explicit RAS_CODE_BUNDLED_DEV=0 opt-out", () =>
        Effect.gen(function* () {
          const env = yield* shareSpawnedEnv({ ambientBundledDev: "0" });
          assert.equal(env?.RAS_CODE_BUNDLED_DEV, "0");
        }),
      );

      it.effect("leaves RAS_CODE_BUNDLED_DEV unset without --share", () =>
        Effect.gen(function* () {
          let captured: Record<string, string | undefined> | undefined;
          const spawnerLayer = Layer.succeed(
            ChildProcessSpawner.ChildProcessSpawner,
          ...
          assert.equal(captured?.RAS_CODE_BUNDLED_DEV, undefined);
        }),
      );
    });
    ...
    describe("ras-code home precedence", () => {
      const makeWorktree = Effect.acquireRelease(
        Effect.sync(() => {
          const root = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "ras-code-devrunner-"));
      ...
      const spawnedHome = (input: {
        ...
        Effect.gen(function* () {
          ...
          yield* runDevRunnerWithInput({ ...devServerInput, rasCodeHome: input.rasCodeHome }).pipe(
            Effect.provide(Layer.mergeAll(emptyConfigLayer, netServiceLayer, spawnerLayer)),
            Effect.provideService(HostProcessPlatform, "linux"),
            Effect.provideService(HostProcessWorkingDirectory, input.cwd),
            Effect.provideService(
              HostProcessEnvironment,
              input.ambientHome === undefined ? {} : { RAS_CODE_HOME: input.ambientHome },
            ),
          );

          return captured?.RAS_CODE_HOME;
        });
      ...
      it.effect("prefers an explicit --home-dir over the worktree default", () =>
        Effect.gen(function* () {
          ...
          const home = yield* spawnedHome({
            rasCodeHome: "/tmp/explicit-home",
            cwd: root,
            ambientHome: "/home/user/.ras-code",
          });
      ...
      it.effect("treats a blank --home-dir as unset rather than as a selection", () =>
        Effect.gen(function* () {
          ...
          assert.equal(home, path.join(path.resolve(root), ".ras-code"));
        }).pipe(Effect.scoped),
      );

      it.effect("prefers the worktree .ras-code over an ambient RAS_CODE_HOME", () =>
        Effect.gen(function* () {
          const path = yield* Path.Path;
          const root = yield* makeWorktree;
          const home = yield* spawnedHome({
      ...
      it.effect("falls back to an ambient RAS_CODE_HOME outside a worktree", () =>
        Effect.gen(function* () {
          const path = yield* Path.Path;
          const home = yield* spawnedHome({
            rasCodeHome: undefined,
          ...
          assert.equal(home, path.resolve("/home/user/.ras-code"));
        }),
      );
