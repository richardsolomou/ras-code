...
export const DEFAULT_RAS_CODE_HOME = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(NodeOS.homedir(), ".ras-code"),
);
...
/**
 * `role` matters because only the backend honours `--host`/`RAS_CODE_HOST`; the
...
export class DevRunnerInvalidPortOffsetError extends Schema.TaggedErrorClass<DevRunnerInvalidPortOffsetError>()(
  ...
  {
    configKey: Schema.Literal("RAS_CODE_PORT_OFFSET"),
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("RAS_CODE_PORT_OFFSET"),
  devInstance: optionalStringConfig("RAS_CODE_DEV_INSTANCE"),
});
...
> {
  if (config.portOffset !== undefined) {
    if (config.portOffset < 0) {
      return Effect.fail(
        new DevRunnerInvalidPortOffsetError({
          configKey: "RAS_CODE_PORT_OFFSET",
    ...
    return Effect.succeed({
      ...
      source: `RAS_CODE_PORT_OFFSET=${config.portOffset}`,
    });
  }
  ...
  if (seed) {
    if (/^\d+$/.test(seed)) {
      return Effect.succeed({
        offset: Number(seed),
        source: `numeric RAS_CODE_DEV_INSTANCE=${seed}`,
      });
    }

    const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
    return Effect.succeed({ offset, source: `hashed RAS_CODE_DEV_INSTANCE=${seed}` });
  }
...
function resolveBaseDir(baseDir: string | undefined): Effect.Effect<string, never, Path.Path> {
  return Effect.gen(function* () {
    ...
    return yield* DEFAULT_RAS_CODE_HOME;
  });
}
...
interface CreateDevRunnerEnvInput {
  ...
  readonly rasCodeHome: string | undefined;
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    // Precedence (--home-dir > worktree .ras-code > ambient RAS_CODE_HOME) is resolved
    // by the caller; an unset rasCodeHome here genuinely means "use the default".
    const configuredBaseDir = rasCodeHome?.trim() || undefined;
    ...
    if (configuredBaseDir !== undefined) {
      output.RAS_CODE_HOME = resolvedBaseDir;
    } else {
      delete output.RAS_CODE_HOME;
    }

    // A dev-runner server is never launcher-managed. When the shell that runs
    // this script was itself spawned by the machine's managed ras-code service (an
    // agent working inside RAS Code), these leak through and the child server
    // fails startup with "The service launcher started a different ras-code version"
    // (serviceLauncherClient.ts resolveStartup).
    delete output.RAS_CODE_SERVICE_LAUNCHER_CONTEXT;
    delete output.RAS_CODE_BOOT_SERVICE_UNIT;

    if (!isDesktopMode) {
      output.RAS_CODE_PORT = String(serverPort);
      ...
      if (mode === "dev" || mode === "dev:web") {
        ...
        output.RAS_CODE_SINGLE_ORIGIN_DEV = "1";
      } else {
        output.VITE_HTTP_URL = `http://localhost:${serverPort}`;
        output.VITE_WS_URL = `ws://localhost:${serverPort}`;
        delete output.RAS_CODE_SINGLE_ORIGIN_DEV;
      }
    } else {
      output.RAS_CODE_PORT = String(serverPort);
      ...
      delete output.RAS_CODE_SINGLE_ORIGIN_DEV;
      delete output.RAS_CODE_MODE;
      delete output.RAS_CODE_NO_BROWSER;
      delete output.RAS_CODE_HOST;
    }

    if (!isDesktopMode && host !== undefined) {
      output.RAS_CODE_HOST = host;
    }

    if (!isDesktopMode) {
      output.RAS_CODE_NO_BROWSER = browser === true ? "0" : "1";
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.RAS_CODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.RAS_CODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.RAS_CODE_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.RAS_CODE_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.RAS_CODE_MODE = "web";
      delete output.RAS_CODE_DESKTOP_WS_URL;
    }
 ...
 * target reach it there. When `--host`/`RAS_CODE_HOST` moves the backend onto
...
export function runDevRunnerWithInput(input: DevRunnerCliInput) {
  return Effect.gen(function* () {
      ...
      Effect.mapError(
        (cause) =>
          new DevRunnerConfigurationError({
            configKeys: ["RAS_CODE_PORT_OFFSET", "RAS_CODE_DEV_INSTANCE"],
            cause,
          }),
      ),
    );
    ...
    const resolvedRasCodeHome =
      (input.rasCodeHome?.trim() || undefined) ??
      worktreeHome ??
      (hostEnvironment.RAS_CODE_HOME?.trim() || undefined);
    const env = yield* createDevRunnerEnv({
      mode: input.mode,
      baseEnv: hostEnvironment,
      serverOffset,
      webOffset,
      rasCodeHome: resolvedRasCodeHome,
    ...
    const selectionSuffix =
      serverOffset !== offset || webOffset !== offset
        ...
        : "";
    const baseDir = env.RAS_CODE_HOME ?? (yield* DEFAULT_RAS_CODE_HOME);

    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.RAS_CODE_PORT)} webPort=${String(env.PORT)} baseDir=${baseDir}`,
    );
    ...
    if (input.share) {
      if (input.mode === "dev:server") {
        ...
        if (shared) {
          ...
          env.RAS_CODE_DEV_ALLOWED_ORIGINS = [
            env.RAS_CODE_DEV_ALLOWED_ORIGINS,
          ...
          // a few chunk requests. Only defaulted, so RAS_CODE_BUNDLED_DEV=0
          // still opts a --share run back out.
          if (env.RAS_CODE_BUNDLED_DEV === undefined) {
            env.RAS_CODE_BUNDLED_DEV = "1";
          }
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  rasCodeHome: Flag.string("home-dir").pipe(
    Flag.withDescription(
      "Explicit RAS Code data directory; runtime state is stored under userdata (equivalent to RAS_CODE_HOME). Inside a git worktree this defaults to that worktree's own .ras-code so dev state stays off the shared home.",
    ),
    Flag.optional,
  ...
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to RAS_CODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("RAS_CODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription("WebSocket event logging toggle (equivalent to RAS_CODE_LOG_WS_EVENTS)."),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("RAS_CODE_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to RAS_CODE_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("RAS_CODE_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to RAS_CODE_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("RAS_CODE_PORT")),
  ),
