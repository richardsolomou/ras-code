...
export function reconcileRootEnvRelayUrl(contents: string, relayUrl: string): string {
  return reconcileRootEnvPublicConfig(contents, {
    ...
    .split("\n")
    .filter((line) => !line.startsWith("RAS_CODE_MOBILE_OTLP_TRACES_"))
    .filter((line) => !line.startsWith("RAS_CODE_RELAY_CLIENT_OTLP_TRACES_"))
    .join("\n");
}
...
const readRelayPublicConfig = Effect.fn("relay.deploy.readState")(function* (stage: string) {
  ...
  const output = yield* service.getOutput({ stack: "RasCodeRelay", stage });
...
export const relayDeployCommand = Command.make(
  ...
  deploy,
).pipe(Command.withDescription("Deploy the RAS Code relay through Alchemy."));
