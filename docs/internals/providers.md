...
RAS Code-managed OpenCode chat uses one server per thread. Its MCP registrations are directory-scoped, while
RAS Code's MCP connection is thread-scoped. Sharing a chat server between threads in one directory would
...
RAS Code auth session. The client carries the return URL back to the environment because the provider's
