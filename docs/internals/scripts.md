...
RAS Code uses [Vite+](https://viteplus.dev/guide/). Install the global `vp` command, install
...
- `vp run dev --share`: Also publishes the web port over HTTPS on this machine's tailnet. The
  ...
  Shared runs default to Vite's bundled dev mode (`RAS_CODE_BUNDLED_DEV=1`): a remote browser pays a
  network round trip per import level in unbundled dev, which turns a cold module graph into
  minutes of waterfall. Set `RAS_CODE_BUNDLED_DEV=0` to opt a shared run back out.
- `vp run dev --browser`: Auto-opens a browser. Off by default. The dev runner writes
  `RAS_CODE_NO_BROWSER` itself from this flag, so setting `RAS_CODE_NO_BROWSER=0` in your environment has
...
- Pass dev-runner flags directly after the root task name, for example:
  `vp run dev --home-dir /tmp/ras-code-dev`

### Dev state directories

- Dev commands run from a linked **git worktree** default to that worktree's gitignored `.ras-code`, even
  when `RAS_CODE_HOME` is set, storing state in `<worktree>/.ras-code/userdata`. Pass `--home-dir <path>` to
  choose another isolated directory explicitly. Submodules are not worktrees and keep the normal
  precedence.
- From the **main checkout**, dev commands implicitly use `~/.ras-code/dev`, keeping development state
  separate from `~/.ras-code/userdata`. An explicit `--home-dir <path>` stores state under
  `<path>/userdata`; the base directory remains available for caches, worktrees, and other shared
  data.

## Build, check, test

- `vp run build`: Fans out over `apps/*`, `packages/*`, `oxlint-plugin-ras-code`, and `scripts`.
...
- `node apps/server/scripts/ras-sqlite-state.ts <query|exec> --base-dir <path> ...`: Inspects or seeds
  an isolated RAS Code SQLite database; writes create a private backup first.
...
When `RAS_CODE_DESKTOP_REUSE_RESOURCE_MONITOR=true` points the build at an existing resource monitor,
...
- The DMG chrome follows the release channel: neutral for Latest and the Canary sky artwork for
  Canary. Blueprint artwork remains exclusive to Dev builds. Packaging rasterizes the selected
...
- Desktop production windows load the bundled UI from the `ras-code://app/` root URL (not a
  `127.0.0.1` document URL, and not an explicit `index.html` path).
- Desktop packaging includes `apps/server/dist` (the `ras-code` backend) and starts it on loopback with an
...
- Signed macOS builds also require `RAS_CODE_APPLE_TEAM_ID` and
  `RAS_CODE_MACOS_PROVISIONING_PROFILE`. The passkey RP domain is derived from
  `RAS_CODE_CLERK_PUBLISHABLE_KEY` unless `RAS_CODE_CLERK_PASSKEY_RP_DOMAINS` overrides it.
...
- Example: `RAS_CODE_DEV_INSTANCE=branch-a vp run dev:desktop`

Offset resolution, in order:

1. `RAS_CODE_PORT_OFFSET`, which must be a non-negative integer. Negative values are rejected.
2. `RAS_CODE_DEV_INSTANCE`. An all-digit value is used directly as the offset; any other non-empty
