...
RAS Code has one runtime boundary: a client talks to a RAS Code server over HTTP and WebSocket, and the server
...
┌───────────────▼──────────────────────────────┐
│ Execution environment = one RAS Code server   │
...
One running RAS Code server instance. It owns provider availability and auth, model availability, projects
...
Tailscale is the first provider, and RAS Code manages more than discovery. When `tailscaleServeEnabled` is
...
(`apps/server/src/server.ts`, using [`@ras-code/tailscale`](../../packages/tailscale/src/tailscale.ts)).
...
Access answers one question: how does the client speak WebSocket to a RAS Code server? It does not answer
how the server got started or who manages the process.

### Direct WebSocket access

`wss://ras.example.com` or `ws://10.0.0.15:3773`, paired as a bearer target. This is the base model.
...
A RAS Code-managed `tailscale serve` mapping exposes the server on the tailnet over HTTPS, and the
...
[packages/ssh/src/tunnel.ts][sshtunnel], which resolves the target, launches or reuses the remote RAS Code
...
- **Client-managed local publish.** A local server is published through the relay with
  `ras connect link`, exposing a desktop-hosted environment to mobile without router or firewall
  changes.
