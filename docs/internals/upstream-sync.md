...
[`scripts/lib/upstreamRebrandMap.ts`](../../scripts/lib/upstreamRebrandMap.ts) is the single source of truth for the fork's vocabulary: `T3CODE_*` to `RAS_CODE_*`, `T3 Code` to `RAS Code`, `com.t3tools.t3code` to our reverse-DNS ids, the CLI's own name where shell text calls it, the mobile native module directories, and the rest. `@t3tools/*` is ours and stays. It also holds the do-not-rename list, which is matched first and copied through untouched.
