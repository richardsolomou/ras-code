...
The launcher is the only runtime writer of `service-state.json`. `ras service install` and
`ras service update` may replace the launcher and state while the unit is stopped. Server children
...
1. The active server installs `ras-code@<target>` into a unique staging directory.
...
```sh
npx ras-code@<version> service update
```
