...
> For maintainers. Using RAS Code? See [Quote an assistant response](../user/composer.md#quote-an-assistant-response).
...
```text
[Assistant quote](ras-code-citation://v1/<environmentId>/<threadId>/<messageId>?text=<quote>&start=<start>&end=<end>&prefix=<prefix>&suffix=<suffix>&comment=<optional-user-comment>)
```
