...
RAS Code has one server-side observability model:
...
stdout/stderr at `~/.ras-code/ssh-launch/<state>/server.log`.
...
- SSH-managed launch persistence: `~/.ras-code/ssh-launch/<state>/server.log`
...
`<home>/userdata/logs/server.trace.ndjson` (so `~/.ras-code/userdata/...` by default, or
`/custom/path/userdata/...` with `--home-dir /custom/path`), a linked worktree dev run uses
`<worktree>/.ras-code/userdata/logs/server.trace.ndjson`, and an implicit dev run outside a linked
worktree uses `~/.ras-code/dev/logs/server.trace.ndjson`.
...
```bash
export RAS_CODE_OTLP_TRACES_URL=http://localhost:4318/v1/traces
export RAS_CODE_OTLP_METRICS_URL=http://localhost:4318/v1/metrics
export RAS_CODE_OTLP_SERVICE_NAME=ras-code-local
```

Optional:

```bash
export RAS_CODE_TRACE_MIN_LEVEL=Info
export RAS_CODE_TRACE_TIMING_ENABLED=true
```
...
Launch the actual app executable from the same shell so the desktop app and embedded backend inherit `RAS_CODE_OTLP_*`.

macOS app bundle example:

```bash
RAS_CODE_OTLP_TRACES_URL=http://localhost:4318/v1/traces \
RAS_CODE_OTLP_METRICS_URL=http://localhost:4318/v1/metrics \
RAS_CODE_OTLP_SERVICE_NAME=ras-code-desktop \
"/Applications/RAS Code.app/Contents/MacOS/RAS Code"
```
...
```bash
TRACE_FILE="${RAS_CODE_HOME:-$HOME/.ras-code}/userdata/logs/server.trace.ndjson"
```

A dev server started from a linked worktree defaults to that worktree's local home:

```bash
TRACE_FILE="$WORKTREE/.ras-code/userdata/logs/server.trace.ndjson"
```

Only an implicit dev run outside a linked worktree uses the shared dev directory:

```bash
TRACE_FILE="$HOME/.ras-code/dev/logs/server.trace.ndjson"
```
...
- service name such as `ras-code-local`, `ras-code-dev`, or `ras-code-desktop`
...
- `ras_rpc_request_duration`
- `ras_orchestration_command_duration`
- `ras_orchestration_command_ack_duration`
- `ras_provider_turn_duration`
- `ras_git_command_duration`

Counters tell you volume and failure rate:

- `ras_rpc_requests_total`
- `ras_orchestration_commands_total`
- `ras_provider_turns_total`
- `ras_git_commands_total`
...
`ras_orchestration_command_ack_duration` measures:
...
1. Check `ras_orchestration_command_ack_duration` by `commandType`.
...
- `RAS_CODE_OTLP_TRACES_URL` was not set
...
- `RAS_CODE_TRACE_FILE`: override trace file path
- `RAS_CODE_TRACE_MAX_BYTES`: per-file rotation size, default `10485760`
- `RAS_CODE_TRACE_MAX_FILES`: rotated file count, default `10`
- `RAS_CODE_TRACE_BATCH_WINDOW_MS`: flush window, default `200`
- `RAS_CODE_TRACE_MIN_LEVEL`: minimum trace level, default `Info`
- `RAS_CODE_TRACE_TIMING_ENABLED`: enable timing metadata, default `true`

OTLP export:

- `RAS_CODE_OTLP_TRACES_URL`: OTLP trace endpoint
- `RAS_CODE_OTLP_METRICS_URL`: OTLP metric endpoint
- `RAS_CODE_OTLP_EXPORT_INTERVAL_MS`: export interval, default `10000`
- `RAS_CODE_OTLP_SERVICE_NAME`: service name, default `ras-code-server`
