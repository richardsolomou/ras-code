...
Deployment and client configuration for RAS Connect. The [architecture note](../internals/ras-connect.md)
explains the trust boundaries; the [relay README](../../infra/relay/README.md#deployment) owns relay
provisioning instructions.

## Public application configuration

RAS Connect is disabled in a fresh clone. To build against the production deployment, copy the
...
1. Create a public OAuth application for the RAS Code CLI, using authorization-code exchange with PKCE.
2. Allow both redirect URIs: `http://127.0.0.1:34338/callback` and
   `https://app.t3.codes/connect/callback`. A custom `RAS_CODE_HOSTED_APP_URL` needs its own
...
4. Set `RAS_CODE_CLERK_CLI_OAUTH_CLIENT_ID` to the generated public client ID in local and release
   build environments.

## JWT template

Create a Clerk JWT template named `ras-code-relay` with claims:

```json
{ "aud": "ras-code-relay" }
```

Set `RAS_CODE_CLERK_JWT_TEMPLATE=ras-code-relay` for clients and
`CLERK_JWT_AUDIENCE=ras-code-relay` for the relay. The production relay deployment environment
...
Development uses `ras-code-dev://app`; production uses `ras-code://app`. Update the array with
...
For a production macOS app with bundle ID `com.richardsolomou.ras-code`:
...
4. Check `https://<frontend-api>/.well-known/apple-app-site-association`. Its
   `webcredentials.apps` must include `<TEAM_ID>.com.richardsolomou.ras-code`.
