...
Flags go directly after the task name, for example `vp run dev --home-dir /tmp/ras-code-dev`.
Add `--browser` to open a browser automatically.

### State and ports

Linked worktrees default to their own `.ras-code/userdata`, even when `RAS_CODE_HOME` is set.
The main checkout defaults to `~/.ras-code/dev/userdata`. An explicit `--home-dir` wins in both cases.
Never run a development server against the live `~/.ras-code/userdata`.
...
but occupied ports can shift them. `RAS_CODE_PORT_OFFSET` or `RAS_CODE_DEV_INSTANCE` can select a
...
`RAS_CODE_BUNDLED_DEV=0` opts out when debugging bundler differences. Two reload traps matter
...
DMG chrome follows the release channel: neutral for Latest, the Canary sky artwork for Canary, and
the Blueprint artwork only for Dev builds. Packaging rasterizes the selected SVG into standard and
Retina PNGs inside the disposable staging directory. The Finder window is 540×412 against a 540×380
background; the extra 32px is the title bar Finder counts in window bounds.

A production desktop window loads the bundled UI from the `ras-code://app/` root URL, not a
`127.0.0.1` document URL and not an explicit `index.html` path. Packaging includes `apps/server/dist`
and starts it on loopback with an auth token for WebSocket and API traffic. An unsigned build opens
on macOS by right-clicking the app and choosing **Open** on first launch.
