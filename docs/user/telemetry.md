...
RAS Code does not send prompts, responses, repository contents, terminal output, provider error messages, abort reasons, secrets, IP address properties, local variables, source context, absolute source paths, conversation identifiers, raw provider events, or child-agent output. Token totals cover the main agent only and can be complete, partial, or unavailable, depending on what the provider reports. It does not use PostHog surveys, advertising, or person profiles.
