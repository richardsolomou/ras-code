...
RAS Code runs Google's official Antigravity ACP agent. By default it signs in with your personal
...
not sign in this RAS Code provider.
...
2. Choose **Install Antigravity**. RAS Code downloads the official runtime from Google to that
...
6. Wait for RAS Code to confirm sign-in and load the model choices. Select an Antigravity model
...
machine as the RAS Code environment, sign-in can finish directly.
...
return URL field in RAS Code. Choose **Continue** on web or desktop, or **Complete sign-in** on
mobile. Do not change the address to your server's hostname.

Return to the same RAS Code client and environment where you started sign-in. Another client
...
a thread or bug report. RAS Code waits for Google's agent to confirm sign-in. A successful
...
are stored in plain text in RAS Code settings on that environment and are passed only to the
...
RAS Code does not download it until you choose to install it.

On web or desktop, **Update Antigravity** appears when RAS Code has a newer managed release.
...
A nonempty **Binary path** takes priority. With the field empty, RAS Code uses its managed
runtime, then an installation on the environment's `PATH`. An invalid explicit path reports
an error instead of selecting another installation. RAS Code does not update or remove manual
installations. Clear **Binary path** to use managed installation controls.

## Models and threads

The current official ACP exposes Gemini models only. RAS Code uses the model IDs and names
...
RAS Code asks you to select an available model instead of silently changing it.

Use Antigravity's native `/plan` command to request a plan. RAS Code's separate Plan mode control
is not available for this provider.

Project skills should use `.agents/skills`. RAS Code also discovers `.gemini/skills` and the legacy
...
Antigravity reads and edits workspace files through RAS Code. Each write shows up as a file
...
When the agent offers **Allow for this thread** on a shell or web tool, RAS Code shows Google's
...
RAS Code keeps thread history and file diffs. Antigravity does not support conversation rewind,
...
or reply ownership. RAS Code cannot show separate child entries or separate child replies
...
installation. RAS Code refuses removal while the runtime is in use. An explicit **Binary path**
that points into the managed runtime must be cleared first.

## Account access and errors

Google controls account eligibility, models, and usage limits. RAS Code does not report your
...
Use any retry time Google supplies. RAS Code does not switch to an API key to get past the limit.
