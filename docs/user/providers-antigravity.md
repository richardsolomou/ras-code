...
RAS Code runs Google's official Antigravity ACP agent on your selected environment.
...
**Sign in with Google** and complete the browser sign-in. Wait for RAS Code to confirm
...
hostname. Only that RAS Code sign-in session can finish the attempt. If it expires,
...
Use Antigravity's native `/plan` command for planning. RAS Code's separate Plan mode
...
RAS Code keeps conversation history and file diffs, but Antigravity cannot rewind
...
Put project skills in `.agents/skills`. RAS Code also reads `.gemini/skills` and the
