...
On web and desktop, choose **Create theme** to adjust a palette, or import a RAS Code or VS Code
...
takes precedence. If the server stops publishing the selected theme, RAS Code falls back to its
standard theme.

Run this on the server to set a default and switch connected clients to it:

```bash
ras-code theme set nightfall
```

Clients that are offline apply it when they reconnect. Each client applies the setting once;
choosing another theme afterward sticks until the next `ras-code theme set`. Run the command again to
reapply it, even if the name is unchanged.

`ras-code theme clear` removes the default without changing anyone's current theme. `ras-code theme show` lists
the default and published themes.

### Publish a theme

Save a theme exported from RAS Code into `~/.ras-code/userdata/themes/` on the server, or the `themes`
directory under your custom state directory. The filename supplies the theme ID: `nightfall.json`
can be selected with `ras-code theme set nightfall`. Keep the filename stable when updating its colors.
...
Set `appearance` to `light` or `dark` and supply hex colors for `canvas` and `accent`. RAS Code
