...
Messages can contain up to 120,000 characters. If a draft is longer, RAS Code keeps it in the
...
RAS Code from other apps through the system share sheet. Mobile keeps a local copy of each draft
attachment, so you can still preview it and queue messages while offline. Uploads resume when
you reconnect. Drafts and queued messages survive app restarts; signing out of RAS Connect keeps
...
Model options shown as provider defaults remain display values until you choose them in RAS Code.
RAS Code only sends options you selected explicitly, so an unset reasoning level or service tier can
...
Videos opened from the file explorer or a file-viewer tab also play inside RAS Code. They
...
RAS Code serves the original file without adding it to attachment storage. If that file is moved
...
project exists there. If it does not, RAS Code selects another environment that has the project.
...
and attachments. RAS Code deletes the local audio file after transcription or cancellation. It sends
...
also reports that skill as a native slash command, RAS Code hides the duplicate native entry and keeps
the `/skill:Skill Name` label.

A skill token runs the skill wherever it sits in your message. RAS Code sends it to each provider in
...
them only there. RAS Code's own commands, such as `/model` and `/plan`, and skills stay available on
any line.

On desktop, press `Cmd+Enter` on macOS or `Ctrl+Enter` on Windows and Linux from a new thread to
start it in the background. RAS Code opens another new thread and shows an **Open** action for the
