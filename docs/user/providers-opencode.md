...
RAS Code requires OpenCode 1.14.19 or newer, including when you connect an existing
OpenCode server.

## Local or external server

Leave **Server URL** empty to let RAS Code start OpenCode locally. A password in
provider settings applies to both that server and RAS Code's connection. With no
...
settings. RAS Code uses only that configured password for an external server; it
...
restart before RAS Code can see configuration changes.
