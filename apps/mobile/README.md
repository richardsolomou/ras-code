...
> [!WARNING]
> RAS Code Mobile is currently in development and is not distributed yet. If you want to try it out, you can build it from source.
...
- `development`: Expo dev client, installable side-by-side as `RAS Code Dev`
- `preview`: persistent internal preview build, installable side-by-side as `RAS Code Preview`
- `production`: store/release build as `RAS Code`

Run commands from `apps/mobile`.

RAS Connect is optional and disabled in a fresh clone. Public configuration belongs in the
...
```bash
RAS_CODE_IOS_PERSONAL_TEAM=1 \
RAS_CODE_IOS_PERSONAL_TEAM_BUNDLE_ID=com.example.ras-code.dev \
vp run ios:dev
```
...
```bash
RAS_CODE_IOS_PERSONAL_TEAM=1 \
RAS_CODE_IOS_PERSONAL_TEAM_BUNDLE_ID=com.example.ras-code \
vp run ios:release
```
...
For preview or production EAS environments, set `RAS_CODE_CLERK_PUBLISHABLE_KEY`,
`RAS_CODE_CLERK_JWT_TEMPLATE`, and `RAS_CODE_RELAY_URL`
