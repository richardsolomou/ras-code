...
#import "RasCodeMarkdownTextShadowNode.h"
#import "RasCodeMarkdownTextComponentDescriptor.h"
...
static void RasCodeMarkdownTextApplyParagraphStyles(
    NSMutableAttributedString *attributedString,
    const std::vector<RasCodeMarkdownTextParagraphStyleRange> &styleRanges)
{
...
static void RasCodeMarkdownTextApplyAttachments(
    NSMutableAttributedString *attributedString,
    const std::vector<RasCodeMarkdownTextAttachmentRange> &attachmentRanges,
    NSDictionary<NSString *, UIImage *> *images)
{
  for (const auto &attachmentRange : attachmentRanges) {
    ...
    const CGFloat attachmentSize = RasCodeMarkdownTextAttachmentSize(attachmentRange);
    attachment.bounds = CGRectMake(
        0,
        RasCodeMarkdownTextAttachmentBaselineOffset(attachmentRange),
        attachmentSize,
        attachmentSize);
...
@protocol RasCodeMarkdownOutsideTapTarget <NSObject>
- (void)clearSelectionForOutsideTapWithHitView:(UIView *)hitView;
@end

@interface RasCodeMarkdownOutsideTapCoordinator : NSObject <UIGestureRecognizerDelegate>

- (instancetype)initWithWindow:(UIWindow *)window;
- (void)addTarget:(id<RasCodeMarkdownOutsideTapTarget>)target;
- (void)removeTarget:(id<RasCodeMarkdownOutsideTapTarget>)target;

@end

static const void *RasCodeMarkdownOutsideTapCoordinatorKey =
    &RasCodeMarkdownOutsideTapCoordinatorKey;

@implementation RasCodeMarkdownOutsideTapCoordinator {
  __weak UIWindow *_window;
  UITapGestureRecognizer *_recognizer;
  NSHashTable<id<RasCodeMarkdownOutsideTapTarget>> *_targets;
}
...
- (void)removeTarget:(id<RasCodeMarkdownOutsideTapTarget>)target
{
  ...
  if (objc_getAssociatedObject(window, RasCodeMarkdownOutsideTapCoordinatorKey) == self) {
...
- (void)handleTap:(UITapGestureRecognizer *)sender
{
  ...
  for (id<RasCodeMarkdownOutsideTapTarget> target in _targets.allObjects) {
...
static RasCodeMarkdownOutsideTapCoordinator *
RasCodeMarkdownOutsideTapCoordinatorForWindow(UIWindow *window)
{
  RasCodeMarkdownOutsideTapCoordinator *coordinator =
      objc_getAssociatedObject(window, RasCodeMarkdownOutsideTapCoordinatorKey);
  if (coordinator == nil) {
    coordinator = [[RasCodeMarkdownOutsideTapCoordinator alloc] initWithWindow:window];
    objc_setAssociatedObject(
        window,
        RasCodeMarkdownOutsideTapCoordinatorKey,
        coordinator,
...
@interface RasCodeMarkdownText () <RCTRasCodeMarkdownTextViewProtocol, UIGestureRecognizerDelegate, UITextViewDelegate>

@end

@interface RasCodeMarkdownText () <RasCodeMarkdownOutsideTapTarget>
@end

@implementation RasCodeMarkdownText {
  UIView * _view;
  UITextView * _textView;
  RasCodeMarkdownTextShadowNode::ConcreteState::Shared _state;
...
{
  return concreteComponentDescriptorProvider<RasCodeMarkdownTextComponentDescriptor>();
}

- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    static const auto defaultProps = std::make_shared<const RasCodeMarkdownTextProps>();
    _props = defaultProps;
...
- (void)didMoveToWindow
{
  ...
  if (_outsideTapWindow != nil) {
    RasCodeMarkdownOutsideTapCoordinator *coordinator =
        objc_getAssociatedObject(_outsideTapWindow, RasCodeMarkdownOutsideTapCoordinatorKey);
    [coordinator removeTarget:self];
  }
  _outsideTapWindow = self.window;
  if (_outsideTapWindow != nil) {
    [RasCodeMarkdownOutsideTapCoordinatorForWindow(_outsideTapWindow) addTarget:self];
  }
}
...
- (void)drawRect:(CGRect)rect
{
  ...
  const auto &props = *std::static_pointer_cast<RasCodeMarkdownTextProps const>(_props);
  ...
  RasCodeMarkdownTextApplyParagraphStyles(
      convertedAttrString,
      _state->getData().paragraphStyleRanges);
  RasCodeMarkdownTextApplyAttachments(
      convertedAttrString,
      _state->getData().attachmentRanges,
      _attachmentImages);
  NSUInteger runLocation = 0;
  for (UIView *child in self.subviews) {
    if (![child isKindOfClass:[RasCodeMarkdownTextRun class]]) {
      continue;
    }

    RasCodeMarkdownTextRun *textChild = (RasCodeMarkdownTextRun *)child;
    ...
    NSURL *link = [NSURL URLWithString:
        [NSString stringWithFormat:@"ras-code-markdown-run://%ld", (long)textChild.tag]];
    if (link != nil) {
  ...
  if (_eventEmitter != nullptr) {
    std::dynamic_pointer_cast<const facebook::react::RasCodeMarkdownTextEventEmitter>(_eventEmitter)
    ->onTextLayout(facebook::react::RasCodeMarkdownTextEventEmitter::OnTextLayout{static_cast<int>(self.tag), lines});
  };
}

- (void)loadAttachmentImages:(const std::vector<RasCodeMarkdownTextAttachmentRange> &)attachmentRanges
{
  for (const auto &attachmentRange : attachmentRanges) {
    NSString *imageUri = [NSString stringWithUTF8String:attachmentRange.imageUri.c_str()];
    if ([imageUri hasPrefix:@"sf:"]) {
...
{
  const auto &oldViewProps = *std::static_pointer_cast<RasCodeMarkdownTextProps const>(_props);
  const auto &newViewProps = *std::static_pointer_cast<RasCodeMarkdownTextProps const>(props);
  ...
  if (oldViewProps.ellipsizeMode != newViewProps.ellipsizeMode) {
    if (newViewProps.ellipsizeMode == RasCodeMarkdownTextEllipsizeMode::Head) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingHead;
    } else if (newViewProps.ellipsizeMode == RasCodeMarkdownTextEllipsizeMode::Middle) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingMiddle;
    } else if (newViewProps.ellipsizeMode == RasCodeMarkdownTextEllipsizeMode::Tail) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingTail;
    } else if (newViewProps.ellipsizeMode == RasCodeMarkdownTextEllipsizeMode::Clip) {
...
{
  _state = std::static_pointer_cast<const RasCodeMarkdownTextShadowNode::ConcreteState>(state);
  [self setNeedsDisplay];
}
...
- (nullable RasCodeMarkdownTextRun *)childForCharacterRange:(NSRange)characterRange
{
...
- (RasCodeMarkdownTextRun*)getTouchChild:(CGPoint)location
{
...
{
  RasCodeMarkdownTextRun *child = [self childForCharacterRange:textItem.range];
  if (![child hasContextMenu]) {
    return defaultAction;
  }

  __weak RasCodeMarkdownTextRun *weakChild = child;
...
- (void)textViewDidChangeSelection:(UITextView *)textView
{
  ...
  std::dynamic_pointer_cast<const facebook::react::RasCodeMarkdownTextEventEmitter>(_eventEmitter)
    ->onSelectionChange(facebook::react::RasCodeMarkdownTextEventEmitter::OnSelectionChange{
...
Class<RCTComponentViewProtocol> RasCodeMarkdownTextCls(void)
{
  return RasCodeMarkdownText.class;
}
