...
final class RasCodeNativeFilePresentation: NSObject, QLPreviewControllerDataSource,
  QLPreviewControllerDelegate, UIAdaptivePresentationControllerDelegate {
  let identifier: String
  private var controller: UIViewController?
  private let completion: (Error?) -> Void
  private weak var sources: RasCodePresentationSources?
  ...
  init(identifier: String, sources: RasCodePresentationSources, sourceIdentifier: String, completion: @escaping (Error?) -> Void) {
  ...
  nonisolated private static func prepareFile(url: URL, title: String) async throws -> URL {
    ...
    let directory = FileManager.default.temporaryDirectory.appendingPathComponent("ras-code-preview-\(UUID().uuidString)")
