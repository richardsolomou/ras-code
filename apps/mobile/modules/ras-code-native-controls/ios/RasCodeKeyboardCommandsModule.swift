...
public final class RasCodeKeyboardCommandsModule: Module {
  public func definition() -> ModuleDefinition {
    Name("RasCodeKeyboardCommands")

    View(RasCodeKeyboardCommandsView.self) {
      Prop("enabledCommands") { (view: RasCodeKeyboardCommandsView, commands: [String]) in
...
public final class RasCodeKeyboardCommandsView: ExpoView {
  ...
  @objc private func reclaimFirstResponderIfAvailable() {
    DispatchQueue.main.async { [weak self] in
      guard let self, self.window?.rasCodeFirstResponder == nil else { return }
...
private extension UIView {
  var rasCodeFirstResponder: UIResponder? {
    if isFirstResponder { return self }
    for subview in subviews {
      if let responder = subview.rasCodeFirstResponder { return responder }
    }
    return nil
  }
}
