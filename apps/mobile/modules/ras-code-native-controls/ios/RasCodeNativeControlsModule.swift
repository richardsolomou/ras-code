...
public final class RasCodeNativeControlsModule: Module {
  private let presentationSources = RasCodePresentationSources()
  private var videoPresentation: RasCodeNativeVideoPresentation?
  private var filePresentation: RasCodeNativeFilePresentation?
  ...
  public func definition() -> ModuleDefinition {
    ...
    View(RasCodePresentationSourceView.self) {
      ViewName("PresentationSource")
      Prop("identifier") { (view: RasCodePresentationSourceView, identifier: String) in
    ...
    Function("getShowcaseScene") { () -> String? in
      let scenePath = NSHomeDirectory() + "/Library/Caches/RasCodeShowcaseScene"
    ...
    AsyncFunction("applyShowcaseOrientation") { (orientation: String) in
      ...
      for case let windowScene as UIWindowScene in UIApplication.shared.connectedScenes {
        windowScene.requestGeometryUpdate(.iOS(interfaceOrientations: mask)) { error in
          NSLog("RasCodeNativeControls applyShowcaseOrientation(\(orientation)) failed: \(error)")
        }
    ...
    Function("markShowcaseReady") { (scene: String) in
      let readyPath = NSHomeDirectory() + "/Library/Caches/RasCodeShowcaseReadyScene"
  ...
  private func presentVideo(url: URL, title: String, sourceIdentifier: String, identifier: String, promise: Promise) throws {
    ...
    guard videoPresentation == nil, filePresentation == nil,
      ...
      throw NSError(
        domain: "RasCodeNativeVideo",
        code: 2,
        userInfo: [NSLocalizedDescriptionKey: "The video preview is no longer available."]
      )
    }
    let presentation = RasCodeNativeVideoPresentation(identifier: identifier, url: url, title: title) { [weak self] error in
  ...
  private func shareFile(url: URL, title: String, sourceIdentifier: String, promise: Promise) throws {
    guard let presenter = appContext?.utilities?.currentViewController() else {
      throw NSError(
        domain: "RasCodeNativePresentation",
        code: 2,
