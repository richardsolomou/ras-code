...
final class RasCodeNativeVideoPresentation: NSObject, AVPlayerViewControllerDelegate,
  ...
  func present(from presenter: UIViewController, sources: RasCodePresentationSources, sourceIdentifier: String) {
    let audioSession = AVAudioSession.sharedInstance()
    previousAudioSession = AudioSessionConfiguration(audioSession)
    do {
      try audioSession.setCategory(.playback, mode: .moviePlayback)
    } catch {
      NSLog("RAS Code video audio session: %@", error.localizedDescription)
    }
