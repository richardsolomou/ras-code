# RAS Code Mobile Terminal Native Module
...
custom-I/O and live-padding branch. `RasCodeTerminalView` owns a `libghostty` surface and uses that
callback I/O model:
...
`ras-code/custom-io-ordered-feed` branch when vendored, based on VVTerm's
`vvterm/custom-io-padding` branch).

```bash
apps/mobile/modules/ras-code-terminal/scripts/build-libghostty-ios16.sh
```
...
```bash
apps/mobile/modules/ras-code-terminal/scripts/build-libghostty-android.sh
```
