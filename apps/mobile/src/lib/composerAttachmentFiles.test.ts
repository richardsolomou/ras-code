...
describe("owned attachment paths", () => {
  it.each([
    ...
    const oldUri = `${prefix}${OLD_CONTAINER}/Documents/ras-code-composer-attachments/${FILE_NAME}`;
    const documentUri = `${prefix}${CURRENT_CONTAINER}/Documents/`;
    const currentUri = `${documentUri}ras-code-composer-attachments/${FILE_NAME}`;
  ...
  it("recognizes the private/var alias without changing the stored filename", () => {
    const oldUri = `file:///private/var/mobile/Containers/Data/Application/${OLD_CONTAINER}/Documents/ras-code-composer-attachments/${FILE_NAME}`;
  ...
  it.each([
    `file:///private/var/mobile/Containers/Shared/FileProvider/other/Documents/ras-code-composer-attachments/${FILE_NAME}`,
    `file:///var/mobile/Containers/Shared/AppGroup/other/ras-code-composer-attachments/${FILE_NAME}`,
    `file:///var/mobile/Containers/Data/Application/${OLD_CONTAINER}/Documents/report.pdf`,
    `file:///var/mobile/Containers/Data/Application/${OLD_CONTAINER}/Documents/ras-code-composer-attachments/report.pdf`,
    `file:///downloads/ras-code-composer-attachments/${FILE_NAME}`,
    `content://shared/ras-code-composer-attachments/${FILE_NAME}`,
    `https://example.com/ras-code-composer-attachments/${FILE_NAME}`,
    `file:///var/mobile/Containers/Data/Application/${OLD_CONTAINER}/Documents/ras-code-composer-attachments/..%2F..%2Fsender.pdf`,
    `file:///var/mobile/Containers/Data/Application/${OLD_CONTAINER}/Documents/ras-code-composer-attachments/${FILE_NAME}%2Fnested.pdf`,
