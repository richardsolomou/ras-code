...
describe("buildThreadFeed", () => {
  ...
  it.each([
    {
      ...
      label: "Call MCP tool",
      title: "Call MCP tool",
      item: { server: "ras-code", tool: "preview_navigate" },
      status: "inProgress",
    ...
    },
    {
      ...
      label: "Call MCP tool",
      title: "Call MCP tool",
      item: { server: "ras-code", tool: "task_status" },
      status: "inProgress",
    ...
    },
    {
      ...
      label: "Call MCP tool",
      title: "mcp__ras-code__preview_snapshot",
      item: undefined,
      status: "inProgress",
    ...
    },
    {
      ...
      label: "mcp__ras-code__task_status",
      title: undefined,
      item: undefined,
      status: "inProgress",
      displayName: "Getting delegated task status",
    ...
    },
    {
      ...
      label: "Call MCP tool",
      title: "Call MCP tool",
      item: { server: "ras-code", tool: "preview_click" },
      status: undefined,
  ...
  it("retains Claude MCP metadata behind friendly row and running labels", () => {
    ...
    const toolData = {
      toolName: "mcp__ras-code__preview_click",
          ...
          makeActivity({
            ...
            payload: {
              ...
              ...(index < commandCount
                ...
                : {
                    itemType: "mcp_tool_call",
                    data: { item: { server: "ras-code", tool: "preview_click" } },
                  }),
            },
          }),
        ),
...
describe("provider fallback work-log rows", () => {
  it("renders the payload sentence over two lines instead of the server summary", () => {
    ...
    const renamed = buildThreadFeed(thread, {
      resolveInstanceName: (instanceId) =>
        instanceId === "codex" ? "Codex" : "PostHog AI Gateway (EU)",
    });
    expect(
      renamed.flatMap((entry) => (entry.type === "activity-group" ? entry.activities : []))[0]
        ?.summary,
    ).toContain("PostHog AI Gateway (EU)");
  });
});
