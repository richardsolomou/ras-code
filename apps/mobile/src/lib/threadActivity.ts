...
// Keyed by resolver too: the same activities render different instance names
// once the provider list changes, and the resolver's identity tracks that list.
const activityEntriesCache = new WeakMap<
  ...
  >
>();
const identityInstanceName = (instanceId: string) => instanceId;
...
function getThreadFeedActivityEntries(
  ...
  const resolver = resolveInstanceName ?? identityInstanceName;
  let byResolver = activityEntriesCache.get(activities);
  if (!byResolver) {
    byResolver = new WeakMap();
    activityEntriesCache.set(activities, byResolver);
  }
  const cached = byResolver.get(resolver);
  if (cached) return cached;
  const entries = deriveWorkLogEntries(activities, resolver).map(toThreadFeedActivityEntry);
  byResolver.set(resolver, entries);
  return entries;
}
