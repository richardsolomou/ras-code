...
describe("prepareTurnAttachments", () => {
  ...
  it("reads restored images from the current iOS document container for legacy sends", async () => {
    ...
    const oldUri = `file:///var/mobile/Containers/Data/Application/11111111-1111-4111-8111-111111111111/Documents/ras-code-composer-attachments/${fileName}`;
    const prepared = await prepareTurnAttachments({
      environmentId,
  ...
  it("uploads image bytes over HTTP while retaining the durable offline image", async () => {
    ...
    expect(mocks.writeFile).toHaveBeenCalledWith("file:///cache/ras-code-upload-uuid", "YWJj", {
      encoding: "base64",
    });
    expect(mocks.upload).toHaveBeenCalledWith(
      "file:///cache/ras-code-upload-uuid",
      "https://environment.example/api/attachments/upload/signed",
      expect.objectContaining({ headers: { "Content-Type": "image/png" } }),
    );
    expect(mocks.deleteFile).toHaveBeenCalledExactlyOnceWith("file:///cache/ras-code-upload-uuid");
    expect(prepared.status).toBe("ready");
    if (prepared.status !== "ready") return;
    expect(prepared.attachments).toEqual([
      {
