...
describe("resolveNativeReviewDiffView", () => {
  ...
  it("returns the payload bridge when the native review diff view is installed", async () => {
    ...
    expect(expoMocks.requireNativeView).toHaveBeenCalledWith("RasCodeReviewDiffSurface");
  });
  ...
  it("does not fall back to stale legacy native review diff view names", async () => {
    globalThis.expo = {
      getViewConfig: vi.fn().mockImplementation((moduleName: string) => {
        if (moduleName === "RasCodeReviewDiffView") {
  ...
  it("returns null when the view manager cannot be required", async () => {
    ...
    expect(consoleError).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        nativeModuleName: "RasCodeReviewDiffSurface",
        cause,
      }),
    );
  ...
  it("recognizes registration races for the installed native view name", async () => {
    ...
    expect(
      isPendingNativeViewRegistration(
        new Error("Unable to find the 'RasCodeReviewDiffSurface' view for this native tag"),
      ),
    ).toBe(true);
    expect(
      isPendingNativeViewRegistration(
        new Error("Unable to find the 'RasCodeReviewDiffView' view for this native tag"),
      ),
    ).toBe(false);
    expect(
      isPendingNativeViewRegistration(
        new Error(
          "Unable to find the class expo.modules.ras_code.reviewdiff.RasCodeReviewDiffView view with tag 1150",
        ),
      ),
    ).toBe(true);
  });
