...
interface CloudEnvironmentRowsProps {
  ...
  /**
   * Hide the "RAS Connect" section title + refresh button for hosts that
...
/**
 * "RAS Connect" section: every environment published to the signed-in account,
 * with connect switches, availability status, refresh, and loading/error
 * states. Shared between the Settings environments screen and the RAS Connect
 * onboarding sheet.
 *
    ...
    <View collapsable={false} className={cn("gap-3", showHeader && "mt-5")}>
      {showHeader ? (
        <View className="flex-row items-center justify-between px-1">
          <Text className="text-sm font-ras-code-legend text-foreground-muted">RAS Connect</Text>
          {discoveryAvailable ? (
            <Pressable
      ...
      !controller.relayDiscovery.isRefreshing ? (
        <View collapsable={false} className="gap-3 rounded-[24px] bg-card p-5">
          <Text className="text-base font-ras-code-bold text-foreground">
            Could not load RAS Connect environments
          </Text>
          ...
          >
            <Text className="text-xs font-ras-code-bold text-foreground">Try again</Text>
          </Pressable>
        </View>
      ) : null}
    </View>
...
function CopyTraceIdButton(props: { readonly traceId: string }) {
  ...
  return (
    ...
    >
      ...
      <Text className="text-xs font-ras-code-bold text-foreground">Copy trace ID</Text>
    </Pressable>
  );
}
