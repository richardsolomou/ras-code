    ...
    >
      ...
      >
        ...
        <View className="absolute inset-0 items-center justify-center">
          {props.loading ? (
            <ActivityIndicator size="small" color={iconMutedColor} />
          ) : (
            <SymbolView
              name={props.icon}
              size={variant === "primary" ? 16 : 20}
              weight={variant === "primary" ? "semibold" : "regular"}
              tintColor={variant === "primary" ? primaryForegroundColor : iconColor}
              type="monochrome"
            />
          )}
        </View>
    ...
    <View className="relative h-11 min-w-0 flex-1 justify-center">
      {isError ? (
        <View className="min-w-0 flex-row items-center gap-1.5 px-2">
          ...
          >
            <SymbolView name="xmark" size={12} tintColor={iconMutedColor} type="monochrome" />
          </Pressable>
        </View>
      ) : (
        <View
