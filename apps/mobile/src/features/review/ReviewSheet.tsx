...
const ReviewNotice = memo(function ReviewNotice(props: { readonly notice: string }) {
  return (
    <View className="border-b border-amber-200 bg-amber-50 px-4 py-3 dark:border-amber-900/60 dark:bg-amber-950/40">
      <Text className="text-xs font-ras-code-bold uppercase text-amber-700 dark:text-amber-300">
        Partial diff
      </Text>
  ...
  const content = (
    <>
      <SymbolView
        ...
        type="monochrome"
      />
      <Text className="text-base font-ras-code-bold text-primary-foreground">{props.title}</Text>
    </>
  );
  ...
  return (
    ...
    >
      <Text
        className={
          selected
            ? "text-xs font-ras-code-bold text-foreground"
            : "text-xs font-ras-code-medium text-foreground-secondary"
        }
        numberOfLines={2}
      >
        {file.path}
      ...
      <View className="mt-1 flex-row gap-2">
        <Text className="text-2xs font-ras-code-bold text-emerald-600">+{file.additions}</Text>
        <Text className="text-2xs font-ras-code-bold text-rose-600">-{file.deletions}</Text>
      </View>
    </Pressable>
  );
});
    ...
    <View className="flex-1 border-l border-border bg-sheet">
      <View className="border-b border-border" style={{ paddingTop: headerInset }}>
        <View className="px-4 py-3">
          <Text className="text-sm font-ras-code-bold text-foreground">Changed files</Text>
    ...
    if (error) {
      children.push(
        <View key="review-error" className="border-b border-border bg-card px-4 py-3">
          <Text className="text-sm font-ras-code-bold text-foreground">Review unavailable</Text>
          ...
          >
            ...
            {!selectedSection ? (
              <View className="border-b border-border bg-card px-4 py-5">
                <Text className="text-sm font-ras-code-bold text-foreground">No review diffs</Text>
              ...
              <View className="border-b border-border bg-card px-4 py-5">
                <Text className="text-sm font-ras-code-bold text-foreground">No changes</Text>
                <Text className="text-xs leading-normal text-foreground-muted">
