...
import { DPOP_UNKNOWN_HINT, ManagedRelay } from "@ras-code/client-runtime/relay";
  ...
  it.effect("reuses the status-and-connect token when connecting from the cloud list", () =>
    Effect.gen(function* () {
      const fetchMock = vi.fn((url: string | URL, _init?: RequestInit) => {
        ...
        if (String(url).endsWith("/.well-known/ras-code/environment")) {
          return Promise.resolve(
            Response.json({
              environmentId: "env-1",
              label: "Desktop",
      ...
      expect(environmentTokenBody.get("client_label")).toBe("RAS Code Mobile");
      ...
      Effect.gen(function* () {
        vi.stubGlobal(
          ...
          vi.fn((url: string | URL) => {
            ...
            if (value.endsWith("/.well-known/ras-code/environment")) {
              return Promise.resolve(
                Response.json({
                  environmentId: "env-1",
                  label: "Desktop",
        ...
        vi.stubGlobal(
          ...
          vi.fn((url: string | URL) =>
            Promise.resolve(
              String(url).endsWith("/v1/client/dpop-token")
                ? Response.json(validDpopAccessTokenResponse("environment:connect"))
                : String(url).endsWith("/.well-known/ras-code/environment")
                  ? Response.json({
                      environmentId: "env-other",
                      label: "Other Desktop",
                      platform: { os: "darwin", arch: "arm64" },
