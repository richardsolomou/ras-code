      ...
      >
        <View className="gap-3 rounded-[22px] border border-border bg-card px-4 py-4">
          <View className="flex-row items-center justify-between gap-3">
            ...
            <Text className="text-foreground text-base font-ras-code-bold">
              {gitStatus.data?.refName ?? "(detached HEAD)"}
            </Text>
          </View>
          {isDefaultRef ? (
            <Text className="text-xs leading-normal text-[#8a6a12] dark:text-[#f0c24b]">
        ...
        <View className="gap-3 rounded-[22px] border border-border bg-card px-4 py-4">
          <View className="flex-row items-center justify-between gap-3">
            <View className="gap-1">
              <Text className="text-foreground text-base font-ras-code-bold">Files</Text>
            ...
            <View className="flex-row items-center gap-2">
              {!allSelected && isEditingFiles ? (
                ...
                >
                  <Text className="text-foreground text-2xs font-ras-code-legend">Reset</Text>
                </Pressable>
              ) : null}
              <Pressable
                className="bg-subtle rounded-full px-3 py-2"
              ...
              >
                <Text className="text-foreground text-2xs font-ras-code-legend">
            ...
            <View className="gap-2">
              {selectedFilePreview.map((file) => (
                <View key={file.path} className="flex-row items-center justify-between gap-3">
                  ...
                  <Text className="text-xs font-ras-code-bold text-[#2f8f4a] dark:text-[#52c46f]">
                    +{file.insertions}
                  </Text>
                  <Text className="text-xs font-ras-code-bold text-[#b33a2f] dark:text-[#e5645a]">
                    -{file.deletions}
                  </Text>
                </View>
              ))}
                  ...
                  >
                    ...
                    <View className="flex-row items-start justify-between gap-3">
                      <View className="flex-1 gap-1">
                        <Text
                          selectable
                          className={`text-sm font-ras-code-bold ${included ? "text-foreground" : "text-foreground-muted"}`}
                        >
                          {file.path}
                        </Text>
                        {!included ? (
        ...
        <View className="gap-2">
          <Text className="text-foreground text-sm font-ras-code-bold">Commit message</Text>
          <TextInput
            multiline
