...
export function ComposerSurface(props: {
  ...
  // Drop shadow lives on an outer wrapper: the clip that keeps the expanded
  // toolbar inside the glass sets `masksToBounds`, which suppresses an iOS
  // shadow drawn on the same view.
  ...
  return (
    <Animated.View layout={layoutTransition} style={shadowStyle}>
      <Animated.View
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Text
          className="max-w-[260px] text-sm font-ras-code-bold leading-snug text-foreground"
          numberOfLines={1}
        >
          {props.status.label}
        </Text>
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      armAgentAwarenessLiveActivityForLocalWork({
        ...
        projectTitle: props.environmentLabel ?? "RAS Code",
      });
    } finally {
