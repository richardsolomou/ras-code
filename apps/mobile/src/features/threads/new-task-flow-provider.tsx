...
export function NewTaskFlowProvider(props: React.PropsWithChildren) {
  ...
  // checked-in ras.json, then the server's configured default.
  const projectFileQuery = useEnvironmentQuery(
    selectedProject !== null && selectedProject.workspaceRoot !== ""
      ? projectEnvironment.readFile({
          environmentId: selectedProject.environmentId,
          input: { cwd: selectedProject.workspaceRoot, relativePath: RAS_PROJECT_FILE_NAME },
        })
      : null,
  );
  ...
  const projectFileDefaultMode = useMemo(() => {
    ...
    return parseRasProjectFile(projectFileData.contents)?.defaultThreadEnvMode ?? null;
  }, [projectFileData]);
  ...
  // the frozen interim value beats the ras.json default once it loads.
  ...
  const modelOptions = useMemo(
    () =>
      buildModelOptions(
        selectedEnvironmentServerConfig,
        draftModelSelection ?? effectiveDefaultModelSelection ?? stickyModelSelection,
      ),
    [
      selectedEnvironmentServerConfig,
      draftModelSelection,
  ...
  const selectedModel = resolveNewTaskModelSelection({
    ...
    projectDefaultSelection: effectiveDefaultModelSelection,
