...
import type { EnvironmentMachineKind } from "@ras-code/contracts";
import { canSnooze, resolveSnoozePresets } from "@ras-code/client-runtime/state/thread-settled";
...
// Console state hues, matching sidebar v1 and the web status labels: amber
// waits on the user, green is in flight, red failed.
const STATUS_LABEL_BY_STATUS: Partial<
  Record<ThreadListV2Status, { label: string; className: string }>
> = {
  approval: { label: "Approval", className: "text-[#8a6a12] dark:text-[#f0c24b]" },
  input: { label: "Input", className: "text-[#8a6a12] dark:text-[#f0c24b]" },
  working: { label: "Working", className: "text-[#2f8f4a] dark:text-[#52c46f]" },
  failed: { label: "Failed", className: "text-[#b33a2f] dark:text-[#e5645a]" },
};
...
export const ThreadListV2SectionDivider = memo(function ThreadListV2SectionDivider(props: {
  ...
  return (
    ...
    >
      <Text className="text-xs font-ras-code-medium text-foreground-tertiary">{props.label}</Text>
...
export const ThreadListV2SnoozedShelfHeader = memo(function ThreadListV2SnoozedShelfHeader(props: {
  ...
  return (
    ...
    >
      <Text className="text-xs font-ras-code-medium text-blue-600 dark:text-blue-400">
...
export const ThreadListV2SettledShelfHeader = memo(function ThreadListV2SettledShelfHeader(props: {
  ...
  return (
    <Pressable
      ...
      style={({ pressed }) => ({ opacity: pressed ? 0.6 : 1 })}
    >
      <Text className="text-xs font-ras-code-medium text-foreground-tertiary">
...
export const ThreadListV2PendingRow = memo(function ThreadListV2PendingRow(props: {
  ...
  const rowContent = (
    <>
      <View className="flex-row items-center gap-1.5">
        {props.project ? (
          <ProjectFavicon
            ...
            iconEmoji={props.project.iconEmoji}
            size={15}
        ...
        <Text
          className="flex-1 text-sm font-ras-code-medium text-foreground-muted"
          numberOfLines={1}
        >
          {projectTitle}
        </Text>
      ...
      <Text className="mt-1 text-base font-ras-code-medium text-foreground" numberOfLines={1}>
        {pendingTask.title}
      </Text>
  ...
  const cardContent = (
    <>
      <View className="flex-row items-center gap-1.5">
        ...
        <Text
          className={cn(
            "flex-1 text-sm font-ras-code-medium",
      ...
      <Text
        className={cn(
          "mt-1 text-base font-ras-code-medium",
