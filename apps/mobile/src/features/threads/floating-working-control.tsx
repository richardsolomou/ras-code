  ...
  return (
    ...
    >
      <SymbolView
        ...
        type="monochrome"
      />
      <Text className="font-ras-code-medium text-xs text-foreground">Compacting…</Text>
    </View>
  );
}
  ...
  if (props.status.kind === "syncing") {
    return (
      ...
      >
        ...
        <Text className="font-ras-code-medium text-xs text-foreground">{props.status.label}</Text>
      </View>
    );
  }
...
function WorkingDuration(props: { readonly startedAt: string }) {
  ...
  return (
    <View accessible accessibilityLabel={label} className="h-11 flex-row items-center px-4">
      <Text className="font-ras-code-medium text-xs text-foreground">Working for </Text>
      <SystemText
