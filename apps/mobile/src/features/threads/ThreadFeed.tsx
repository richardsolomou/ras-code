        ...
        <View className="h-12 w-10 shrink-0 items-center justify-center rounded-lg bg-subtle">
          {opening ? (
            ...
            <SymbolView
              name="doc.text"
              size={26}
              tintColor={isPdf ? dangerColor : mutedColor}
              type="monochrome"
            />
          )}
        </View>
        <View className="min-w-0 flex-1 gap-1">
          <Text className="font-ras-code-medium text-sm text-foreground" numberOfLines={2}>
            {attachment.name}
          </Text>
...
function MarkdownInlineCode(props: {
  ...
  return (
    <NativeText
      className={presentation ? "font-ras-code-bold" : "font-mono"}
  ...
  return (
    <View className="my-2 min-w-0 flex-row items-center gap-3 rounded-2xl border border-border bg-card px-3 py-3">
      ...
      {props.onUse ? (
        ...
        >
          <Text className="font-ras-code-bold text-xs text-foreground">Use template</Text>
        </Pressable>
      ) : null}
    </View>
  );
    ...
    return (
      ...
      >
        <Text
          ...
          className="font-ras-code-medium text-sm tabular-nums text-foreground-muted"
        >
          {entry.label}
        </Text>
        <ThreadDisclosureChevron
      ...
      >
        ...
        <View className="shrink-0 flex-row items-center gap-1.5">
          <SymbolView
            ...
            type="monochrome"
          />
          <Text className="font-ras-code-medium text-xs text-foreground-muted">{label}</Text>
        </View>
        <View className="h-px flex-1 bg-adaptive-neutral-200-a80-white-a8" />
      </View>
    );
      ...
      return (
        ...
        >
          ...
          <View className="mt-1 flex-row items-center justify-end gap-1 pr-0.5">
            <Text className="font-ras-code-medium text-xs tabular-nums text-neutral-600 dark:text-neutral-400">
              {timestampLabel}
            </Text>
            {message.text.trim().length > 0 ? (
              <CopyTextButton
  ...
  return (
    ...
    >
      <View className="max-w-[320px] items-center gap-2">
        <Text className="text-center font-ras-code-bold text-lg text-foreground">
          {props.title}
        </Text>
