...
import { EnvironmentId, ProjectId, ProviderInstanceId, ThreadId } from "@ras-code/contracts";
...
describe("buildHomeThreadGroups", () => {
  it("builds one v2 scope for the same repository across environments", () => {
    ...
    const repositoryIdentity = {
      ...
      locator: {
        ...
        remoteName: "origin",
        remoteUrl: "git@github.com:richardsolomou/ras-code.git",
      },
    };
    const projects = [
      makeProject({
    ...
    expect(scopes[0]?.title).toBe("ras-code");
  ...
  it("routes stale duplicate project refs through the canonical repository group", () => {
    ...
    const local = makeProject({
      ...
      environmentId: localEnvironmentId,
      title: "ras-code",
      workspaceRoot: "/workspaces/ras-code",
      repositoryIdentity,
    });
  ...
  it("matches web repository, repository-path, and separate grouping modes", () => {
    ...
    const projects = [
      makeProject({
        ...
        title: "Web",
        workspaceRoot: "/workspaces/ras-code/apps/web",
        repositoryIdentity,
      }),
      makeProject({
        environmentId,
        ...
        title: "Mobile",
        workspaceRoot: "/workspaces/ras-code/apps/mobile",
        repositoryIdentity,
      }),
    ];
    const threads = projects.map((project) =>
  ...
  it("does not apply the recency window while searching", () => {
    ...
    const group = buildGroups([project], threads, { searchQuery: "RAS Code" })[0];
