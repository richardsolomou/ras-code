...
import { assetUrlFailureText, deriveAssetUrlState } from "./asset-url-state";
...
describe("deriveAssetUrlState", () => {
  ...
  it("reports disconnected when the query failed while the environment is down", () => {
    for (const connectionPhase of ["available", "offline", "reconnecting", "error"] as const) {
      expect(
        deriveAssetUrlState({
          connectionPhase,
          shared: { _tag: "Failure", reason: "unavailable" },
        }),
      ).toEqual({
        _tag: "Failure",
        reason: "disconnected",
  ...
  it("labels a dead environment differently from a file problem", () => {
    expect(assetUrlFailureText("disconnected")).toBe("Environment disconnected");
    expect(assetUrlFailureText("not-found")).toBe("Image not found");
  });

  it("keeps the file's own failure reason on a connected environment", () => {
    expect(
      deriveAssetUrlState({
        connectionPhase: "connected",
        shared: { _tag: "Failure", reason: "not-found" },
      }),
    ).toEqual({ _tag: "Failure", reason: "not-found" });
  });
});
