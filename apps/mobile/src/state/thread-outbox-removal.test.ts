import { CommandId, EnvironmentId, MessageId, ProjectId, ThreadId } from "@ras-code/contracts";
...
describe("thread outbox removal", () => {
  ...
  it("clears a removed pending task draft and includes its editor-only files", async () => {
    const message = queuedMessage({
      ...
      fileUri: "file:///documents/ras-code-composer-attachments/queued.pdf",
      creation: true,
    });
    const editorOnlyFile = {
      ...
      mimeType: "application/pdf",
      sizeBytes: 84,
      fileUri: "file:///documents/ras-code-composer-attachments/editor-only.pdf",
    };
  ...
  it("releases only the cleared environment's queued attachment files", async () => {
    const cleared = queuedMessage({
      ...
      fileUri: "file:///documents/ras-code-composer-attachments/cleared.pdf",
    });
  ...
  it("clears only removed pending drafts and keeps drafts for live messages", async () => {
    const cleared = queuedMessage({
      ...
      fileUri: "file:///documents/ras-code-composer-attachments/cleared-pending.pdf",
      creation: true,
    });
    const replaced = queuedMessage({
      environmentId: "environment-1",
      messageId: "message-replaced-pending",
      fileUri: "file:///documents/ras-code-composer-attachments/replaced-pending.pdf",
      creation: true,
    });
    const kept = queuedMessage({
      environmentId: "environment-2",
      messageId: "message-kept-pending",
      fileUri: "file:///documents/ras-code-composer-attachments/kept-pending.pdf",
      creation: true,
    });
    ...
    const editorOnlyFile = {
      ...
      mimeType: "application/pdf",
      sizeBytes: 84,
      fileUri: "file:///documents/ras-code-composer-attachments/cleared-editor.pdf",
    };
