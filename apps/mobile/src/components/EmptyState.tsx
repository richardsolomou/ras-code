...
export function EmptyState(props: {
  ...
  if (props.variant === "plain") {
    return (
      <View className="items-center px-8 py-8">
        <Text className="text-center text-xl font-ras-code-bold text-foreground">
          {props.title}
        </Text>
        ...
        {props.actionLabel && props.onAction ? (
          ...
          >
            <Text className="text-sm font-ras-code-bold text-primary-foreground">
              {props.actionLabel}
            </Text>
          </Pressable>
        ) : null}
  ...
  return (
    <View className="rounded-[22px] border border-border bg-card p-5">
      <Text className="font-ras-code-bold text-lg text-foreground">{props.title}</Text>
