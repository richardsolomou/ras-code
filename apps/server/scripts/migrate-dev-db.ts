 ...
 * ~/.ras-code database, then run this checkout's migrations against it.
 *
 * `vp run migrate-dev-db` from a worktree:
 *   1. Nukes `<worktree>/.ras-code/userdata/state.sqlite`.
...
export class MigrateDevDbNotInWorktreeError extends Schema.TaggedErrorClass<MigrateDevDbNotInWorktreeError>()(
  ...
  override get message(): string {
    return "Not inside a linked git worktree. Pass --base-dir to target an isolated .ras-code directory.";
  }
}
...
export class MigrateDevDbSharedHomeError extends Schema.TaggedErrorClass<MigrateDevDbSharedHomeError>()(
  ...
  override get message(): string {
    return "Refusing to rebuild the shared ~/.ras-code database. Use an isolated --base-dir.";
  }
}
...
export class MigrateDevDbServerRunningError extends Schema.TaggedErrorClass<MigrateDevDbServerRunningError>()(
  ...
  override get message(): string {
    return `Dev database at '${this.databasePath}' is open by a running server (pid ${this.pid} per server-runtime.json). Stop that server first; if that pid is not actually a RAS Code server (stale descriptor, reused pid), delete the server-runtime.json next to the database and retry.`;
  }
}
...
export interface RunMigrateDevDbInput {
  /** Isolated .ras-code directory. Defaults to `<worktree>/.ras-code` of the cwd. */
  readonly baseDir?: string | undefined;
  /** Source database. Defaults to `~/.ras-code/userdata/state.sqlite`. */
...
export const runMigrateDevDb = Effect.fn("runMigrateDevDb")(function* (
  ...
  const sharedHome = path.resolve(options.sharedHome ?? path.join(NodeOS.homedir(), ".ras-code"));
  ...
  const baseDir =
    input.baseDir !== undefined
      ...
      : yield* resolveWorktreeRasCodeHome(process.cwd());
...
export const migrateDevDbCommand = Command.make(
  "migrate-dev-db",
  {
    ...
    source: Flag.string("source").pipe(
      Flag.optional,
      Flag.withDescription("Source database. Defaults to ~/.ras-code/userdata/state.sqlite."),
    ),
  },
  ...
  Command.withDescription(
    "Rebuild the worktree dev database from a pruned snapshot of the real ~/.ras-code data, then run migrations.",
  ),
);
