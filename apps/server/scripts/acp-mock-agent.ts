...
const antigravityProfile = process.env.RAS_ACP_ANTIGRAVITY === "1";
...
const waitForResumeRelease = process.env.RAS_ACP_WAIT_FOR_RESUME_RELEASE === "1";
const completeFirstPromptOnCancel = process.env.RAS_ACP_COMPLETE_FIRST_PROMPT_ON_CANCEL === "1";
const floodStderr = process.env.RAS_ACP_FLOOD_STDERR === "1";
  ...
  yield* agent.handleUnknownExtRequest((method, params) => {
    if (method === "_test/environment") {
      return Effect.succeed({
        inherited: process.env.RAS_ACP_RUNTIME_AMBIENT === "sentinel",
        explicit: process.env.RAS_ACP_RUNTIME_EXPLICIT === "kept",
      });
    }
