...
/**
 * A pinned runtime is an exact `ras-code@<version>` npm-installed into
 ...
 * switching over, never `npx ras-code`, whose cache is ephemeral and whose
...
export function pinnedRuntimePaths(
  ...
  return {
    versionDir,
    entryPath: path.join(versionDir, "node_modules", "ras-code", "dist", "bin.mjs"),
...
/**
 * Installs `ras-code@<version>` into the pinned runtime directory unless a complete
...
const installPinnedRuntime = Effect.fn("cloud.pinned_runtime.ensure_installed")(function* (
  ...
  const stagingPaths: PinnedRuntimePaths = {
    ...
    entryPath: input.path.join(stagingDir, "node_modules", "ras-code", "dist", "bin.mjs"),
