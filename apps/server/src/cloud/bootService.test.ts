...
it("keeps systemd pinned to the stable launcher rather than a versioned server", () => {
  const unit = BootService.renderBootServiceUnit({
    ...
    launcherPath: "/home/theo/.ras-code/runtime/service-launcher.mjs",
    baseDir: "/home/theo/.ras-code",
    logPath: "/home/theo/.ras-code/userdata/logs/boot-service.log",
    unitPath: "/home/theo/.config/systemd/user/ras-code.service",
  });

  expect(unit).toContain(
    "ExecStart=/usr/bin/node /home/theo/.ras-code/runtime/service-launcher.mjs",
  );
...
const macPlan = {
  ...
  launcherPath: "/Users/theo/.ras-code/runtime/service-launcher.mjs",
  baseDir: "/Users/theo/.ras-code",
  logPath: "/Users/theo/.ras-code/userdata/logs/boot-service.log",
  unitPath: "/Users/theo/Library/LaunchAgents/com.richardsolomou.ras-code.service.plist",
};
...
it("keeps launchd pinned to the stable launcher rather than a versioned server", () => {
  ...
  expect(plist).toContain("<string>/Users/theo/.ras-code/runtime/service-launcher.mjs</string>");
...
it("appends both stdio streams to the boot service log", () => {
  ...
  expect(plist).toContain(
    "<key>StandardOutPath</key>\n  <string>/Users/theo/.ras-code/userdata/logs/boot-service.log</string>",
  );
  expect(plist).toContain(
...
it("escapes XML in host paths", () => {
  const plist = BootService.renderBootServicePlist(
    { ...macPlan, baseDir: "/Users/theo/RAS Code & <Co>" },
    { homeDir: "/Users/theo", environmentPath: "/Users/theo/Tools & <Scripts>:/usr/bin" },
  );

  expect(plist).toContain("<string>/Users/theo/RAS Code &amp; &lt;Co&gt;</string>");
...
const makeHarness = Effect.fn("test.make_boot_service_harness")(function* (
  ...
  const home = yield* fs.makeTempDirectoryScoped({ prefix: "ras-code-boot-service-test-" });
  const baseDir = path.join(home, ".ras-code");
  ...
  const runner = ProcessRunner.ProcessRunner.of({
    run: Effect.fn("test.run_boot_service_command")(function* (
      ...
      if (!failed && command === "loginctl enable-linger --no-ask-password 501")
        control.linger = "yes";
      if (!failed && command === "systemctl --user enable ras-code.service") control.enabled = true;
      if (!failed && command === "systemctl --user restart ras-code.service") control.active = true;
      if (
        control.stateAfterStop !== undefined &&
        (command === "systemctl --user stop ras-code.service" ||
...
it.layer(NodeServices.layer)("boot service install", (it) => {
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(commands).not.toContain("systemctl --user stop ras-code.service");
      }),
  );
  ...
  it.effect("installs, reports current state, and uninstalls", () =>
    Effect.gen(function* () {
      ...
      expect(timeouts.get("systemctl --user disable --now ras-code.service")).toEqual(
    ...
    (platform) =>
      Effect.gen(function* () {
        ...
        expect(
          ...
          platform === "linux"
            ? [
                "systemctl --user stop ras-code.service",
                "systemctl --user restart ras-code.service",
              ]
            : [
                "launchctl bootout --wait gui/501/com.richardsolomou.ras-code.service",
  ...
  it.effect("installs, reports current state, and uninstalls on macOS", () =>
    Effect.gen(function* () {
      ...
      expect(
        timeouts.get("launchctl bootout --wait gui/501/com.richardsolomou.ras-code.service"),
  ...
  it.effect("restarts the launch agent when repair fails", () =>
    Effect.gen(function* () {
      ...
      expect(commands.filter((command) => command.startsWith("launchctl "))).toEqual([
        "launchctl bootout --wait gui/501/com.richardsolomou.ras-code.service",
        "launchctl enable gui/501/com.richardsolomou.ras-code.service",
  ...
  it.effect("ignores a bootout for an agent that is not loaded", () =>
    Effect.gen(function* () {
      ...
      yield* service.install();
      control.failCommand = "launchctl bootout --wait gui/501/com.richardsolomou.ras-code.service";
