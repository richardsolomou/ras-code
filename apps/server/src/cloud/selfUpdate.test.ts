...
import { ServerSelfUpdateError, ThreadId } from "@ras-code/contracts";
...
const makeHarness = Effect.fn("test.make_self_update_harness")(function* (
  ...
  const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "ras-code-self-update-test-" });
  const order: string[] = [];
    ...
    run: (input) =>
      Effect.gen(function* () {
        if (input.command === "npm") {
          ...
          const entry = path.join(prefix, "node_modules", "ras-code", "dist", "bin.mjs");
...
it.layer(NodeServices.layer)("server self update", (it) => {
  ...
  it.effect("rejects invalid versions and desktop-managed servers before staging", () =>
    Effect.gen(function* () {
      ...
      ).toBe("'latest' is not an exact ras version.");
