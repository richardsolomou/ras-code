  ...
  Effect.mapError(
    () =>
      new EnvironmentHttpInternalServerError({
        message: "RAS_CODE_RELAY_URL must be configured as a secure absolute HTTPS origin.",
      }),
  ),
);
...
function requestAbsoluteUrl(request: HttpServerRequest.HttpServerRequest): string | null {
  try {
    ...
    const relayOrigin = stripManagedEndpointGatewayPrefix(request.originalUrl)
      ? parseRasRelayPublicOrigin(request.headers[RAS_RELAY_PUBLIC_ORIGIN_HEADER] ?? "")
      : null;
    const host = firstForwardedHeaderValue(request.headers.host) ?? "127.0.0.1";
    try {
      return new URL(request.originalUrl, relayOrigin ?? `http://${host}`).href;
    } catch {
      return null;
    }
  }
...
// A relay endpoint is provisioned by the relay and points back at this local
// origin. Manual endpoints are reached out of band or used for publish-only.
export function linkProofScopes(
  request: RelayLinkProofRequest,
): RelayEnvironmentLinkProofPayload["scopes"] {
  return request.endpoint.providerKind === "ras_relay"
    ? ["agent_activity_notifications", "managed_relay"]
...
const makeCloudLinkProof = Effect.fn("environment.cloud.makeLinkProof")(function* (
  ...
  if (!isAllowedEndpointOrigin({ origin: request.origin, requestUrl })) {
  ...
  const payload = {
    iss: `ras-env:${descriptor.environmentId}`,
...
const relayClientRequest = <A>(
  ...
  HttpClientRequest.post(input.url).pipe(
    ...
    Effect.mapError(
      (cause) =>
        new EnvironmentHttpInternalServerError({
          message: `RAS Connect relay request failed: ${String(cause)}`,
        }),
    ),
...
const reconcileDesiredCloudLinkWith = Effect.fn("environment.cloud.reconcileDesiredLinkWith")(
  function* (dependencies: CloudHttpDependencies, localOrigin: string) {
          ...
          onNone: () =>
            Effect.fail(
              new EnvironmentHttpUnauthorizedError({
                message: "Run `ras connect link` to authorize this environment.",
              }),
            ),
    ...
    const managedRelayEnabled = mode !== "publish_only";
    ...
    const proof = yield* makeCloudLinkProof(
      dependencies,
      {
        ...
        endpoint: {
          ...
          providerKind: managedRelayEnabled ? "ras_relay" : "manual",
        },
        origin: {
  ...
  Effect.catchIf(
    ServerSecretStore.isSecretStoreError,
    failEnvironmentCloudInternalError("Could not persist desired RAS Connect link state."),
  ),
  Effect.catchTags({
...
const cloudEnvironmentHealthHandler = Effect.fn("environment.cloud.health")(
  function* (dependencies: CloudHttpDependencies, request: RelayCloudEnvironmentHealthRequest) {
    ...
    const proofOption = yield* verifyRelayJwt({
      ...
      audience: `ras-env:${environmentId}`,
