...
// "managed" enables the built-in relay (default, legacy value "true").
// "publish_only" links the environment to the relay purely to publish agent
// activity — no connector, no relay-advertised endpoint — so activity can flow to
// mobile clients even when they reach the environment out of band (Tailscale,
// direct pairing) without RAS Connect.
