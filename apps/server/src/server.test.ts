...
import { HTTP_ROUTER_CONFIG, makeRoutesLayer, managedEndpointGatewayMiddleware } from "./server.ts";
import {
  isThreadDetailEvent,
...
const makeCloudMintCredentialRequest = (input: {
  ...
  const payload = {
    ...
    aud: input.audience ?? `ras-env:${input.environmentId}`,
...
it.layer(NodeServices.layer)("server router seam", (it) => {
  ...
  for (const manifest of [
    ...
    it.effect(`revalidates hash-like static filenames with a ${manifest.label} manifest`, () =>
      Effect.gen(function* () {
        ...
        const path = yield* Path.Path;
        const staticDir = yield* fileSystem.makeTempDirectoryScoped({
          prefix: "ras-code-static-mutable-",
        });
        yield* fileSystem.makeDirectory(path.join(staticDir, "assets"));
  ...
  it.effect("binds static metadata and bytes to one file across atomic replacement", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const staticDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "ras-code-static-replace-",
      });
  ...
  it.effect("serves the public environment descriptor without requiring auth", () =>
    Effect.gen(function* () {
      ...
      const url = yield* getHttpServerUrl("/.well-known/ras-code/environment");
  ...
  it.effect("reports unauthenticated session state without requiring auth", () =>
    Effect.gen(function* () {
      ...
      assert.isTrue(body.auth.sessionCookieName.startsWith("ras_session_"));
    }).pipe(Effect.provide(NodeHttpServer.layerTest)),
  );
  ...
  it.effect("migrates a valid legacy remote-web session cookie", () =>
    Effect.gen(function* () {
      ...
      const legacyCookie = currentCookie.replace(/^ras_session_[^=]+=/, "ras_session=");
      const sessionUrl = yield* getHttpServerUrl("/api/auth/session");
    ...
    (source) =>
      Effect.gen(function* () {
        ...
        const response = yield* fetchEffect(sessionUrl, {
          headers:
            source === "cookie"
              ? { cookie: `${sessionCookie}; ras_session=stale` }
              : { authorization: `Bearer ${sessionToken}`, cookie: "ras_session=stale" },
        });
        const body = yield* responseJsonEffect<{ readonly authenticated: boolean }>(response);
  ...
  it.effect("serves signed RAS Connect environment health checks", () =>
    Effect.gen(function* () {
      ...
      const healthUrl = yield* getHttpServerUrl("/api/connect/health");
    ...
    Effect.gen(function* () {
      ...
      const wrongAudience = yield* postMint(
        makeCloudMintCredentialRequest({
          ...
          environmentId: testEnvironmentDescriptor.environmentId,
          clientProofKeyThumbprint: "client-proof-key-thumbprint",
          audience: "ras-env:other-environment",
  ...
  for (const desktopOrigin of ["ras-code://app", "ras-code-dev://app"]) {
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        assert.equal(record.resourceAttributes["service.name"], "ras-web");
  ...
  it.effect("rejects websocket rpc handshake when session authentication is missing", () =>
    Effect.gen(function* () {
      ...
      const workspaceDir = yield* fs.makeTempDirectoryScoped({
        prefix: "ras-code-ws-auth-required-",
      });
  ...
  it.effect("routes websocket rpc projects.searchEntries", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const workspaceDir = yield* fs.makeTempDirectoryScoped({
        prefix: "ras-code-ws-project-search-",
      });
      yield* fs.writeFileString(
        path.join(workspaceDir, "needle-file.ts"),
        "export const needle = 1;",
  ...
  it.effect("routes websocket rpc projects.listEntries and projects.readFile", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const workspaceDir = yield* fs.makeTempDirectoryScoped({
        prefix: "ras-code-ws-project-files-",
      });
  ...
  it.effect("routes websocket rpc projects.searchEntries excludes gitignored files", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const workspaceDir = yield* fs.makeTempDirectoryScoped({
        prefix: "ras-code-ws-project-search-gitignored-",
      });
  ...
  it.effect("preserves structured workspace rpc failures", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const workspaceDir = yield* fs.makeTempDirectoryScoped({
        prefix: "ras-code-ws-workspace-errors-",
      });
      const outsideDir = yield* fs.makeTempDirectoryScoped({
        prefix: "ras-code-ws-workspace-errors-outside-",
      });
  ...
  it.effect("reports workspace root stat failures without relabeling them as missing", () =>
    Effect.gen(function* () {
      ...
      const blockedRoot = yield* fs.makeTempDirectoryScoped({
        prefix: "ras-code-ws-workspace-stat-error-",
      });
  ...
  it.effect("routes websocket rpc projects.writeFile", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const workspaceDir = yield* fs.makeTempDirectoryScoped({
        prefix: "ras-code-ws-project-write-",
      });
  ...
  it.effect("creates a missing workspace root during websocket project.create dispatch", () =>
    Effect.gen(function* () {
      ...
      const parentDir = yield* fs.makeTempDirectoryScoped({
        prefix: "ras-code-ws-project-create-",
      });
      ...
      Effect.gen(function* () {
        ...
        const refreshStatus = vi.fn((_: string) =>
          Effect.succeed({
            ...
            hasPrimaryRemote: true,
            isDefaultRef: false,
            refName: "ras-code/bootstrap-refName",
            hasWorkingTreeChanges: false,
            workingTree: {
              files: [],
              insertions: 0,
            ...
            client[ORCHESTRATION_WS_METHODS.dispatchCommand]({
              ...
              bootstrap: {
                ...
                prepareWorktree: {
                  ...
                  baseBranch: "main",
                  branch: "ras-code/bootstrap-refName",
        ...
        assert.deepEqual(createWorktree.mock.calls[0]?.[0], {
          ...
          newRefName: "ras-code/bootstrap-refName",
          baseRefName: "main",
          path: null,
        });
...
it.live(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const reportPath = yield* Config.string("RAS_CODE_TRANSFER_BUDGET_REPORT_PATH").pipe(
        Config.option,
      );
      ...
      const resultPath = yield* Config.string("RAS_CODE_TRANSFER_BUDGET_RESULT_PATH").pipe(
        Config.option,
      );
