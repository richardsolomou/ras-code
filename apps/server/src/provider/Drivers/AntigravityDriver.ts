    ...
    Effect.gen(function* () {
      ...
      const makeDisposableRuntime = Effect.fn("AntigravityDriver.makeDisposableRuntime")(function* (
        ...
        const cwd = yield* fileSystem
          .makeTempDirectoryScoped({ prefix: "ras-code-antigravity-setup-" })
          .pipe(
            Effect.mapError(
              () =>
                new ProviderSetupError({
        ...
        const runtime = yield* makeRuntime({
          cwd,
          clientInfo: { name: "ras-code-provider-setup", version: "0.0.0" },
          mcpServers: [],
      ...
      const textGeneration = yield* makeAntigravityTextGeneration({
        ...
        makeRuntime: (cwd) =>
          makeRuntime({
            cwd,
            clientInfo: { name: "ras-code-text", version: "0.0.0" },
            mcpServers: [],
          }),
      });
