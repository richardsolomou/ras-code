...
const makeHarness = Effect.fn("makeAntigravityDriverHarness")(function* (
  ...
  const root = yield* fs.makeTempDirectoryScoped({ prefix: "ras-code-antigravity-driver-" });
  ...
  const instance = yield* AntigravityDriver.create({
    ...
    environment: [
      ...
      { name: "RAS_ACP_ANTIGRAVITY", value: "1" },
      { name: "RAS_ACP_REQUEST_LOG_PATH", value: requestLog },
...
const testLayer = ServerConfig.layerTest(process.cwd(), {
  prefix: "ras-code-antigravity-driver-config-",
}).pipe(
