...
>()("ras-code/provider/providerMaintenanceRunner") {}
            ...
            return yield* finish(
              makeUpdateState({
                ...
                message: couldNotVerify
                  ? "Update command completed, but RAS Code could not verify the provider version."
                  : stillOutdated
                    ? "Update command completed, but RAS Code still detects an outdated provider version."
                    : "Provider updated.",
                output: commandOutput(result),
              }),
            );
