...
it.layer(NodeServices.layer)("providerMaintenance", (it) => {
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        const tempDir = yield* makeTempDir("ras-code-vite-plus-capabilities");
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        const tempDir = yield* makeTempDir("ras-code-bun-capabilities");
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        const tempDir = yield* makeTempDir("ras-code-pnpm-capabilities");
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        const tempDir = yield* makeTempDir("ras-code-native-package-tool-native-capabilities");
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        const tempDir = yield* makeTempDir("ras-code-scoped-package-tool-native-capabilities");
  ...
  it.effect("keeps npm updates for binaries symlinked into npm's global node_modules tree", () =>
    Effect.gen(function* () {
      const tempDir = yield* makeTempDir("ras-code-npm-capabilities");
  ...
  it.effect("uses Effect FileSystem realPath when detecting pnpm global symlinks", () =>
    Effect.gen(function* () {
      const tempDir = yield* makeTempDir("ras-code-pnpm-realpath-capabilities");
      const binDir = NodePath.join(tempDir, "bin");
      const packageBinDir = NodePath.join(
        tempDir,
        ".local",
