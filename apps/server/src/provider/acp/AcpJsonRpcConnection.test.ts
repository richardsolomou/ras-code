    ...
    Effect.gen(function* () {
      ...
      const runtime = yield* AcpSessionRuntime.make({
        ...
        spawn: {
          ...
          env: { RAS_ACP_WAIT_FOR_RESUME_RELEASE: "1" },
        },
  ...
  it.effect("waits for native cancellation and drains final updates before another prompt", () =>
    Effect.gen(function* () {
      ...
      const runtime = yield* AcpSessionRuntime.make({
        ...
        spawn: {
          ...mockRuntimeOptions.spawn,
          env: { RAS_ACP_COMPLETE_FIRST_PROMPT_ON_CANCEL: "1" },
        },
    ...
    Effect.gen(function* () {
      ...
      for (const logStderr of [false, true]) {
        yield* Effect.gen(function* () {
          const runtime = yield* AcpSessionRuntime.make({
            ...mockRuntimeOptions,
            spawn: { ...mockRuntimeOptions.spawn, env: { RAS_ACP_FLOOD_STDERR: "1" } },
            ...(logStderr
              ? {
  ...
  it.effect("does not restore ambient variables to a sanitized child environment", () =>
    Effect.gen(function* () {
      yield* Effect.acquireRelease(
        Effect.sync(() => {
          const previous = process.env.RAS_ACP_RUNTIME_AMBIENT;
          process.env.RAS_ACP_RUNTIME_AMBIENT = "sentinel";
          return previous;
        }),
        (previous) =>
          Effect.sync(() => {
            if (previous === undefined) delete process.env.RAS_ACP_RUNTIME_AMBIENT;
            else process.env.RAS_ACP_RUNTIME_AMBIENT = previous;
          }),
      );
      const runtime = yield* AcpSessionRuntime.make({
        ...mockRuntimeOptions,
        spawn: {
          ...
          args: mockAgentArgs,
          extendEnv: false,
          env: { RAS_ACP_RUNTIME_EXPLICIT: "kept" },
        },
      });
      ...
      Effect.provide(
        AcpSessionRuntime.layer({
          spawn: {
            ...
            args: mockAgentArgs,
            env: {
              RAS_ACP_EMIT_INTERLEAVED_ASSISTANT_TOOL_CALLS: "1",
            },
          },
          cwd: process.cwd(),
          clientInfo: { name: "ras-code-test", version: "0.0.0" },
      ...
      Effect.provide(
        AcpSessionRuntime.layer({
          spawn: {
            ...
            args: mockAgentArgs,
            env: {
              RAS_ACP_EMIT_GENERIC_TOOL_PLACEHOLDERS: "1",
            },
          },
          cwd: process.cwd(),
          clientInfo: { name: "ras-code-test", version: "0.0.0" },
      ...
      Effect.provide(
        AcpSessionRuntime.layer({
          ...
          spawn: {
            ...
            args: mockAgentArgs,
            env: {
              RAS_ACP_HANG_LOAD_SESSION_AFTER_REPLAY: "1",
              RAS_ACP_LOAD_SESSION_DELAY_MS: "10000",
            },
          },
          cwd: process.cwd(),
          resumeSessionId: "mock-session-1",
