...
/**
 * Client captured the plan for RAS Code's proposed-plan card. Abandon the native
 * Grok plan-approval gate so the turn unblocks; the user implements via RAS Code UI.
 */
 ...
 * Used so RAS Code can show the plan while plan mode is still active (before exit).
 */
...
export const makeXAiPromptCompletionRuntime = Effect.fn("makeXAiPromptCompletionRuntime")(
  function* (runtime: AcpSessionRuntime.AcpSessionRuntime["Service"]) {
    ...
    const allocatePromptFallbackId = Effect.sync(() => {
      ...
      return `ras-code-xai-prompt-${nextPromptFallbackId}`;
    });
