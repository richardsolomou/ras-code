...
// RAS_CODEX_COLLAB_SCRIPT env var (a JSON file path) when the first turn
...
const script = JSON.parse(NodeFS.readFileSync(process.env.RAS_CODEX_COLLAB_SCRIPT, "utf8"));
...
rl.on("line", (line) => {
  ...
  if (method === undefined && script.serverRequests?.some((request) => request.id === id)) {
    NodeFS.appendFileSync(
      `${process.env.RAS_CODEX_COLLAB_SCRIPT}.responses`,
  ...
  if (method === "initialize") {
    write({
      id,
      result: {
        userAgent: "ras-code-collab-mock/0.0.0",
        codexHome: "/tmp",
        platformFamily: "unix",
        platformOs: "linux",
      },
  ...
  if (method === "turn/interrupt") {
    ...
    NodeFS.appendFileSync(
      `${process.env.RAS_CODEX_COLLAB_SCRIPT}.interrupts`,
