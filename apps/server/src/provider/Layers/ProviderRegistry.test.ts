...
import { deepMerge } from "@ras-code/shared/Struct";
import { createModelCapabilities } from "@ras-code/shared/model";
import { applyServerSettingsPatch } from "@ras-code/shared/serverSettings";
...
process.env.RAS_CODE_CURSOR_ENABLED = "1";
  ...
  (it) => {
    ...
    describe("ProviderRegistryLive", () => {
            ...
            ProviderRegistryLive.pipe(
              ...
              Layer.provideMerge(
                ServerConfig.layerTest(process.cwd(), {
                  prefix: "ras-code-provider-registry-background-refresh-",
                }),
              ),
        ...
        Effect.gen(function* () {
          ...
          const runtimeServices = yield* Layer.build(
            ProviderRegistryLive.pipe(
              ...
              Layer.provideMerge(
                ServerConfig.layerTest(process.cwd(), {
                  prefix: "ras-code-provider-registry-merged-persist-",
                }),
              ),
          ...
          Effect.gen(function* () {
            ...
            const runtimeServices = yield* Layer.build(
              ProviderRegistryLive.pipe(
                ...
                Layer.provideMerge(
                  ServerConfig.layerTest(process.cwd(), {
                    prefix: "ras-code-provider-registry-opencode-authoritative-persist-",
                  }),
                ),
                Layer.provideMerge(NodeServices.layer),
              ),
        ...
        Effect.gen(function* () {
          ...
          const runtimeServices = yield* Layer.build(
            ProviderRegistryLive.pipe(
              ...
              Layer.provideMerge(
                ServerConfig.layerTest(process.cwd(), {
                  prefix: "ras-code-provider-registry-refresh-failure-",
                }),
              ),
              Layer.provideMerge(BackgroundPolicyAlwaysRunLayer),
              Layer.provideMerge(NodeServices.layer),
        ...
        Effect.gen(function* () {
          ...
          const runtimeServices = yield* Layer.build(
            ProviderRegistryLive.pipe(
              ...
              Layer.provideMerge(
                ServerConfig.layerTest(process.cwd(), {
                  prefix: "ras-code-provider-registry-sync-failure-",
                }),
              ),
              Layer.provideMerge(BackgroundPolicyAlwaysRunLayer),
              Layer.provideMerge(NodeServices.layer),
      ...
      it.effect("propagates real Codex probe failures to the aggregator at boot", () =>
        Effect.gen(function* () {
          const missingBinary = `ras-code_codex_missing_`;
          const serverSettings = yield* makeMutableServerSettingsService(
            decodeServerSettings(
              deepMerge(encodedDefaultServerSettings, {
                ...
                providerInstances: {
                  // Matches the shape the user had in `.ras-code/dev/settings.json`
          ...
          const providerRegistryLayer = ProviderRegistryLive.pipe(
            ...
            Layer.provideMerge(
              ServerConfig.layerTest(process.cwd(), {
                prefix: "ras-code-provider-registry-",
              }),
            ),
      ...
      it.effect("re-probes when settings change the codex binaryPath", () =>
        Effect.gen(function* () {
          const firstMissing = `ras-code_codex_first_`;
          const secondMissing = `ras-code_codex_second_`;
          ...
          Effect.gen(function* () {
            ...
            yield* Effect.gen(function* () {
              ...
              assert.strictEqual(
                ...
                "Cursor is disabled in RAS Code settings.",
              );
      ...
      it.effect("skips codex probes entirely when the provider is disabled", () =>
        Effect.gen(function* () {
          ...
          assert.strictEqual(status.installed, false);
          assert.strictEqual(status.message, "Codex is disabled in RAS Code settings.");
        }),
      );
    });
    ...
    describe("checkClaudeProviderStatus", () => {
      ...
      const slugsOfferingOption = (models: ReadonlyArray<ServerProviderModel>, optionId: string) =>
        models
          .filter((model) =>
            model.capabilities?.optionDescriptors?.some((descriptor) => descriptor.id === optionId),
          )
          .map((model) => model.slug);

      it.effect("offers the 1M context window only on Claude models that accept it", () =>
        Effect.gen(function* () {
          const status = yield* checkClaudeProviderStatus(
            defaultClaudeSettings,
            claudeCapabilities(),
          );
          assert.deepStrictEqual(slugsOfferingOption(status.models, "contextWindow"), [
            "claude-fable-5-1",
            "claude-fable-5",
            "claude-opus-5",
            "claude-opus-4-8",
      ...
      it.effect("offers fast mode only on Claude models that support it", () =>
        Effect.gen(function* () {
          const status = yield* checkClaudeProviderStatus(
            defaultClaudeSettings,
            claudeCapabilities(),
          );
          assert.deepStrictEqual(slugsOfferingOption(status.models, "fastMode"), [
            "claude-opus-5",
            "claude-opus-4-8",
          ]);
        }).pipe(Effect.provide(claudeAtVersion("2.1.257"))),
      ...
      it.effect("caps Claude Opus 4.5 reasoning at high", () =>
        Effect.gen(function* () {
          const status = yield* checkClaudeProviderStatus(
            defaultClaudeSettings,
            claudeCapabilities(),
          ...
          const effort = status.models
            .find((model) => model.slug === "claude-opus-4-5")
      ...
      it("clamps xhigh only for Claude models whose catalog omits it", () => {
        assert.deepStrictEqual(
          [
            "claude-fable-5-1",
            "claude-fable-5",
            ...
            "claude-sonnet-4-6",
            "claude-haiku-4-5",
          ].map((model) => [model, normalizeClaudeCliEffort("xhigh", model)]),
          [
      ...
      it.effect("runs Claude status probes with the configured CLAUDE_CONFIG_DIR", () => {
        const claudeConfigDir = "/tmp/ras-code-claude-home";
