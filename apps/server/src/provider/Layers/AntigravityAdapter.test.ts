...
const layer = ServerConfig.layerTest(process.cwd(), {
  prefix: "ras-code-antigravity-adapter-test-",
...
it.layer(layer)("AntigravityAdapter", (it) => {
    ...
    () =>
      Effect.gen(function* () {
        ...
        const cwd = yield* fileSystem.makeTempDirectoryScoped({
          prefix: "ras-code-antigravity-transport-",
        });
          ...
          makeRuntime: (input) =>
            makeAntigravityAcpRuntime({
              ...
              spawn: {
                ...
                env: {
                  ...process.env,
                  RAS_ACP_ANTIGRAVITY: "1",
                  RAS_ACP_REQUEST_LOG_PATH: requestLog,
                },
                extendEnv: false,
              },
  ...
  it.effect("serves client file reads and writes only inside the session roots", () =>
    Effect.gen(function* () {
      ...
      const cwd = yield* fs.makeTempDirectoryScoped({ prefix: "ras-code-agy-fs-" });
      const outside = yield* fs.makeTempDirectoryScoped({ prefix: "ras-code-agy-outside-" });
