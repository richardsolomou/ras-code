...
export function makeOpenCodeAdapter(
  ...
  return Effect.gen(function* () {
    ...
    // RAS Code supplies the message ID to match prompt admission events. Keep OpenCode's sortable native shape so equal-time messages retain their upstream order.
    ...
    const failPromptAdmissionRecovery = Effect.fn("failPromptAdmissionRecovery")(function* (
      ...
      const detail =
        "OpenCode accepted the prompt, but RAS Code could not confirm its message or session status.";
