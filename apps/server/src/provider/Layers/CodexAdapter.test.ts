...
class CodexAdapter extends Context.Service<CodexAdapter, CodexAdapterShape>()(
  "ras-code/provider/Layers/CodexAdapter.test/CodexAdapter",
) {}
...
sessionErrorLayer("CodexAdapterLive session errors", (it) => {
  ...
  it.effect("uses RAS_CODE_CODEX_LAUNCH_ARGS for the session runtime", () => {
    const runtimeFactory = makeRuntimeFactory();
    const layer = Layer.effect(
      CodexAdapter,
      Effect.gen(function* () {
        ...
        return yield* makeCodexAdapter(codexConfig, {
          environment: { RAS_CODE_CODEX_LAUNCH_ARGS: " --strict-config --enable env-feature " },
          makeRuntime: runtimeFactory.factory,
        });
      }),
    ).pipe(
...
lifecycleLayer("CodexAdapterLive lifecycle", (it) => {
  ...
  it.effect("stamps the session model onto turn.started", () =>
    Effect.gen(function* () {
      const adapter = yield* CodexAdapter;
      yield* adapter.startSession({
        provider: ProviderDriverKind.make("codex"),
        threadId: asThreadId("thread-model"),
        runtimeMode: "full-access",
        modelSelection: createModelSelection(ProviderInstanceId.make("codex"), "gpt-5.3-codex", []),
      });
      const runtime = lifecycleRuntimeFactory.lastRuntime;
      ...
      const events = Array.from(yield* Fiber.join(eventsFiber));
      NodeAssert.equal(events[0]?.type, "turn.started");
      const startedPayload = events[0]?.payload as Record<string, unknown>;
      NodeAssert.equal(startedPayload.model, "gpt-5.3-codex");
    }),
  );
  ...
  it.effect("labels MCP lifecycle entries with server and tool names", () =>
    Effect.gen(function* () {
      ...
      NodeAssert.equal(firstEvent.value.payload.title, "ras-code · preview_status");
...
it.effect("flushes managed native logs when the adapter layer shuts down", () =>
  Effect.gen(function* () {
    const tempDir = NodeFS.mkdtempSync(
      NodePath.join(NodeOS.tmpdir(), "ras-codex-adapter-native-log-"),
    );
