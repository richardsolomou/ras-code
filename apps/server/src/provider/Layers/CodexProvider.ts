...
export function appendCustomCodexModels(
  ...
  for (const entry of readCustomModelEntries(customModels)) {
    ...
    customEntries.push({
      ...
      isCustom: true,
      capabilities:
        entry.capabilities ?? (inheritsCodexModelOptions(entry.slug) ? fallbackCapabilities : null),
    });
  }
...
export function buildCodexInitializeParams(): CodexSchema.V1InitializeParams {
  return {
    clientInfo: {
      name: "ras-code_desktop",
      title: "RAS Code Desktop",
    ...
    if (!codexSettings.enabled) {
      return buildServerProvider({
        ...
        probe: {
          ...
          status: "warning",
          auth: { status: "unknown" },
          message: "Codex is disabled in RAS Code settings.",
        },
      });
    }
