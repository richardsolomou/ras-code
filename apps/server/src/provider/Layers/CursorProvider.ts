...
const CURSOR_ACP_MODEL_DISCOVERY_FAILED_MESSAGE = [
  ...
  "Cursor CLI setup may be incomplete; install or enable the Cursor CLI, restart RAS Code, and try again.",
    ...
    if (!cursorSettings.enabled) {
      return buildServerProvider({
        ...
        probe: {
          ...
          status: "warning",
          auth: { status: "unknown" },
          message: "Cursor is disabled in RAS Code settings.",
        },
      });
    }
  ...
  Effect.gen(function* () {
    ...
    const acpContext = yield* Layer.build(
      AcpSessionRuntime.layer({
        ...
        cwd: process.cwd(),
        clientInfo: { name: "ras-code-provider-probe", version: "0.0.0" },
...
function buildCursorCliCommandMissingMessage(binaryPath: string): string {
  return [
    ...
    `Install or enable the Cursor CLI, make sure \`${binaryPath}\` is on PATH, then restart RAS Code.`,
    `See ${CURSOR_CLI_INSTALLATION_DOCS_URL}.`,
  ].join(" ");
}
