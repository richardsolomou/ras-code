        ...
        probe: {
          ...
          message:
            openCodeSettings.serverUrl.trim().length > 0
              ? "OpenCode is disabled in RAS Code settings. A server URL is configured."
              : "OpenCode is disabled in RAS Code settings.",
        },
      });
    }
...
> {
  ...
  if (!isExternalServer) {
    ...
    if (!version) {
      return fallback(
        new Error(
          `Unable to determine OpenCode version from \`opencode --version\` output. RAS Code requires OpenCode v${MINIMUM_OPENCODE_VERSION} or newer.`,
        ),
        null,
      );
    }
