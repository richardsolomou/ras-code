...
export const ANTIGRAVITY_AUTH_BROWSER_MARKER = "__RAS_CODE_ANTIGRAVITY_AUTH_URL__";
...
const browserHelperSource =
  ...
  `()=>process.exit(0))`;
const browserPreflightUrl = "https://example.invalid/ras-code-antigravity-browser-preflight";
...
/** Recognizes native auth failures and interactive login blocked by RAS Code. */
...
export const prepareAntigravityProfile = Effect.fn("prepareAntigravityProfile")(function* (input: {
  ...
  if (
    ...
    return yield* authSupportError(
      "The RAS Code runtime path cannot be used to suppress Antigravity browser launches.",
    );
  }
...
/** Receives native 1.1.1 sign-in URLs and RAS Code browser-helper URLs without logging stderr. */
