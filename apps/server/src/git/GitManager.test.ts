...
> {
  return Effect.gen(function* () {
    const remoteDir = yield* makeTempDir("ras-code-git-remote-");
...
function makeManager(input?: {
  ...
  const serverConfigLayer = ServerConfig.layerTest(process.cwd(), {
    prefix: "ras-code-git-manager-test-",
  });
...
const GitManagerTestLayer = GitVcsDriver.layer.pipe(
  Layer.provide(ServerConfig.layerTest(process.cwd(), { prefix: "ras-code-git-manager-test-" })),
  Layer.provideMerge(VcsProcess.layer),
  Layer.provideMerge(NodeServices.layer),
);
...
it.layer(GitManagerTestLayer)("GitManager", (it) => {
  it.effect("status includes PR metadata when branch already has an open PR", () =>
    Effect.gen(function* () {
      const repoDir = yield* makeTempDir("ras-code-git-manager-");
  ...
  it.effect("status returns an explicit non-repo result for non-git directories", () =>
    Effect.gen(function* () {
      const cwd = yield* makeTempDir("ras-code-git-manager-non-repo-");
      const { manager } = yield* makeManager();
  ...
  it.effect("status returns an explicit non-repo result for deleted directories", () =>
    Effect.gen(function* () {
      const rootDir = yield* makeTempDir("ras-code-git-manager-missing-dir-");
              ...
              JSON.stringify([
                {
                  ...
                  isCrossRepository: true,
                  headRepository: {
                    nameWithOwner: "lnieuwenhuis/ras-code",
                  },
                  headRepositoryOwner: {
                    login: "lnieuwenhuis",
                  },
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* runGit(repoDir, ["checkout", "-b", "ras-code/pr-488/statemachine"]);
        ...
        const status = yield* manager.status({ cwd: repoDir });
        expect(status.refName).toBe("ras-code/pr-488/statemachine");
        expect(status.pr).toEqual({
          number: 488,
          title: "Rebase this PR on latest main",
          url: "https://github.com/pingdotgg/codething-mvp/pull/488",
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* runGit(repoDir, ["checkout", "-b", "ras-code/pr-777/main"]);
        ...
        const status = yield* manager.status({ cwd: repoDir });
        expect(status.refName).toBe("ras-code/pr-777/main");
        expect(status.pr).toEqual({
          number: 777,
          title: "Fork PR from main",
          url: "https://github.com/pingdotgg/codething-mvp/pull/777",
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* runGit(repoDir, ["remote", "add", "fork-seed", forkDir]);
        yield* runGit(repoDir, ["push", "-u", "fork-seed", "statemachine"]);
        yield* runGit(repoDir, ["checkout", "-b", "ras-code/pr-142/statemachine"]);
        yield* runGit(repoDir, ["branch", "--set-upstream-to", "fork-seed/statemachine"]);
        yield* configureVisibleRemoteUrlWithLocalRewrite(
          repoDir,
          "fork-seed",
        ...
        const { manager, ghCalls } = yield* makeManager({
          ghScenario: {
            prListByHeadSelector: {
              // @effect-diagnostics-next-line preferSchemaOverJson:off
              "ras-code/pr-142/statemachine": JSON.stringify([]),
              // @effect-diagnostics-next-line preferSchemaOverJson:off
  ...
  it.effect("accepts fork PR metadata when origin is the fork checkout remote", () =>
    Effect.sync(() => {
      const headContext = {
        headBranch: "ras-code/git-audit-stability",
        headRepositoryNameWithOwner: "justsomelegs/ras-code",
        ...
        GitManager.matchesBranchHeadContext(
          {
            ...
            baseRefName: "main",
            headRefName: "ras-code/git-audit-stability",
            state: "open",
            updatedAt: Option.none(),
            isCrossRepository: true,
            headRepositoryNameWithOwner: "justsomelegs/ras-code",
  ...
  it.effect("generates PR content against the remote base when the local base is stale", () =>
    Effect.gen(function* () {
      ...
      const peerDir = yield* makeTempDir("ras-code-git-peer-");
  ...
  it.effect("creates cross-repo PRs with the fork owner selector and default base branch", () =>
    Effect.gen(function* () {
      ...
      yield* runGit(repoDir, ["commit", "-m", "Feature commit"]);
      yield* runGit(repoDir, ["push", "-u", "fork-seed", "statemachine"]);
      yield* runGit(repoDir, ["checkout", "-b", "ras-code/pr-91/statemachine"]);
      yield* runGit(repoDir, ["branch", "--set-upstream-to", "fork-seed/statemachine"]);
      yield* configureVisibleRemoteUrlWithLocalRewrite(
        repoDir,
        "fork-seed",
  ...
  it.effect("preserves both branch materialization failures when the fallback also fails", () =>
    Effect.gen(function* () {
      ...
      expect(error).toMatchObject({
        ...
        localBranch: "ras-code/pr-93/feature/missing-fork-branch",
      });
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(result.branch).toBe("ras-code/pr-91/main");
        expect(result.worktreePath).not.toBeNull();
        expect((yield* runGit(repoDir, ["branch", "--show-current"])).stdout.trim()).toBe("main");
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(result.branch).toBe("ras-code/pr-92/main");
        expect((yield* runGit(repoDir, ["rev-parse", "main"])).stdout.trim()).toBe(localMainBefore);
        expect(
          (yield* runGit(result.worktreePath as string, [
            "rev-parse",
