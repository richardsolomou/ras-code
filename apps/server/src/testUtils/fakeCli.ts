 ...
 *
 * The launcher runs this process's own `node`, by absolute path: a test that
 * hands the code under test a replacement environment drops PATH, and the
 * shell's fallback PATH finds node on some hosts and not others.
 */
export function writeFakeCli(options: FakeCliOptions): string {
  ...
  if ((options.platform ?? HostProcessPlatform.defaultValue()) === "win32") {
    ...
    NodeFS.writeFileSync(
      ...
      [
        "@echo off",
        `"${process.execPath}" "%~dp0${options.name}-stub.mjs" %*`,
        "exit /b %ERRORLEVEL%",
        "",
      ].join("\r\n"),
      "utf8",
  ...
  NodeFS.writeFileSync(
    ...
    [
      "#!/bin/sh",
      `exec "${process.execPath}" "$(dirname "$0")/${options.name}-stub.mjs" "$@"`,
      "",
    ].join("\n"),
    "utf8",
  );
