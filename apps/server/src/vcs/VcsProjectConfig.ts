...
import { VcsDriverKind, type VcsDriverKind as VcsDriverKindType } from "@ras-code/contracts";
...
export const make = Effect.gen(function* () {
  ...
  const findConfigPath = Effect.fn("VcsProjectConfig.findConfigPath")(function* (cwd: string) {
    let current = cwd;
    while (true) {
      const candidate = path.join(current, ".ras-code", "vcs.json");
