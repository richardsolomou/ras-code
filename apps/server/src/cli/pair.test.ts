  ...
  Effect.acquireUseRelease(
    Effect.callback<NodeHttp.Server>((resume) => {
      const server = NodeHttp.createServer((request, response) => {
        if (request.url === "/.well-known/ras-code/environment") {
...
describe("ras pair", () => {
  it.effect("mints a token and prints a QR pairing URL for a live server", () =>
    withDescriptorServer((origin) =>
      Effect.gen(function* () {
        const baseDir = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "ras-code-pair-test-"));
        ...
        assert.equal(credentials[0]?.label, "ras pair");
      }),
    ...
    withDescriptorServer((origin) =>
      Effect.gen(function* () {
        const baseDir = NodeFS.mkdtempSync(
          NodePath.join(NodeOS.tmpdir(), "ras-code-pair-dev-test-"),
        );
        const port = Number(new URL(origin).port);
  ...
  it.effect("directs to ras serve or ras connect when no server is running", () =>
    Effect.gen(function* () {
      const baseDir = NodeFS.mkdtempSync(
        NodePath.join(NodeOS.tmpdir(), "ras-code-pair-none-test-"),
      );
      ...
      assert.include(rendered, "No running RAS Code server found.");
      assert.include(rendered, "npx ras-code serve");
      assert.include(rendered, "npx ras-code connect");
    }).pipe(Effect.provide(NodeServices.layer)),
  );
  ...
  it.effect("ignores runtime state whose recorded pid is no longer alive", () =>
    withDescriptorServer((origin) =>
      Effect.gen(function* () {
        const baseDir = NodeFS.mkdtempSync(
          NodePath.join(NodeOS.tmpdir(), "ras-code-pair-pid-test-"),
        );
        const statePath = NodePath.join(baseDir, "userdata", "server-runtime.json");
  ...
  it.effect("ignores stale runtime state pointing at a dead server", () =>
    Effect.gen(function* () {
      const baseDir = NodeFS.mkdtempSync(
        NodePath.join(NodeOS.tmpdir(), "ras-code-pair-stale-test-"),
      );
      const statePath = NodePath.join(baseDir, "userdata", "server-runtime.json");
