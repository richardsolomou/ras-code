/**
 * All text `ras triage` hands to the coding agent. Kept as bare template strings
...
export const TRIAGE_PLAYBOOK = `# RAS Code triage playbook

You are a support engineer for RAS Code (https://github.com/richardsolomou/ras-code), working
...
Fetch https://raw.githubusercontent.com/richardsolomou/ras-code/main/.github/triage/PLAYBOOK.md.
...
- How is RAS Code running on this machine: \`npx ras-code serve\` in a terminal, the
  background service, or the desktop app?
- Which surface is the user connecting from: the website (code.ras.sh), the
...
Search existing issues in richardsolomou/ras-code (use \`gh\`, or the public GitHub search
...
Do not patch the RAS Code source as a fix. A good issue with strong repro steps
...
- If \`gh\` is not authenticated, offer \`gh auth login\`, or build a prefilled
  https://github.com/richardsolomou/ras-code/issues/new URL with title and body query
...
export const buildTriageLaunchPrompt = (promptFilePath: string) =>
  `Read the file "${promptFilePath}" and follow its instructions exactly: it is your RAS Code triage playbook, and it starts with asking the user what went wrong.`;

/** The full seed prompt, written to `prompt.md` in the triage scratch dir. */
export const buildTriageSeedPrompt = (contextFilePath: string) => `A RAS Code user is \
having a problem with their install and started this session with \`ras triage\`.
...
export const buildTriageContext = (input: TriageContextInput) => `# RAS Code triage context

Generated by \`ras triage\` at ${input.generatedAt}.
...
- Repo: https://github.com/richardsolomou/ras-code
