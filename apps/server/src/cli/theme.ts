...
export class ThemeFileInvalidError extends Schema.TaggedErrorClass<ThemeFileInvalidError>()(
  ...
  override get message(): string {
    return `${this.filePath} is not a valid theme file. Use a theme exported from RAS Code, or a seeded file with name, appearance, canvas, and accent.`;
  }
}
...
const envRasCodeHome = Config.string("RAS_CODE_HOME").pipe(Config.option);

const resolveThemePaths = Effect.fn(function* (explicitBaseDir: Option.Option<string>) {
  // Same precedence as the rest of the CLI: --base-dir, then RAS_CODE_HOME,
  // then the default home. A provisioning script exporting RAS_CODE_HOME must
  // not have this one command silently target the default install.
  const envHome = Option.filter(yield* envRasCodeHome, (value) => value.trim().length > 0);
