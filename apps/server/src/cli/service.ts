...
const serviceInstallCommand = Command.make("install", serviceReconcileFlags).pipe(
  Command.withDescription("Install RAS Code as a background service for this user."),
  Command.withHandler((flags) =>
    runServiceCommand(
      ...
      Effect.gen(function* () {
        ...
        if (!result.changed) {
          yield* Console.log(
            `RAS Code service is already installed with ras-code@${packageJson.version}.`,
          );
          return;
        }
        yield* Console.log(
          `${result.previouslyInstalled ? "Updated" : "Installed"} RAS Code service with ras-code@${packageJson.version}.\nLogs: ${result.plan.logPath}`,
        );
      }),
    ),
  ),
...
const serviceUpdateCommand = Command.make("update", serviceReconcileFlags).pipe(
  Command.withDescription(
    "Update or repair the background service using this CLI version. Use `npx ras-code@latest service update` for the latest release.",
  ),
  Command.withHandler((flags) =>
    runServiceCommand(
      flags,
      Effect.gen(function* () {
        const result = yield* reconcileService({ allowDowngrade: flags.allowDowngrade });
        if (!result.changed) {
          yield* Console.log(`RAS Code service is already using ras-code@${packageJson.version}.`);
          return;
        }
        yield* Console.log(
          `${result.previouslyInstalled ? "Updated" : "Installed"} RAS Code service with ras-code@${packageJson.version}.\nLogs: ${result.plan.logPath}`,
...
const serviceUninstallCommand = Command.make("uninstall", projectLocationFlags).pipe(
  Command.withDescription("Stop and remove the RAS Code background service."),
  Command.withHandler((flags) =>
    runServiceCommand(
      flags,
      Effect.gen(function* () {
        ...
        yield* Console.log(
          removed ? "Removed the RAS Code service." : "RAS Code service is not installed.",
        );
      }),
    ),
  ),
...
const serviceStatusCommand = Command.make("status", projectLocationFlags).pipe(
  Command.withDescription("Show whether the RAS Code background service is installed."),
  Command.withHandler((flags) =>
    runServiceCommand(
      flags,
      Effect.gen(function* () {
...
export const offerServiceDuringOnboarding = Effect.gen(function* () {
  ...
  if (
    ...
    yield* Console.log(
      `A newer ras-code@${status.installedVersion} background service is installed. Leaving it unchanged.`,
    );
  ...
  const wanted = yield* Prompt.run(
    Prompt.confirm({
      message: installed
        ? "The installed RAS Code service needs an update or repair. Update it now?"
        : platform === "darwin"
          ? "Run RAS Code in the background whenever you log in to this Mac? " +
            "It stays reachable through RAS Connect while you are logged in."
          : "Run RAS Code in the background whenever this machine boots? " +
            "It stays reachable through RAS Connect even after you log out.",
      initial: true,
    }),
  );
  if (!wanted) {
...
export const serviceCommand = Command.make("service").pipe(
  Command.withDescription("Manage the RAS Code background service."),
