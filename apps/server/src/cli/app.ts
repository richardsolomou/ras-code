...
export class DesktopAppUnreachableError extends Schema.TaggedErrorClass<DesktopAppUnreachableError>()(
  ...
  override get message(): string {
    return "Could not reach the RAS Code desktop app. Start or update the desktop app on this machine, then run `t3 app` again. A running RAS Code server is not enough.";
  }
}
...
export class DesktopAppRequestFailedError extends Schema.TaggedErrorClass<DesktopAppRequestFailedError>()(
  ...
  override get message(): string {
    return `RAS Code could not open ${this.workspaceRoot} (${this.code}).`;
  }
}
...
const runAppCommand = Effect.fn("cli.app")(function* (flags: {
  ...
  const configuredBaseDir = Option.getOrUndefined(flags.baseDir) ?? environment.rasCodeHome;
  const baseDir = yield* resolveBaseDir(configuredBaseDir);
  const allowDevFallback = Option.isNone(flags.baseDir) && !environment.rasCodeHome?.trim();
  ...
  yield* Console.log(`Opened ${workspaceRoot} in RAS Code.`);
});
...
export const appCommand = Command.make("app", {
  ...
  Command.withDescription("Open a project in the running RAS Code desktop app."),
