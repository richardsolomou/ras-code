...
import { parsePersistedServerObservabilitySettings } from "@ras-code/shared/serverSettings";
import { DesktopBackendBootstrap, PortSchema } from "@ras-code/contracts";
...
export const baseDirFlag = Flag.string("base-dir").pipe(
  Flag.withDescription(
    "Explicit RAS Code data directory; runtime state is stored under userdata (equivalent to RAS_CODE_HOME).",
  ),
  Flag.optional,
);
...
export const logWebSocketEventsFlag = Flag.boolean("log-websocket-events").pipe(
  Flag.withDescription(
    "Emit server-side logs for outbound WebSocket push traffic (equivalent to RAS_CODE_LOG_WS_EVENTS).",
  ),
...
const EnvServerConfig = Config.all({
  logLevel: Config.logLevel("RAS_CODE_LOG_LEVEL").pipe(Config.withDefault("Info")),
  traceMinLevel: Config.logLevel("RAS_CODE_TRACE_MIN_LEVEL").pipe(Config.withDefault("Info")),
  traceTimingEnabled: Config.boolean("RAS_CODE_TRACE_TIMING_ENABLED").pipe(
    Config.withDefault(true),
  ),
  traceFile: Config.string("RAS_CODE_TRACE_FILE").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  traceMaxBytes: Config.int("RAS_CODE_TRACE_MAX_BYTES").pipe(Config.withDefault(10 * 1024 * 1024)),
  traceMaxFiles: Config.int("RAS_CODE_TRACE_MAX_FILES").pipe(Config.withDefault(10)),
  traceBatchWindowMs: Config.int("RAS_CODE_TRACE_BATCH_WINDOW_MS").pipe(Config.withDefault(1_000)),
  otlpTracesUrl: Config.string("RAS_CODE_OTLP_TRACES_URL").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  otlpMetricsUrl: Config.string("RAS_CODE_OTLP_METRICS_URL").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  otlpExportIntervalMs: Config.int("RAS_CODE_OTLP_EXPORT_INTERVAL_MS").pipe(
    Config.withDefault(10_000),
  ),
  otlpServiceName: Config.string("RAS_CODE_OTLP_SERVICE_NAME").pipe(
    Config.withDefault("ras-code-server"),
  ),
  mode: Config.schema(ServerConfig.RuntimeMode, "RAS_CODE_MODE").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  port: Config.port("RAS_CODE_PORT").pipe(Config.option, Config.map(Option.getOrUndefined)),
  host: Config.string("RAS_CODE_HOST").pipe(Config.option, Config.map(Option.getOrUndefined)),
  rasCodeHome: Config.string("RAS_CODE_HOME").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  devUrl: Config.url("VITE_DEV_SERVER_URL").pipe(Config.option, Config.map(Option.getOrUndefined)),
  devAllowedOrigins: Config.string("RAS_CODE_DEV_ALLOWED_ORIGINS").pipe(
    Config.withDefault(""),
    Config.map((value) =>
      value
        .split(",")
  ...
  noBrowser: Config.boolean("RAS_CODE_NO_BROWSER").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  bootstrapFd: Config.int("RAS_CODE_BOOTSTRAP_FD").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  autoBootstrapProjectFromCwd: Config.boolean("RAS_CODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  logWebSocketEvents: Config.boolean("RAS_CODE_LOG_WS_EVENTS").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  tailscaleServeEnabled: Config.boolean("RAS_CODE_TAILSCALE_SERVE").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  tailscaleServePort: Config.port("RAS_CODE_TAILSCALE_SERVE_PORT").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
});
...
export const resolveServerConfig = (
  ...
  Effect.gen(function* () {
    ...
    const explicitBaseDir = resolveOptionPrecedence(
      ...
      Option.fromUndefinedOr(env.rasCodeHome),
    ).pipe(Option.filter((value) => value.trim().length > 0));
    const baseDir = yield* resolveBaseDir(
      Option.getOrUndefined(
        resolveOptionPrecedence(explicitBaseDir, Option.fromUndefinedOr(bootstrap?.rasCodeHome)),
      ),
    );
