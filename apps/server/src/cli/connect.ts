...
function formatCloudStatus(status: CloudCliStatus, options?: { readonly json?: boolean }): string {
  ...
  const nextStep = !status.authenticated
    ? "Run `ras connect link` to authorize and enable T3 Connect."
    : !status.desired
      ? "Run `ras connect link` to enable T3 Connect."
      : !status.linked
...
const confirmRelayClientInstall = (version: string) =>
  Prompt.run(
    Prompt.confirm({
      message: `The RAS Code relay client is required for T3 Connect. Download and install version ${version}?`,
      initial: false,
    }),
  );
    ...
    runCloudCommand(
      ...
      Effect.gen(function* () {
        ...
        if (yield* CliState.readCliDesiredCloudLink) {
          yield* Console.log(
            "A T3 Connect link is already pending. Start RAS Code to finish provisioning it; publishing starts once it links.",
          );
          return;
        }
