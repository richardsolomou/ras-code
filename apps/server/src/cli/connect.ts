...
const authorizeCli = Effect.fn("cloud.cli.authorize")(function* (options: {
  ...
  const existing = yield* tokens.getExisting.pipe(
    Effect.catchTag("CloudCliCredentialRefreshError", () =>
      Console.log(
        "The stored RAS Connect credential could not be refreshed; signing in again.",
...
function formatCloudStatus(status: CloudCliStatus, options?: { readonly json?: boolean }): string {
  ...
  const nextStep = !status.authenticated
    ? "Run `ras connect link` to authorize and enable RAS Connect."
    : !status.desired
  ...
  return [
    ...
    "",
    "This is saved setup, not a live connection check. Check the background service with `ras service status`.",
...
const logCloudDisconnectFailure = (
  ...
  Effect.logWarning("RAS Connect disconnect operation failed.").pipe(
    Effect.annotateLogs({
      operation,
      clearAuthorization,
    ...
    if (Exit.isFailure(input.relayResult)) {
      ...
      yield* Console.warn(
        input.clearAuthorization
          ...
          : "Could not revoke the relay-side environment record yet.\nRun `ras connect unlink` again when the relay is reachable.",
      );
...
const connectLoginCommand = Command.make("login", {
  ...
  Command.withDescription("Authorize the RAS Connect CLI without enabling remote access."),
  Command.withHandler((flags) =>
    runCloudCommand(
      flags,
      Effect.gen(function* () {
        yield* Console.log("RAS Connect\n");
...
const connectLinkCommand = Command.make("link", {
  ...
  Command.withDescription("Authorize this environment for RAS Connect on next start."),
  Command.withHandler((flags) =>
    runCloudCommand(
      flags,
      Effect.gen(function* () {
      ...
      Effect.gen(function* () {
        ...
        if (!enabled) {
          ...
          if (!linkedNow && (yield* CliState.readCliDesiredLinkMode) === "publish_only") {
            yield* CliState.setCliDesiredCloudLink(false);
            yield* Console.log("Cancelled the pending publish-only RAS Connect link.");
          }
        ...
        // out of band without RAS Connect.
        if (!(yield* tokens.hasCredential)) {
          yield* Console.log(
            "Run `ras connect login` first so this environment can be authorized to publish.",
          );
          return;
        }
        // A link may already be desired (e.g. `ras connect link` before the
        ...
        if (yield* CliState.readCliDesiredCloudLink) {
          yield* Console.log(
            "A RAS Connect link is already pending. Start RAS Code to finish provisioning it; publishing starts once it links.",
          );
          return;
        }
...
const connectUnlinkCommand = Command.make("unlink", {
  ...
  Command.withDescription("Disable RAS Connect while retaining the stored authorization."),
  Command.withHandler((flags) =>
...
const connectLogoutCommand = Command.make("logout", {
  ...
  Command.withDescription("Disable RAS Connect and clear the stored CLI authorization."),
  Command.withHandler((flags) =>
...
export const connectCommand = Command.make("connect", {
  ...
  Command.withDescription("Set up RAS Connect for this machine."),
  Command.withHandler((flags) =>
    runCloudCommand(
      flags,
      Effect.gen(function* () {
        ...
        if (background) {
          ...
          yield* Console.log(
            platform === "darwin"
              ? "\n✓ Background service ready\n\nRAS Code is set to run while you are logged in to this Mac. The server establishes the RAS Connect link on startup."
              : "\n✓ Background service ready\n\nRAS Code is set to keep running after you log out. The server establishes the RAS Connect link on startup.",
          );
          return;
        }
        const serveCommand = yield* resolveCliCommand("serve");
