/**
 * `ras pair` - mint a pairing token for an already-running server and print it
 ...
 * own `.ras-code` is checked first (matching dev-runner precedence); otherwise the
 * shared RAS Code home. `--tailscale` publishes the server over Tailscale Serve
...
import { resolveWorktreeRasCodeHome } from "@ras-code/shared/devHome";
import {
  buildTailscaleHttpsBaseUrl,
  DEFAULT_TAILSCALE_SERVE_PORT,
  ensureTailscaleServe,
  readTailscaleStatus,
} from "@ras-code/tailscale";
...
const WELL_KNOWN_ENVIRONMENT_PATH = "/.well-known/ras-code/environment";
...
export class NoRunningServerError extends Schema.TaggedErrorClass<NoRunningServerError>()(
  ...
  override get message(): string {
    return [
      "No running RAS Code server found.",
      ...this.checkedStatePaths.map((statePath) => `  checked ${statePath}`),
      "Start one with `npx ras-code serve`, or connect this machine with RAS Connect: `npx ras-code connect`.",
    ].join("\n");
  }
}
...
export class ServesOtherEnvironmentError extends Schema.TaggedErrorClass<ServesOtherEnvironmentError>()(
  ...
  override get message(): string {
    return `Tailscale Serve on HTTPS port ${String(this.servePort)} already fronts a different RAS Code server. Pass --tailscale-serve-port to publish this one on another port.`;
  }
}
...
export class ServePortOccupiedError extends Schema.TaggedErrorClass<ServePortOccupiedError>()(
  ...
  override get message(): string {
    return `HTTPS port ${String(this.servePort)} on the tailnet already serves something that is not a RAS Code server. Pass --tailscale-serve-port to publish this one on another port.`;
  }
}
...
/**
 * Three outcomes, because they drive different decisions: a RAS Code descriptor
 * (pair with it), nothing answering (safe to configure Tailscale Serve), or
 * something answering that is not a RAS Code server (do NOT overwrite its mapping).
 */
type EnvironmentProbeResult =
  | { readonly _tag: "descriptor"; readonly descriptor: ExecutionEnvironmentDescriptor }
  | { readonly _tag: "unreachable" }
  | { readonly _tag: "not-a-ras-code-server" };
...
const probeEnvironmentDescriptor = (
  ...
  Effect.gen(function* () {
    ...
    // it as unreachable lets `ras pair --tailscale` repair its own mapping
    ...
    const descriptor = yield* HttpClientResponse.filterStatusOk(response).pipe(
      ...
      Effect.mapError(() => ({ _tag: "not-a-ras-code-server" }) as const),
    );
...
const discoverPairTarget = Effect.fn("pair.discoverPairTarget")(function* (
  ...
  if (explicitBaseDir !== undefined && explicitBaseDir.trim().length > 0) {
    ...
    // Same precedence as dev-runner: inside a linked worktree its own `.ras-code`
    // outranks the shared home, so `ras pair` in a worktree pairs with the dev
    // server under test rather than the daily-driver install.
    const worktreeHome = yield* resolveWorktreeRasCodeHome(process.cwd());
    if (worktreeHome !== undefined) {
      bases.push(worktreeHome);
    }
    const envHome = yield* Config.string("RAS_CODE_HOME").pipe(Config.option);
...
const makePairServerConfig = Effect.fn(function* (input: {
  ...
  return ServerConfig.make({
    ...
    otlpServiceName: "ras-code-server",
    mode: "web",
    port: state.port,
    host: state.host,
    cwd: process.cwd(),
...
const resolveTailscalePairingBase = Effect.fn("pair.resolveTailscalePairingBase")(
  function* (input: { readonly target: DiscoveredPairTarget; readonly servePort: number }) {
    ...
    // environment, is safe to (re)configure. Any other responder — RAS Code or not
    ...
    if (existing._tag === "not-a-ras-code-server") {
...
const mintPairingLink = Effect.fn("pair.mintPairingLink")(function* (input: {
  ...
  return yield* Effect.gen(function* () {
    ...
    return yield* environmentAuth.createPairingLink({
      ...
      label: Option.getOrElse(input.label, () => "ras pair"),
...
export const pairCommand = Command.make("pair", {
  ...
  Command.withDescription(
    "Mint a pairing token for a running RAS Code server and print it as a QR code.",
  ),
