...
>()("ras-code/diagnostics/ProcessDiagnostics") {}
    ...
    function* (input) {
      if (input.pid === process.pid) {
        return {
          ...
          signal: input.signal,
          signaled: false,
          message: Option.some("Refusing to signal the RAS Code server process."),
        };
      }
      ...
      if (!canSignalCategory(selected.category)) {
        return {
          ...
          signaled: false,
          message: Option.some(
            `Process ${input.pid} is not a signalable RAS Code backend descendant.`,
          ),
        };
      }
