...
> {
  switch (command.type) {
    ...
    case "project.meta.update": {
      ...
      return {
        ...
        payload: {
          ...
          ...(command.iconEmoji !== undefined ? { iconEmoji: command.iconEmoji } : {}),
    ...
    case "thread.fork": {
      ...
      // A fork inherits the parent's transcript, so it must stay inside the
      // project whose workspace that transcript talks about.
      if (sourceThread.projectId !== command.projectId) {
        return yield* new OrchestrationCommandInvariantError({
          commandType: command.type,
          detail: `Source thread '${command.sourceThreadId}' belongs to project '${sourceThread.projectId}', not '${command.projectId}'.`,
        });
      }
      return {
        ...(yield* withEventBase({
        ...
        payload: {
          ...
          branch: command.branch,
          worktreePath: command.worktreePath,
          inheritedMessages: command.inheritedMessages,
          createdAt: command.createdAt,
          updatedAt: command.createdAt,
        },
      };
    ...
    case "thread.turn.resume-anchor.set": {
      ...
      return {
        ...
        payload: {
          ...
          resumeAnchor: command.resumeAnchor,
          createdAt: command.createdAt,
        },
      };
    }
    ...
    case "thread.auto-settle": {
      ...
      // An automatic settle is the server's own rule firing, so it carries no
      // client-supplied reason.
      const commandReason = command.type === "thread.settle" ? (command.reason ?? "user") : null;
        ...
        payload: {
          ...
          settledAt: alreadySettled
            ...
            : command.type === "thread.auto-settle"
              ? command.settledAt
              : occurredAt,
          ...(settledReason === null ? {} : { reason: settledReason }),
