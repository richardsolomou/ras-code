...
describe("ProviderRuntimeIngestion", () => {
  ...
  async function createHarness(options?: {
    ...
    const workspaceRoot = makeTempDir("ras-code-provider-project-");
  ...
  it("buffers assistant deltas with one lifecycle query per event until completion", async () => {
    ...
    const message = thread?.messages.find(
      (entry) => entry.id === "assistant:turn-buffered:item-buffered",
    );
  ...
  it("flushes and completes buffered assistant text when an approval request opens", async () => {
    ...
    const thread = await waitForThread(harness.readModel, (entry) =>
      entry.messages.some(
        (message: ProviderRuntimeTestMessage) =>
          message.id === "assistant:turn-buffered-request-flush:item-buffered-request-flush" &&
          !message.streaming &&
    ...
    const message = thread.messages.find(
      (entry: ProviderRuntimeTestMessage) =>
        entry.id === "assistant:turn-buffered-request-flush:item-buffered-request-flush",
    );
    expect(message?.streaming).toBe(false);
  });
  ...
  it("flushes and completes buffered assistant text when user input is requested", async () => {
    ...
    const thread = await waitForThread(harness.readModel, (entry) =>
      entry.messages.some(
        (message: ProviderRuntimeTestMessage) =>
          message.id ===
            "assistant:turn-buffered-user-input-flush:item-buffered-user-input-flush" &&
          !message.streaming &&
    ...
    const message = thread.messages.find(
      (entry: ProviderRuntimeTestMessage) =>
        entry.id === "assistant:turn-buffered-user-input-flush:item-buffered-user-input-flush",
    );
    expect(message?.streaming).toBe(false);
  });
  ...
  it("keeps streaming while an async question is pending", async () => {
    ...
    await harness.drain();
    const pending = (await harness.readModel()).threads[0];
    expect(pending?.session?.status).toBe("running");
    expect(
      pending?.activities.find((activity) => activity.kind === "user-input.requested")?.payload,
    ).toMatchObject({ responseMode: "message", requestId: "codex-async:question-1" });

    // #23 made the buffered path unconditional, so the deltas either side of the question land
    // as one message when the turn completes rather than streaming through it.
    harness.emit({
      ...base,
      type: "turn.completed",
      eventId: asEventId("async-turn-completed"),
      payload: { state: "completed" },
    });
    await harness.drain();
  ...
  it("does not create assistant segments for whitespace-only buffered text at approval boundaries", async () => {
    ...
    expect(
      thread.messages.some(
        (message: ProviderRuntimeTestMessage) =>
          message.id ===
          "assistant:turn-buffered-whitespace-request:item-buffered-whitespace-request",
      ),
    ).toBe(false);
  });
  ...
  it("starts a new buffered assistant message segment after approval and completes without duplication", async () => {
    ...
    await waitForThread(harness.readModel, (entry) =>
      entry.messages.some(
        (message: ProviderRuntimeTestMessage) =>
          message.id === "assistant:turn-buffered-request-append:item-buffered-request-append" &&
          !message.streaming &&
    ...
    const thread = await waitForThread(harness.readModel, (entry) =>
      entry.messages.some(
        (message: ProviderRuntimeTestMessage) =>
          message.id ===
            "assistant:turn-buffered-request-append:item-buffered-request-append:segment:1" &&
          !message.streaming &&
    ...
    const firstMessage = thread.messages.find(
      (entry: ProviderRuntimeTestMessage) =>
        entry.id === "assistant:turn-buffered-request-append:item-buffered-request-append",
    );
    ...
    const assistantEvents = events.filter(
      (event): event is Extract<(typeof events)[number], { type: "thread.message-sent" }> =>
        ...
        event.payload.messageId.startsWith(
          "assistant:turn-buffered-request-append:item-buffered-request-append",
        ),
    );
  ...
  it("keeps one message per reply when a provider reuses an item id across turns", async () => {
    const harness = await createHarness();
    const now = "2026-01-01T00:00:00.000Z";

    for (const turn of ["turn-reused-first", "turn-reused-second"]) {
      harness.emit({
        type: "content.delta",
        eventId: asEventId(`evt-delta-${turn}`),
        provider: ProviderDriverKind.make("codex"),
        createdAt: now,
        threadId: asThreadId("thread-1"),
        turnId: asTurnId(turn),
        itemId: asItemId("msg_0"),
        payload: {
          streamKind: "assistant_text",
          delta: turn,
        },
      ...
      harness.emit({
        type: "item.completed",
        eventId: asEventId(`evt-completed-${turn}`),
        provider: ProviderDriverKind.make("codex"),
        createdAt: now,
        threadId: asThreadId("thread-1"),
        turnId: asTurnId(turn),
      ...
      harness.emit({
        type: "turn.completed",
        eventId: asEventId(`evt-turn-completed-${turn}`),
        provider: ProviderDriverKind.make("codex"),
        createdAt: now,
        threadId: asThreadId("thread-1"),
        turnId: asTurnId(turn),
    ...
    expect(replies.map((entry: ProviderRuntimeTestMessage) => entry.text)).toEqual([
      "turn-reused-first",
      "turn-reused-second",
    ]);
  });

  it("keeps one message per reply when a provider reuses an item id inside one turn", async () => {
    const harness = await createHarness();
    const now = "2026-01-01T00:00:00.000Z";
    ...
    for (const reply of ["first-reply", "second-reply"]) {
      harness.emit({
        type: "content.delta",
        eventId: asEventId(`evt-delta-${reply}`),
        provider: ProviderDriverKind.make("codex"),
        createdAt: now,
        threadId: asThreadId("thread-1"),
        turnId: asTurnId("turn-reused-in-turn"),
        itemId: asItemId("msg_0"),
        payload: {
          streamKind: "assistant_text",
          delta: reply,
      ...
      harness.emit({
        type: "item.completed",
        eventId: asEventId(`evt-completed-${reply}`),
        provider: ProviderDriverKind.make("codex"),
        createdAt: now,
        threadId: asThreadId("thread-1"),
        turnId: asTurnId("turn-reused-in-turn"),
    ...
    harness.emit({
      type: "turn.completed",
      eventId: asEventId("evt-turn-completed-reused-in-turn"),
      provider: ProviderDriverKind.make("codex"),
      createdAt: now,
      threadId: asThreadId("thread-1"),
      turnId: asTurnId("turn-reused-in-turn"),
