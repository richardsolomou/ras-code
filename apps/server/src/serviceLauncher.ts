...
// `ras service update`. Keep runtime imports limited to Node built-ins.
...
const runtimePaths = (baseDir: string, version: string) => {
  ...
  return {
    versionDir,
    entryPath: NodePath.join(versionDir, "node_modules", "ras-code", "dist", "bin.mjs"),
...
export class Launcher {
  ...
  async #startChild(version: string, role: ChildRole, update?: ServiceUpdateRecord): Promise<void> {
    ...
    if (!(await runtimeExists(this.#baseDir, version))) {
      throw new Error(`Selected ras-code@${version} runtime is missing or incomplete.`);
    }
    if (this.#stopping) return;
...
async function main(): Promise<void> {
  const baseDir = process.env.RAS_CODE_HOME?.trim();
  if (baseDir === undefined || baseDir === "") {
    throw new Error("RAS_CODE_HOME is required by the RAS Code service launcher.");
  }
