...
import { CodexSettings, ProviderInstanceId, TextGenerationError } from "@ras-code/contracts";
...
const CodexTextGenerationTestLayer = ServerConfig.ServerConfig.layerTest(process.cwd(), {
  prefix: "ras-code-codex-text-generation-test-",
  ...
  return Effect.gen(function* () {
    ...
    yield* fs.writeFileString(
      ...
      [
        ...
        "  cat > \"$output_path\" <<'__RAS_CODE_FAKE_CODEX_OUTPUT__'",
        input.output,
...
function withFakeCodexEnv<A, E, R>(
  ...
  return Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "ras-code-codex-text-" });
...
it.layer(CodexTextGenerationTestLayer)("CodexTextGeneration", (it) => {
  ...
  it.effect("uses RAS_CODE_CODEX_LAUNCH_ARGS for codex exec over settings", () =>
    withFakeCodexEnv(
      {
        output: JSON.stringify({
          subject: "Add important change",
        ...
        environment: { RAS_CODE_CODEX_LAUNCH_ARGS: " --strict-config --listen off " },
        requireArg: "--strict-config",
