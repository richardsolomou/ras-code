...
const makeFixture = Effect.fn("makeAntigravityTextGenerationFixture")(function* (
  ...
  const root = yield* fs.makeTempDirectoryScoped({ prefix: "ras-code-antigravity-text-test-" });
...
it.layer(NodeServices.layer)("AntigravityTextGeneration", (it) => {
  ...
  it.effect(
    "uses the native default without sending RAS Code's default selection as a model ID",
    () =>
      Effect.gen(function* () {
        const fixture = yield* makeFixture();
