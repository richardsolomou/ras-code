...
>()("ras-code/desktopUpdate/DesktopAppUpdate") {}
...
export const make = Effect.fn("desktopUpdate.desktopAppUpdate.make")(function* () {
    ...
    Effect.gen(function* () {
      ...
      if (report.outcome === "up-to-date") {
        return yield* failWith(
          `The RAS Code desktop app on this machine is already up to date on ${report.state.currentVersion}.`,
        );
      }
      return yield* failWith(
  ...
  const run: DesktopAppUpdate["Service"]["run"] = Effect.fn("desktopUpdate.desktopAppUpdate.run")(
    function* (reportProgress) {
      if (!available) {
        return yield* failWith(
          "This server was not started by the RAS Code desktop app, so it cannot drive a desktop update.",
        );
      }
      ...
      return yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          yield* receiver
            ...
            .pipe(
              Effect.mapError((error) =>
                failWith("Could not reach the RAS Code desktop app on this machine.", error),
              ),
            );
    ...
    const terminal = yield* Effect.scoped(
      Effect.gen(function* () {
        ...
        yield* receiver
          ...
          .pipe(
            Effect.mapError((error) =>
              failWith("Could not reach the RAS Code desktop app.", error),
            ),
          );
