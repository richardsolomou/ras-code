...
it.layer(NodeServices.layer)("ServerEnvironmentLive", (it) => {
  it.effect.each([
    ...
    Effect.gen(function* () {
      ...
      const baseDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "ras-code-server-environment-concurrent-test-",
      });
  ...
  it.effect("persists the environment id across service restarts", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const baseDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "ras-code-server-environment-test-",
      });
      ...
      expect(second.capabilities.threadForkAfterMessage).toBe(true);
  ...
  it.effect("reports agent activity publishing from the current secret state", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const baseDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "ras-code-server-environment-publish-test-",
      });
  ...
  it.effect("advertises desktopAppUpdate only with desktop mode and the control fd", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const baseDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "ras-code-server-environment-desktop-update-test-",
      });
      const serverConfig = yield* makeServerConfig(baseDir);
      yield* fileSystem.makeDirectory(serverConfig.stateDir, { recursive: true });
  ...
  it.effect("structures persisted environment id filesystem failures", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const baseDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "ras-code-server-environment-error-test-",
      });
      const serverConfig = yield* makeServerConfig(baseDir);
