...
export interface TelemetryCounters {
  ...
  readonly allRasCode: GroupCounters;
}
...
export interface MergeProcessesResult {
  ...
  readonly groups: {
    ...
    readonly allRasCode: ResourceTelemetryAggregate;
  };
...
function applyLifecycleCounters(input: {
  ...
  let allRasCode = input.counters.allRasCode;
  for (const processDelta of input.deltas) {
    ...
    allRasCode = incrementCounters(allRasCode, processDelta);
  }
  ...
  for (const [identityKey, current] of input.current) {
    ...
    allRasCode = incrementCounters(allRasCode, { processStarts: 1 });
  }
  ...
  for (const [identityKey, previous] of input.previous) {
    ...
    allRasCode = incrementCounters(allRasCode, { processExits: 1 });
  }

  return { backend, electron, monitor, allRasCode };
}
...
export function mergeProcesses(input: MergeProcessesInput): MergeProcessesResult {
  ...
  return {
    ...
    groups: {
      ...
      allRasCode: aggregate(ordered, counters.allRasCode),
    },
    deltas: processDeltas,
  };
}
