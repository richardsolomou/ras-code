...
const makeServerConfigLayer = (overrides?: Partial<ServerConfig.ServerConfig["Service"]>) =>
  Layer.effect(
    ...
    Layer.provide(ServerConfig.layerTest(process.cwd(), { prefix: "ras-code-auth-session-test-" })),
  );
...
it.layer(NodeServices.layer)("SessionStore.layer", (it) => {
  it.effect("keys remote cookies by environment identity instead of state directory", () =>
    Effect.gen(function* () {
      ...
      const original = yield* cookieName("/srv/release-a", EnvironmentId.make("environment-one"));
      const moved = yield* cookieName("/srv/release-moved", EnvironmentId.make("environment-one"));
      const other = yield* cookieName("/srv/release-a", EnvironmentId.make("environment-two"));
