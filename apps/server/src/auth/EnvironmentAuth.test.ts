...
import { AuthAdministrativeScopes } from "@ras-code/contracts";
...
const makeServerConfigLayer = (overrides?: Partial<ServerConfig.ServerConfig["Service"]>) =>
  Layer.effect(
    ...
    Layer.provide(ServerConfig.layerTest(process.cwd(), { prefix: "ras-code-auth-server-test-" })),
  );
...
it.layer(NodeServices.layer)("EnvironmentAuth.layer", (it) => {
  ...
  it.effect("prefers a bearer token over a stale legacy cookie", () =>
    Effect.gen(function* () {
      ...
      const verified = yield* serverAuth.authenticateHttpRequest({
        cookies: { [sessions.legacyCookieName ?? "ras_session"]: "stale" },
