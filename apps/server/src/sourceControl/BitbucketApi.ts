...
const BitbucketApiEnvConfig = Config.all({
  baseUrl: Config.string("RAS_CODE_BITBUCKET_API_BASE_URL").pipe(
    Config.withDefault(DEFAULT_API_BASE_URL),
  ),
  accessToken: Config.string("RAS_CODE_BITBUCKET_ACCESS_TOKEN").pipe(Config.option),
  email: Config.string("RAS_CODE_BITBUCKET_EMAIL").pipe(Config.option),
  apiToken: Config.string("RAS_CODE_BITBUCKET_API_TOKEN").pipe(Config.option),
});
...
>()("ras-code/sourceControl/BitbucketApi") {}
...
function checkoutBranchName(input: {
  ...
  return `ras-code/pr-${input.pullRequestId}/${sanitizeBranchFragment(input.headBranch)}`;
}
...
function authFromConfig(
  ...
  return {
    ...
    host: Option.some("bitbucket.org"),
    detail: Option.some(
      "Set RAS_CODE_BITBUCKET_EMAIL and RAS_CODE_BITBUCKET_API_TOKEN, or RAS_CODE_BITBUCKET_ACCESS_TOKEN.",
    ),
  };
}
