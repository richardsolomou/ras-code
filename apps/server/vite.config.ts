...
const cliBuildChannel = packageJson.version.includes("-canary.") ? "canary" : "latest";
...
export default mergeConfig(
  ...
  defineConfig({
    ...
    pack: {
      ...
      define: {
        __RAS_CODE_BUILD_CHANNEL__: JSON.stringify(cliBuildChannel),
        __RAS_CODE_BUILD_RELAY_URL__: JSON.stringify(repoEnv.RAS_CODE_RELAY_URL?.trim() ?? ""),
        __RAS_CODE_BUILD_CLERK_PUBLISHABLE_KEY__: JSON.stringify(
          repoEnv.RAS_CODE_CLERK_PUBLISHABLE_KEY?.trim() ?? "",
        ),
