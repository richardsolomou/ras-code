import type { EnvironmentId, ServerConfig, ServerSelfUpdateCapability } from "@ras-code/contracts";
...
export const VERSION_MISMATCH_DISMISSALS_STORAGE_KEY = "ras-code:version-mismatch-dismissals:v1";
...
/**
 * The skew a user can act on: the connected server runs an older RAS Code than
 * this client, so the server is the side that needs updating.
 *
 * Two canary builds compare their full versions, including the date and run.
 * Other combinations compare their core `major.minor.patch` only, so a stable
 * build and a canary build with the same core do not cause an update warning.
...
export function resolveVersionMismatch(
  ...
  const compareCanaryBuilds =
    parseSemver(normalizedClientVersion)?.prerelease[0] === "canary" &&
    parseSemver(normalizedServerVersion)?.prerelease[0] === "canary";
  const serverIsBehind =
    parseSemver(clientCore) && parseSemver(serverCore)
      ? compareSemverVersions(
          compareCanaryBuilds ? normalizedServerVersion : serverCore,
          compareCanaryBuilds ? normalizedClientVersion : clientCore,
        ) < 0
  ...
  return {
    ...
    hint: "Version mismatch. Try syncing the client and server to the same RAS Code version.",
  };
}
