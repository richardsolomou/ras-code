...
export function useCloudLinkController() {
  ...
  const reportUpdateFailure = (cause: unknown) => {
    const message = cause instanceof Error ? cause.message : "Could not update RAS Connect access.";
    const traceId = findErrorTraceId(cause);
    console.error("[ras-connect] Could not update RAS Connect", { message, traceId, cause });
    setOperationError(traceId ? `${message} Trace ID: ${traceId}` : message);
    toastManager.add({
      type: "error",
      title: "Could not update RAS Connect",
      description: message,
      data: traceId
        ? {
  ...
  const reconcileCloudState = async (desired: CloudLinkDesiredState): Promise<boolean> => {
    ...
    if (!wantsLink) {
      ...
      // leave the user unable to turn RAS Connect off.
