...
describe("terminalLinkAtColumn", () => {
  it("maps terminal cells to UTF-16 offsets after a wide emoji", () => {
    const cells = [
      cell("🙂"),
      cell(""),
      ...Array.from("https://example.com", (character) => cell(character)),
    ];
    ...
    expect(terminalLinkAtColumn(row, 2)).toBe("https://example.com");
    expect(terminalLinkAtColumn(row, cells.length - 1)).toBe("https://example.com");
  ...
  it("refuses links truncated at the viewport edges instead of mis-resolving", () => {
    ...
    const tailCut = [row("https://example.com", false, true)];
    expect(terminalLinkAtPosition(tailCut, 0, 8)).toBeNull();
    // A partial bottom row is provably complete and still resolves.
    const complete = [row("https://example.com", false), row("", false)];
    expect(terminalLinkAtPosition(complete, 0, 8)).toBe("https://example.com");
    ...
    const wideFull: GhosttyRow = {
      cells: [
        { ...cell("🙂"), wide: 1 },
        { ...cell(""), wide: 2 },
        ...Array.from("https://example.co", (character) => cell(character)),
      ],
    ...
    expect(terminalLinkAtPosition([unwrittenTail], 0, 8)).toBe("https://example.com");
  });
});
