...
import { resolveWorkEntryToolPresentation } from "@ras-code/client-runtime/work-log/presentation";
...
describe("deriveWorkLogEntries", () => {
  ...
  it("preserves MCP server, tool, arguments, and results for expanded display", () => {
    ...
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        id: "mcp-tool-done",
        kind: "tool.completed",
        summary: "ras-code · preview_status",
        payload: {
          itemType: "mcp_tool_call",
          title: "ras-code · preview_status",
          data: { item },
        },
      }),
    ];
    ...
    const [entry] = deriveWorkLogEntries(activities);
    expect(entry?.toolTitle).toBe("ras-code · preview_status");
  ...
  it.each([
    ...
    (status, displayName) => {
      const data = {
        toolName: "mcp__ras_code__preview_click",
  ...
  it("keeps MCP payloads while collapsing lifecycle updates", () => {
    ...
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        ...
        kind: "tool.updated",
        summary: "ras-code · preview_snapshot",
        payload: {
          itemType: "mcp_tool_call",
          toolCallId: "call-1",
          data: { item },
