...
vi.mock("@ras-code/client-runtime/environment", () => ({
  scopedProjectKey: () => "remote-project",
  scopeProjectRef: (environmentId: string, projectId: string) => ({ environmentId, projectId }),
  scopeThreadRef: (environmentId: string, threadId: string) => ({ environmentId, threadId }),
}));
vi.mock("@ras-code/contracts", () => ({ DEFAULT_RUNTIME_MODE: "default" }));
vi.mock("@ras-code/shared/threadEnvMode", () => ({
...
vi.mock("@tanstack/react-router", () => ({
  ...
  // Our pane hooks navigate through useNavigate rather than router.navigate. Point both at the
  // same spy so the assertions below still cover the path this fork actually takes.
  useNavigate: () => testState.router.navigate,
}));
vi.mock("react", () => ({
  useCallback: <T>(callback: T) => callback,
  useMemo: <T>(factory: () => T) => factory(),
  // Our pane hooks keep a ref; one object per test run is enough for these assertions.
...
vi.mock("../lib/rasProjectFileDefaults", () => ({
  readRasProjectFileDefaultThreadEnvMode: () => testState.projectFileRead,
}));
vi.mock("../lib/utils", () => ({
