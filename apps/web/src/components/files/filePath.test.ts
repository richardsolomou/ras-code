...
describe("fileBreadcrumbs", () => {
  ...
  it("starts host paths outside the workspace at the filesystem root", () => {
    expect(fileBreadcrumbs("ras-code", "/tmp/ras-code-cleanup/report.md")).toEqual([
      { label: "tmp", path: "/tmp", kind: "directory" },
      { label: "ras-code-cleanup", path: "/tmp/ras-code-cleanup", kind: "directory" },
      { label: "report.md", path: "/tmp/ras-code-cleanup/report.md", kind: "file" },
    ]);
