    ...
    if (isDesktopAppUpdate) {
      ...
      const confirmed =
        (await requestConfirmDialog(
          `Update the RAS Code desktop app that runs the ${serverLabel}? It will close and relaunch on that machine.`,
        )) ?? true;
      if (!confirmed) {
        return;
      }
    ...
    try {
      ...
      toastManager.add({
        ...
        description: isDesktopAppUpdate
          ...
          : `Reconnected on ras-code@${result.value.targetVersion}.`,
      });
    } finally {
