  ...
  return (
    <div className="min-w-0 lg:flex lg:h-full lg:min-h-0 lg:flex-col">
      ...
      >
        ...
        <div
          className="px-4 py-4 lg:h-full lg:overflow-y-auto"
          hidden={visibleTab !== "configuration"}
        >
          <div className={PROVIDER_FIELD_GRID_CLASS_NAME}>
            {authEmail?.trim() ? (
              <>
                <span className="pt-1.5 text-xs font-medium text-foreground">Account email</span>
            ...
            {driverOption?.purpose ? (
              <p className="col-span-full text-xs text-muted-foreground">{driverOption.purpose}</p>
            ) : null}
            {driverOption ? (
