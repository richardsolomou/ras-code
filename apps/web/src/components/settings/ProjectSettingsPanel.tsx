...
import { useRasProjectFileState } from "../../hooks/useRasProjectFileScripts";
...
function ProjectDetail({ group }: { group: SidebarProjectSnapshot }) {
  const navigate = useNavigate();
  const primaryEnvironmentId = usePrimaryEnvironmentId();
  // A project lives on one device, so its defaults resolve against that
  // device's settings and providers rather than whichever device serves the UI.
  ...
  const serverProviders =
    useAtomValue(serverEnvironment.configValueAtom(representative.environmentId))?.providers ??
  ...
  const rasFile = useRasProjectFileState(
  ...
  const importFileScript = useCallback(
    async (fileScript: RasProjectFileScript) => {
    ...
    <>
      <SettingsPageContainer width="wide" className="gap-8">
        <SettingsSection title="Project">
          ...
          <SettingsRow
            title="Workspace"
            description="Where new threads in this project start. Overrides ras.json and the global default; applies to every checkout in this group."
            resetAction={
              storedEnvMode !== null ? (
                <SettingResetButton
              ...
              >
                ...
                <SelectPopup align="end" alignItemWithTrigger={false}>
                  <SelectItem value="inherit">
                    {group.memberProjects.length > 1
                      ? "Default (each checkout's ras.json or global setting)"
        ...
        >
              ...
              {importableScripts.length > 0 ? (
                <Menu>
                  ...
                  <MenuPopup align="end" className="w-72">
                    <MenuGroup>
                      <MenuGroupLabel>Import from ras.json</MenuGroupLabel>
          ...
          {rasFile.status === "invalid" ? (
            <SettingsRow
              title="ras.json is invalid"
              description="A ras.json exists in this checkout but fails to parse, so every action and icon it declares is ignored. Check the JSON syntax and icon values."
