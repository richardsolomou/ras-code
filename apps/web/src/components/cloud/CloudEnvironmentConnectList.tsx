...
import type { RelayClientEnvironmentRecord } from "@ras-code/contracts/relay";
...
/**
 * The user's RAS Connect environments from relay discovery, each with a
  ...
  const connectEnvironment = async (environment: RelayClientEnvironmentRecord) => {
    ...
    if (result._tag === "Success") {
      toastManager.add({
        ...
        description: `Connecting to ${environment.label} through RAS Connect.`,
      });
      return;
    }
    ...
    const message =
      cause instanceof Error ? cause.message : "Could not connect the RAS Connect environment.";
    const traceId = findErrorTraceId(cause);
    console.error("[ras-connect] Could not connect environment", { message, traceId, cause });
    toastManager.add({
      type: "error",
