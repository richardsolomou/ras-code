...
function ChatViewContent(
  ...
  // Threads with a stored plan mode resolve to "default" like everything else;
  // the next send persists that back.
  ...
  const [resumeCompactionPermanentlyDismissed, setResumeCompactionPermanentlyDismissed] =
    useLocalStorage(
      `ras-code:resume-compaction-dismissed:${environmentId}:${activeProviderInstanceId ?? "claudeAgent"}`,
      false,
      Schema.Boolean,
    );
  ...
  // Keep the off-flow strip measurable while its relocated controls are hidden.
  ...
  // the sent message can stay near the top. RAS Code only owns streaming adjustments
  ...
  const rightPanelContent = activeThreadRef ? (
    renderedRightPanelSurface?.kind === "preview" ? (
      ...
      <PullRequestsUnavailableState
        ...
        error="Update this environment's RAS Code server to browse pull requests."
      />
      ...
      >
        ...
        >
          ...
          <ChatHeader
            ...
            activeProjectIconEmoji={activeProject?.iconEmoji ?? null}
            openInCwd={gitCwd}
