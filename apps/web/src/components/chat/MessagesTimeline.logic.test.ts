...
describe("streaming row projection", () => {
  function fixture(text = "") {
    ...
    const input = {
      ...
      checkpointTurnCountByAssistantMessageId: new Map<MessageId, number>(),
  ...
  it("keeps full parity through completion, trailing failure, metadata, expansion, and paging", () => {
    ...
    check({
      checkpointTurnCountByAssistantMessageId: new Map([[MessageId.make("live-assistant"), 3]]),
    });
  });
});
...
describe("work entry labels", () => {
  ...
  it("uses the active summary state for legacy tools without a lifecycle status", () => {
    const browserEntry = { ...entry, toolTitle: "RAS-code.preview_click" };
...
describe("deriveMessagesTimelineRows", () => {
  ...
  it("projects assistant diff summaries and user revert counts onto the affected rows", () => {
    ...
    const rows = deriveMessagesTimelineRows({
      ...
      checkpointTurnCountByAssistantMessageId: new Map([["assistant-1" as never, 1]]),
    });
    ...
    (isWorking) => {
      ...
      const timelineEntries = Array.from({ length: 1_000 }, (_, index) => ({
        ...
        entry: {
          ...
          createdAt,
          turnId,
          label: "ras-code.preview_snapshot",
          tone: "tool" as const,
          toolLifecycleStatus:
