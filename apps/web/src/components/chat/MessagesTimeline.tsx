...
const TIMELINE_MAINTAIN_SCROLL_AT_END = {
  ...
  on: {
    dataChange: true,
    // Composer inset changes must not move already-visible messages. New rows and row growth still keep live-follow pinned.
    footerLayout: false,
    itemLayout: true,
    layout: true,
  },
  ...
  return (
    ...
    >
      ...
      {typeof checkpointTurnCount === "number" ? (
        <AssistantMessageActionsMenu messageId={message.id} />
      ) : null}
      {!message.streaming && (
        <Tooltip>
          <TooltipTrigger render={<p className="text-muted-foreground text-xs tabular-nums" />}>
...
function WorkEntryIcon({ name, className }: { name: WorkEntryIconName; className: string }) {
  switch (name) {
    ...
    case "ras-code":
      return <RasCodeWordmark className={className} hidden />;
    case "check":
...
const FallbackNoticeRow = memo(function FallbackNoticeRow(props: {
  ...
  return (
    <div className="flex select-none items-center gap-1.5 py-0.5" role="status">
      <span className="flex size-6 shrink-0 items-center justify-center text-icon-muted">
        <WorkEntryIcon name="zap" className="block size-4 shrink-0 stroke-[1.8] opacity-70" />
      </span>
