...
function createSource({ reducedMotion = false } = {}) {
  ...
  return {
    ...
    highlight: () => highlights.get("ras-code-assistant-citation")?.values().next().value,
    commentHighlight: () => highlights.get("ras-code-assistant-citation-comment"),
    cleanup: () => {
...
describe("assistant citation source lifecycle", () => {
  ...
  it("does not erase another highlight when its source unmounts", async () => {
    ...
    source.highlights.set("ras-code-assistant-citation", replacement);
    source.cleanup();
    expect(source.highlights.get("ras-code-assistant-citation")).toBe(replacement);
  });
...
describe("assistant citation comment source lifecycle", () => {
  it("keeps the exact live range highlighted until cleanup without changing selection or navigation", () => {
    ...
    source.highlights.set("ras-code-assistant-citation", navigationHighlight);
    ...
    expect(source.highlights.get("ras-code-assistant-citation")).toBe(navigationHighlight);
  ...
  it("does not delete a replacement highlight when an older editor closes", () => {
    ...
    source.highlights.set("ras-code-assistant-citation-comment", replacement);
