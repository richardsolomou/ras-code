...
function useComposerRestingTransition(
  ...
  const clearOverlayPin = useCallback(() => {
    // Use the captured overlay because React may detach the composer ref first.
  ...
  const transitionToCurrentGeometry = useCallback(
    (stateChanged: boolean) => {
      ...
      // Preserve reveal timing when body resizes retarget the geometry mid-transition.
      if (stateChanged) {
      ...
      if (
        ...
        // Pin the overlay during the tween to avoid frame-by-frame inset feedback.
        ...
        // Pin the footer while the outer height changes.
        if (footer) {
        ...
        // Publish destination geometry now; ResizeObserver handles later changes.
        ...
        if (stateChanged) {
          ...
          // Reveal added prompt lines through a bottom clip so they do not cross the footer.
          ...
          // Fade portal-moved controls so they do not pop or overlap the prompt.
        ...
        // Wall-clock cleanup covers a suspended document whose animation never finishes.
  ...
  useLayoutEffect(() => {
    ...
    // Wait for the parent context-strip layout before reading FLIP geometry.
    queueMicrotask(() => {
  ...
  useEffect(() => {
    // Treat the first measured layout as the baseline to avoid an entry animation.
...
function useRestingComposerControlsLayout(host: HTMLDivElement | null) {
  ...
  const measure = useCallback(() => {
    ...
    // The controls unmount when expanded, avoiding these layout reads.
...
export interface ChatComposerProps {
  ...
  // Project default, or the environment's global default when the project
  // sets none.
  activeDefaultModelSelection: ModelSelection | null | undefined;
...
export const ChatComposer = memo(function ChatComposer(props: ChatComposerProps) {
  ...
  const selectedProviderByThreadId = hasPendingModelSelectionIntent
    ...
    : null;
  // An explicit draft pick wins; otherwise use the provider serving the live
  // session. Resolving the snapshot by `instanceId` gives a custom instance
  // its own slash commands, skills, and model list rather than the first
  // snapshot for the same driver kind.
  const {
    ...
    () =>
      resolveComposerProviderSelection({
        ...
        lockedProvider,
        lockedInstanceId: activeThreadProviderInstanceId,
      }),
    [
      activeDefaultModelSelection?.instanceId,
      activeThreadProviderInstanceId,
          ...
          {showComposerTopDrawer && (!isTasksDrawerOpen || hasBlockingComposerTopDrawer) ? (
            <ComposerBanner.Attachment>
              <ComposerBanner.Root
                ...
                variant={activePendingApproval || activePendingFallbackOffer ? "warning" : "info"}
              >
                {activePendingApproval ? (
                  <ComposerBanner.Row
                    layout="wrap-actions"
