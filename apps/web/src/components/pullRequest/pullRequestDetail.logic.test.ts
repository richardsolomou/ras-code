...
describe("pull request checkout commands", () => {
  it.each([
    ...
    [
      ...
      "feature/checkout",
      "maria/ras-code",
      "git clone --single-branch --branch feature/checkout https://bitbucket.org/maria/ras-code.git ras-code-pr-42",
    ],
...
describe("asking about a change rather than working on it", () => {
  ...
  it("leaves the composer empty, and everything the agent needs in the chip", () => {
    ...
    expect(chip.text).toContain("https://github.com/richardsolomou/ras-code/pull/42");
...
describe("cached pull request detail", () => {
  ...
  it("shrugs off corrupt storage and no storage at all", () => {
    const storage = makeStorage();
    storage.setItem("ras-code.pullRequests.detail:env-1:project-1:acme/web#7", "{not json");
