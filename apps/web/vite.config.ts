...
const isSingleOriginDev = process.env.RAS_CODE_SINGLE_ORIGIN_DEV === "1";
...
const configuredRelayUrl = repoEnv.VITE_RAS_CODE_RELAY_URL?.trim() || "";
...
// as hot patches. Opt-in while experimental: RAS_CODE_BUNDLED_DEV=1 pnpm dev:web
// The dev runner defaults this on for --share runs (remote browsers pay a
// round trip per import level in unbundled dev); RAS_CODE_BUNDLED_DEV=0 opts out.
const bundledDevEnv = process.env.RAS_CODE_BUNDLED_DEV?.trim().toLowerCase();
...
function resolveDevProxyTarget(
  ...
  // RAS_CODE_PORT is set by scripts/dev-runner.ts for every non-desktop mode.
...
const configuredAllowedHosts = (process.env.RAS_CODE_DEV_ALLOWED_HOSTS ?? "")
  .split(",")
...
export default defineConfig(() => {
  return {
    ...
    define: {
      ...
      "import.meta.env.VITE_RAS_CODE_RELAY_URL": JSON.stringify(configuredRelayUrl),
