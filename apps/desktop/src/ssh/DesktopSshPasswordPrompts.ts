...
import { DesktopSshPasswordPromptResolutionInputSchema } from "@ras-code/contracts";
import type { SshPasswordRequest } from "@ras-code/ssh/auth";
...
export class DesktopSshPromptWindowUnavailableError extends Schema.TaggedErrorClass<DesktopSshPromptWindowUnavailableError>()(
  ...
  override get message(): string {
    ...
    return `RAS Code window is unavailable during ${this.stage} for SSH authentication to ${this.destination} (request: ${request}).`;
  }
}
...
>()("@ras-code/desktop/ssh/DesktopSshPasswordPrompts") {}
