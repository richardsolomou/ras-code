...
import { SshPasswordPromptError } from "@ras-code/ssh/errors";
...
function makeTempHomeDir() {
  return Effect.gen(function* () {
    ...
    return yield* fs.makeTempDirectoryScoped({ prefix: "ras-code-ssh-env-test-" });
  });
}
...
describe("sshEnvironment", () => {
  it("keeps prompt presentation diagnostics distinct from the legacy wrapper message", () => {
    ...
    assert.equal(
      ...
      "RAS Code window is not available for SSH authentication.",
    );
  });
