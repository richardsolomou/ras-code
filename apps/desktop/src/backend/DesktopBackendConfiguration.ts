...
>()("@ras-code/desktop/backend/DesktopBackendConfiguration") {}
...
const DESKTOP_BACKEND_ENV_NAMES = [
  ...
  "RAS_CODE_NO_BROWSER",
  "RAS_CODE_HOST",
  "RAS_CODE_DESKTOP_WS_URL",
  "RAS_CODE_DESKTOP_LAN_ACCESS",
  "RAS_CODE_DESKTOP_LAN_HOST",
  "RAS_CODE_DESKTOP_HTTPS_ENDPOINTS",
  "RAS_CODE_TAILSCALE_SERVE",
  "RAS_CODE_TAILSCALE_SERVE_PORT",
] as const;
...
function resourceMonitorBinaryName(platform: NodeJS.Platform): string {
  return platform === "win32" ? "ras-resource-monitor.exe" : "ras-resource-monitor";
}
...
const resolvePrimaryStartConfig = Effect.fn("desktop.backendConfiguration.resolvePrimary")(
  ...
  > {
    ...
    const bootstrap = {
      ...
      rasCodeHome: environment.baseDir,
    ...
    return {
      ...
      // Primary wants process.env (PATH, dev-runner's RAS_CODE_HOME, etc.).
      extendEnv: true,
      bootstrap,
...
> {
  ...
  const bootstrap = {
    ...
    noBrowser: true,
    port: input.port,
    // Omit rasCodeHome so the Linux backend uses its own home dir instead of
  ...
  // Build an explicit copy of process.env minus RAS_CODE_HOME (dev-runner
  // exports the Windows-side base dir for the primary; if it leaks into
  // the WSL backend the Linux side ends up sharing C:\Users\...\.ras-code via
  // /mnt/c, which means both backends read/write the same database and
  // their env-ids collide).
  const parentEnvWithoutRasCodeHome: Record<string, string | undefined> = {};
  for (const [key, value] of Object.entries(process.env)) {
    if (key === "RAS_CODE_HOME") continue;
    parentEnvWithoutRasCodeHome[key] = value;
  }
  const wslEnv = mergeWslEnv(parentEnvWithoutRasCodeHome.WSLENV, forwardedEnvNames);
  ...
  const baseConfig = {
    ...
    cwd: environment.backendCwd,
    env: {
      ...parentEnvWithoutRasCodeHome,
      ...backendChildEnvPatch(),
      ...forwardedEnv,
      ...(wslEnv !== undefined ? { WSLENV: wslEnv } : {}),
    },
    // env is already a complete process.env minus RAS_CODE_HOME; pass it
