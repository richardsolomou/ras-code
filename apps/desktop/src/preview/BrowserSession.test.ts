...
describe("BrowserSession", () => {
  beforeEach(() => {
    ...
    fromPartition.mockImplementation((partition: string) => {
      const browserSession = {
        ...
        getUserAgent: vi.fn(() => "Mozilla/5.0 Electron/41.5.0 ras-code/0.0.27"),
  ...
  it.effect("derives deterministic partitions and memoizes sessions", () =>
    Effect.gen(function* () {
      ...
      assert.strictEqual(partition, "persist:ras-code-preview-f051bb2c68cb7b2fe969");
  ...
  it.effect("keeps scopes that differ only by a lone surrogate in separate partitions", () =>
    Effect.gen(function* () {
      ...
      assert.strictEqual(
        ...
        "persist:ras-code-preview-f051bb2c68cb7b2fe969",
      );
    }).pipe(Effect.provide(layer)),
  );
  ...
  it.effect("keeps legacy defaults disjoint from nondefault profile partitions", () =>
    Effect.gen(function* () {
      ...
      assert.strictEqual(legacyDefault, "persist:ras-code-preview-78f0be89237d77f7a70e");
      assert.strictEqual(
        nondefaultProfile,
        "persist:ras-code-preview-profile-78f0be89237d77f7a70e",
      );
