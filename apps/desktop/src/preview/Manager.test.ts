...
const browserSessionLayer = Layer.succeed(
  ...
  BrowserSession.BrowserSession.of({
    getPartition: () => Effect.succeed("persist:ras-code-preview-test"),
    isPartition: (partition) => partition.startsWith("persist:ras-code-preview-"),
...
const environmentLayer = Layer.succeed(
  ...
  DesktopEnvironment.DesktopEnvironment.of({
    browserArtifactsDir: "/tmp/ras-code/dev/browser-artifacts",
    dirname: "/tmp/ras-code/desktop",
    path: {
...
describe("PreviewManager", () => {
  ...
  effectIt.effect("captures a PNG screenshot into browser artifacts", () =>
    withManager((manager) =>
      Effect.gen(function* () {
        ...
        expect(capturePage).toHaveBeenCalledOnce();
        expect(mkdir).toHaveBeenCalledWith("/tmp/ras-code/dev/browser-artifacts");
        ...
        expect(browserWindowConstructor).toHaveBeenCalledWith(
          expect.objectContaining({
            ...
            webPreferences: expect.objectContaining({
              preload: "/tmp/ras-code/desktop/preview-pip-preload.cjs",
    ...
    withManager((manager) =>
      Effect.gen(function* () {
        yield* manager.revealArtifact(
          "/tmp/ras-code/dev/browser-artifacts/browser-screenshot-test.png",
        );

        expect(showItemInFolder).toHaveBeenCalledWith(
          "/tmp/ras-code/dev/browser-artifacts/browser-screenshot-test.png",
        );
        const exit = yield* Effect.exit(manager.revealArtifact("/tmp/ras-code/dev/settings.json"));
        expect(Exit.isFailure(exit)).toBe(true);
        if (Exit.isSuccess(exit)) return;
        const error = Option.getOrThrow(Cause.findErrorOption(exit.cause));
        expect(error).toMatchObject({
          _tag: "PreviewArtifactPathOutsideDirectoryError",
          artifactPath: "/tmp/ras-code/dev/settings.json",
          artifactDirectory: "/tmp/ras-code/dev/browser-artifacts",
        });
  ...
  effectIt.effect("copies screenshot artifacts to the system clipboard", () =>
    withManager((manager) =>
      Effect.gen(function* () {
        const artifactPath = "/tmp/ras-code/dev/browser-artifacts/browser-screenshot-test.png";
        ...
        const exit = yield* Effect.exit(
          manager.copyArtifactToClipboard("/tmp/ras-code/dev/settings.json"),
        );
        expect(Exit.isFailure(exit)).toBe(true);
        if (Exit.isSuccess(exit)) return;
        const error = Option.getOrThrow(Cause.findErrorOption(exit.cause));
