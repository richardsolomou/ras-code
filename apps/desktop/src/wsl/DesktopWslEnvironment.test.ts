...
describe("buildWslNodeEnvPreamble", () => {
  it("passes the required Node engine range into the shared resolver", () => {
    ...
    expect(preamble).toContain("RAS_NODE_ENGINE_RANGE='^22.16 || ^23.11 || >=24.10'");
    expect(preamble.indexOf("RAS_NODE_ENGINE_RANGE=")).toBeLessThan(
  ...
  it("keeps the shared resolver permissive when no Node engine range is provided", () => {
    expect(buildWslNodeEnvPreamble()).toContain("RAS_NODE_ENGINE_RANGE=''");
  });
});
