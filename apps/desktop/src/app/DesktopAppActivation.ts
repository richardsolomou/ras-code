...
import { HostProcessUserId } from "@ras-code/shared/hostProcess";
  ...
  const server = NodeNet.createServer((socket) => {
    ...
    socket.on("data", (chunk) => {
      ...
      void input.handle(parsed).then(finish, () => {
        finish(
          invalidResponse(parsed.requestId, "RAS Code could not process the desktop app request."),
        );
      });
    });
...
>()("@ras-code/desktop/app/DesktopAppActivation") {}
