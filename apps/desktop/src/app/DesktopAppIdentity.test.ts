      ...
      DesktopAppIdentity.layer.pipe(
        Layer.provideMerge(
          FileSystem.layerNoop({
            readFileString: () =>
              Effect.succeed(input.packageJson ?? '{"rasCodeCommitHash":"abcdef1234567890"}'),
          }),
        ),
  ...
  it.effect("resolves the userData path from the app data directory", () =>
    withIdentity(
      Effect.gen(function* () {
        ...
        assert.equal(userDataPath, "/Users/alice/Library/Application Support/ras-code");
      }),
    ),
  );
  ...
  it.effect("configures app identity from the environment commit override", () => {
    ...
    return withIdentity(
      Effect.gen(function* () {
        ...
        assert.deepEqual(calls.setName, ["RAS Code (Alpha)"]);
        assert.equal(calls.setAboutPanelOptions[0]?.applicationName, "RAS Code (Alpha)");
