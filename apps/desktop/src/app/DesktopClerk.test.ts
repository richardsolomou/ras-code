...
const makeDesktopClerkLayer = (isDevelopment = true, events: string[] = []) => {
  const environment = DesktopEnvironment.DesktopEnvironment.of({
    stateDir: "/tmp/ras-code-state",
    isDevelopment,
    appDataDirectory: "/tmp/app-data",
    userDataDirName: isDevelopment ? "ras-code-dev" : "ras-code",
...
describe("DesktopClerk", () => {
  ...
  it.effect("acquires and releases the SDK bridge with the layer", () => {
    ...
    return Effect.gen(function* () {
        ...
        [
          {
            ...
            passkeys: true,
            renderer: { scheme: "ras-code-dev", host: "app" },
          },
        ],
      ]);
      ...
      assert.deepEqual(events, [
        "setPath:userData:/tmp/app-data/ras-code-dev",
        "createClerkBridge",
      ]);
  ...
  it.effect("preserves bridge initialization failures", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.equal(error.stateDir, "/tmp/ras-code-state");
      assert.equal(error.isDevelopment, true);
      assert.strictEqual(error.cause, cause);
      assert.equal(
        error.message,
        'Failed to initialize the desktop Clerk bridge for state directory "/tmp/ras-code-state" (development: true).',
      );
    });
  });
  ...
  it.effect("preserves bridge cleanup failures", () => {
    ...
    return Effect.gen(function* () {
      ...
      if (exit._tag === "Failure") {
        ...
        assert.equal(
          error.message,
          'Failed to clean up the desktop Clerk bridge for state directory "/tmp/ras-code-state" (development: false).',
        );
      }
    });
  });
  ...
  it.each([
    { isDevelopment: true, scheme: "ras-code-dev" },
    { isDevelopment: false, scheme: "ras-code" },
    ...
    assert.equal(
      DesktopClerk.createDesktopClerkBridge("/tmp/ras-code-state", isDevelopment),
      bridge,
    );
    assert.deepEqual(storageMock.mock.calls, [[{ path: "/tmp/ras-code-state" }]]);
    assert.deepEqual(createClerkBridgeMock.mock.calls, [
      [
        {
          storage: storageAdapter,
