...
export class DesktopAppActivationBroker {
  ...
  request(request: DesktopAppActivationRequest): Promise<DesktopAppActivationResponse> {
    if (this.#closed) {
      return Promise.resolve(
        failure(request.requestId, "renderer-unavailable", "RAS Code is shutting down."),
      );
    }
      ...
      if (pending.dispatched) {
        this.#settle(
          failure(
            ...
            "The RAS Code window closed before it opened the project.",
          ),
        );
      }
    }
  ...
  cancel(requestId: string): void {
    this.#settle(
      failure(requestId, "renderer-unavailable", "The command closed before RAS Code was ready."),
    );
  }
  ...
  close(): void {
    ...
    for (const pending of this.#pending.values()) {
      this.#settle(
        failure(pending.request.requestId, "renderer-unavailable", "RAS Code is shutting down."),
      );
    }
  }
