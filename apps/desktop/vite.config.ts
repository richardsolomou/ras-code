...
const shouldLaunchElectronAfterPack = process.env.RAS_CODE_DESKTOP_DEV === "1";
  ...
  run: {
    tasks: {
      build: {
        ...
        dependsOn: ["ras-code#build"],
        cache: false,
      },
      dev: {
        command:
          "node scripts/build-browser-secret.mjs && node scripts/build-preview-annotation-css.mjs && cross-env RAS_CODE_DESKTOP_DEV=1 vp pack --watch",
        dependsOn: ["ras-code#build"],
        cache: false,
      },
      "dev:bundle": {
