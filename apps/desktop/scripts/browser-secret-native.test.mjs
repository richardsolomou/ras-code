...
// oxlint-disable-next-line ras-code/no-global-process-runtime -- The native compiler targets the actual host; this script has no Effect runtime.
const hostArch = process.arch;
// oxlint-disable-next-line ras-code/no-global-process-runtime -- Native compilation only runs on the actual Linux host.
...
describe.skipIf(hostPlatform !== "linux")("bundled libsecret helper", () => {
  ...
  let executable;
  beforeAll(() => {
    directory = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "ras-code-browser-secret-test-"));
    executable = NodePath.join(directory, "ras-code-browser-secret");
