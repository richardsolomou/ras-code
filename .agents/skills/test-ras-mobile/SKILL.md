---
name: test-ras-mobile
description: Launch and test RAS Code Mobile on an iOS Simulator or Android Emulator against disposable local RAS Code environments, including Metro and dev-client reuse, native rebuild decisions, per-client pairing, seeded projects, semantic UI control, screenshots, and iOS serve-sim streaming. Use after mobile UI or native changes, when reproducing phone or tablet behavior, pairing an emulator to isolated state, or verifying mobile behavior on macOS, Linux, or Windows.
---

# Test RAS Mobile

Run one focused, end-to-end mobile verification pass against disposable RAS Code state. Use the sibling [`test-ras-app`](../test-ras-app/SKILL.md) skill as the detailed reference for pairing-token semantics and SQLite fixtures.
...
- App: `RAS Code Dev`
- Bundle/package identifier: `com.richardsolomou.ras-code.dev`
- URL scheme: `ras-code-dev`
...
## Start one disposable RAS Code environment

Run backend commands from the repository root. Use the ignored, worktree-local `.ras-code` directory or create a fresh directory with the host OS's temporary-directory mechanism. An explicit base directory stores state in `<base-dir>/userdata`; never point testing at shared `~/.ras-code` state.
...
Use direct SQLite mutation only for disposable projection fixtures. Follow `test-ras-app` and stop the backend before writing.
...
1. Inspect any process on the intended Metro port and its `/status` response. Reuse it only when it is healthy, belongs to this worktree, and matches `APP_VARIANT=development`, `--dev-client`, and scheme `ras-code-dev`.
...
- Workspace: `<repo>/apps/mobile/ios/RasCodeDev.xcworkspace`
- Scheme: `RasCodeDev`
- Configuration: `Debug`
- Simulator ID: the selected UDID
- Bundle ID: `com.richardsolomou.ras-code.dev`

Check the installed client with:

```bash
xcrun simctl get_app_container <simulator-udid> com.richardsolomou.ras-code.dev app
...
```bash
adb -s <emulator-serial> shell pm path com.richardsolomou.ras-code.dev
...
adb -s <emulator-serial> shell am start -W \
  ...
  com.richardsolomou.ras-code.dev
```
...
```bash
.agents/skills/test-ras-mobile/scripts/pair-client.sh \
...
```text
ras-code-dev://connections/new?pairingUrl=<encoded-pairing-url>&autoConnect=1
```
...
Use `snapshot_ui` and current element references from XcodeBuildMCP for taps and typing. Stream the same UDID through `ios-simulator-browser` so the user can watch in RAS Code when the host supports it. Use the stream as a visual feed rather than a reason to switch to fragile browser coordinates.
...
3. Remove the disposable environment from RAS Code Dev.
