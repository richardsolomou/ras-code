...
Load this reference only when inspecting or seeding local RAS Code state directly.
...
```bash
node apps/server/scripts/ras-sqlite-state.ts query \
  --base-dir <base-dir> \
...
```bash
node apps/server/scripts/ras-sqlite-state.ts exec \
  --base-dir <base-dir> \
  --file /tmp/ras-seed.sql
```
