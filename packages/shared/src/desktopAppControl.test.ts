...
describe("resolveDesktopAppControlAddress", () => {
  it("keeps Unix socket paths short and separates desktop state directories", () => {
    ...
    const second = resolveDesktopAppControlAddress({
      stateDir: "/home/user/.ras-code/other/userdata",
      platform: "linux",
      tempDir: "/tmp",
      userId: 1000,
      joinPath: (...segments) => segments.join("/"),
    ...
    expect(first.directory).toBe("/tmp/ras-code-1000");
  ...
  it("uses a Windows named pipe", () => {
    const result = resolveDesktopAppControlAddress({
      stateDir: "C:\\Users\\user\\.ras-code\\userdata",
      platform: "win32",
      tempDir: "C:\\Temp",
      userId: undefined,
      joinPath: (...segments) => segments.join("\\"),
    ...
    expect(result.address).toMatch(/^\\\\\.\\pipe\\ras-code-app-[a-f0-9]{24}$/);
  });
});
