 ...
 * `~/.ras-code` that a user's installed RAS Code runs against.
 *
 * A linked git worktree gets its own (gitignored) `.ras-code`: feature work in a
 * throwaway branch must not share a database with the real app, and an ambient
 * `RAS_CODE_HOME` counts as an explicit base dir — flipping the state directory
 ...
 */
export const resolveWorktreeRasCodeHome = (
  cwd: string,
): Effect.Effect<string | undefined, never, FileSystem.FileSystem | Path.Path> =>
  Effect.gen(function* () {
    ...
    const path = yield* Path.Path;
    return path.join(worktreePath, ".ras-code");
  });
