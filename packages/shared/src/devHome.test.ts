...
import { resolveGitWorktreePath, resolveWorktreeRasCodeHome } from "./devHome.ts";
...
const makeRepo = (
  ...
  Effect.acquireRelease(
    Effect.sync(() => {
      const root = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "ras-code-devhome-"));
      if (kind === "worktree") {
...
describe("resolveWorktreeRasCodeHome", () => {
  it.effect("answers with .ras-code before the dev runner creates it", () =>
    Effect.gen(function* () {
      const { root, nested } = yield* makeRepo("worktree");
      const home = yield* resolveWorktreeRasCodeHome(nested);
      assert.equal(home, NodePath.join(NodePath.resolve(root), ".ras-code"));
