...
describe("ProviderInstanceConfig fallback", () => {
  it("decodes a fallback that keeps the requested model", () => {
    const decoded = decodeProviderInstanceConfig({
      driver: "claudeAgent",
      fallback: { instanceId: "claude_gateway" },
    });
    expect(decoded.fallback).toEqual({ instanceId: "claude_gateway" });
  });

  it("decodes a fallback that overrides the model", () => {
    const decoded = decodeProviderInstanceConfig({
      driver: "claudeAgent",
      fallback: { instanceId: "claude_gateway", model: "anthropic/claude-sonnet-4.5" },
    });
    expect(decoded.fallback?.model).toBe("anthropic/claude-sonnet-4.5");
  });

  it("decodes a null fallback as a cleared fallback", () => {
    const decoded = decodeProviderInstanceConfig({ driver: "claudeAgent", fallback: null });
    expect(decoded.fallback).toBeNull();
  });

  it("decodes a config with no fallback key", () => {
    const decoded = decodeProviderInstanceConfig({ driver: "claudeAgent" });
    expect(decoded.fallback).toBeUndefined();
  });

  it("rejects a fallback instance id that fails the slug pattern", () => {
    expect(() =>
      decodeProviderInstanceConfig({
        driver: "claudeAgent",
        fallback: { instanceId: "1gateway" },
      }),
    ).toThrow();
  });
});
