...
export const CodexSettings = makeProviderSettingsSchema(
  {
    ...
    shadowHomePath: TrimmedString.pipe(
      ...
      Schema.annotateKey({
        ...
        providerSettingsForm: {
          placeholder: "~/.codex-ras/personal",
          clearWhenEmpty: "omit",
        },
      }),
    ),
...
export const OpenCodeSettings = makeProviderSettingsSchema(
  {
    ...
    serverUrl: TrimmedString.pipe(
      ...
      Schema.annotateKey({
        title: "Server URL",
        description: "Leave blank to let RAS Code spawn the server when needed.",
        providerSettingsForm: {
...
export const ServerSettings = Schema.Struct({
   ...
   * withholds the MCP credential, so the `ras-code` server (and with it every
  ...
  /**
   * Model new threads start with when their project has no
   * `defaultModelSelection` of its own. Null falls through to the
