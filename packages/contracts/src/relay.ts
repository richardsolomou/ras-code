...
export const RelayManagedEndpointProviderKind = Schema.Literals(["manual", "ras_relay"]);
...
export const RelayEnvironmentLinkChallengeRequest = Schema.Struct({
  ...
  managedRelayEnabled: Schema.Boolean.annotate({
    description: "Whether the relay should expose this environment through the managed relay.",
  }),
...
}).annotate({ description: "Links an authenticated cloud user to a RAS Code environment." });
...
>()("@ras-code/contracts/relay/RelayClientPrincipal") {}
...
>()("@ras-code/contracts/relay/RelayEnvironmentPrincipal") {}

const RelayClientBearerAuthorization = HttpApiSecurity.http({ scheme: "bearer" }).pipe(
  HttpApiSecurity.annotate(
    OpenApi.Description,
    "Clerk session or OAuth bearer token for the signed-in RAS Connect user.",
  ),
);
...
export const RelayApi = HttpApi.make("RelayApi")
  ...
  .annotate(OpenApi.Title, "RAS Code Relay API")
  .annotate(OpenApi.Version, "1.0.0")
  .annotate(
    OpenApi.Description,
    "Control-plane API for linking RAS Code environments, connecting authorized clients, and publishing agent activity.",
  );
