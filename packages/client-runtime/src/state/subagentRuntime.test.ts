...
import { classifyTaskAgentKind, type OrchestrationThreadActivity } from "@ras-code/contracts";
import {
...
describe("terminal robustness", () => {
  ...
  it("provider endedAt wins over ingestion time on the settling transition", () => {
    const agents = fold([
      activity("task.started", { taskId: "ras", taskType: "local_agent" }),
      activity(
        "task.updated",
        { taskId: "ras", status: "failed", endedAt: "2026-08-01T09:59:59.000Z" },
