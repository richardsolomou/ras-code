...
const RAS_MCP_TOOL_LABELS: Record<
  string,
...
> = {
  ...
  create_threads: ["Create", "Creating", "Created", "RAS Code threads"],
  ras_code_thread_start: ["Start", "Starting", "Started", "a RAS Code thread"],
  ras_code_thread_list: ["List", "Listing", "Listed", "RAS Code threads"],
  ras_code_thread_read: ["Read", "Reading", "Read", "a RAS Code thread"],
  ras_code_thread_send: ["Send", "Sending", "Sent", "to a RAS Code thread"],
  ras_code_thread_wait: ["Wait", "Waiting", "Waited", "for a RAS Code thread"],
  ras_code_thread_interrupt: ["Interrupt", "Interrupting", "Interrupted", "a RAS Code thread"],
  ras_code_worktree_handoff: ["Hand off", "Handing off", "Handed off", "thread to a git worktree"],
  ras_code_worktree_status: ["Get", "Getting", "Got", "thread worktree status"],
...
function resolveRasCodeMcpToolPresentation(value: string | undefined, status: string | undefined) {
  if (!value) return null;
  const name = normalizeCompactToolLabel(value).replace(
    /^(?:mcp__(?:ras-code|ras_code|rascode)__|(?:ras-code|ras_code|rascode)(?:[.:/]|\s*·\s*))/i,
    "",
  );
  if (!Object.hasOwn(RAS_MCP_TOOL_LABELS, name)) return null;

  const [action, running, completed, detail] = RAS_MCP_TOOL_LABELS[name]!;
  const verb =
    status === "inProgress"
      ? running
      : status === "completed"
  ...
  return {
    ...
    icon: name.startsWith("preview_") ? ("browser" as const) : ("ras-code" as const),
  };
}
...
export function resolveWorkEntryToolPresentation(
  ...
  if (data !== null && typeof data === "object") {
    if (
      ...
      return resolveRasCodeMcpToolPresentation(`${data.server}.${data.tool}`, status);
    }
    if ("toolName" in data && typeof data.toolName === "string") {
      return resolveRasCodeMcpToolPresentation(data.toolName, status);
    }
  }

  return (
    resolveRasCodeMcpToolPresentation(entry.toolTitle, status) ??
    resolveRasCodeMcpToolPresentation(entry.label, status)
  );
}
