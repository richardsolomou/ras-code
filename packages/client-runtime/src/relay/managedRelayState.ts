      ...
      return yield* Effect.tryPromise({
        try: () => readCachedClerkToken(nowMillis),
          ...
          new ManagedRelaySessionError({
            message: "Could not obtain the RAS Connect session token.",
            cause,
          }),
      });
    }),
  ...
  return session.readClerkToken().pipe(
    Effect.flatMap((token) =>
      token
        ...
        : Effect.fail(
            new ManagedRelaySessionError({
              message: "The RAS Connect session token is unavailable.",
            }),
          ),
    ),
  );
...
export const deregisterManagedRelayEnvironment = Effect.fn(
  ...
  if (!session || session.accountId !== input.accountId) {
    return yield* new ManagedRelaySessionError({
      message: "Sign in to RAS Connect before deregistering an environment.",
    });
  }
...
function requireClerkToken(
  ...
  if (!session || session.accountId !== accountId) {
    return Effect.fail(
      new ManagedRelaySessionError({
        message: "Sign in to RAS Connect before loading relay data.",
      }),
    );
  }
...
export function readManagedRelaySnapshotState<A>(
  ...
  if (result._tag === "Failure") {
    ...
    error = cause instanceof Error ? cause.message : "Could not load RAS Connect data.";
