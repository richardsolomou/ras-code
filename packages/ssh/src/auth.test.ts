...
describe("ssh auth", () => {
  ...
  it.effect("creates askpass env for cached password prompts", () =>
    Effect.gen(function* () {
      ...
      const directory = yield* fs.makeTempDirectoryScoped({ prefix: "ras-code-ssh-askpass-test-" });
      ...
      assert.equal(env.RAS_SSH_AUTH_SECRET, "super-secret");
      assert.equal(env.DISPLAY, "ras-code");
  ...
  it.effect("builds a windows askpass launcher pair", () =>
    Effect.gen(function* () {
      const descriptor = yield* buildSshAskpassHelperDescriptor({
        directory: "C:\\temp\\ras-code-ssh-askpass",
      }).pipe(
        Effect.provide(
      ...
      assert.equal(descriptor.launcherPath, "C:\\temp\\ras-code-ssh-askpass\\ssh-askpass.cmd");
      assert.deepEqual(
