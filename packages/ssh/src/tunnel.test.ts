...
import {
  ...
  buildRemoteRasCodeRunnerScript,
...
describe("ssh tunnel scripts", () => {
  it("builds the remote ras runner with npx and npm fallbacks", () => {
    const script = buildRemoteRasCodeRunnerScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE });
    ...
    assert.include(script, "npm produced no ras executable");
    assert.include(script, 'prepend_path_if_dir "$HOME/.local/bin"');
    assert.include(script, `RAS_NODE_ENGINE_RANGE='${TEST_NODE_ENGINE_RANGE}'`);
    ...
    assert.include(script, 'for RAS_NODE_BIN in "$NVM_DIR"/versions/node/*/bin');
  ...
  it("does not hard-code a remote node engine range", () => {
    ...
    assert.include(script, "RAS_NODE_ENGINE_RANGE=''");
  ...
  it("builds the remote ras runner with a node script override", () => {
    ...
    assert.include(
      script,
      "RAS_NODE_SCRIPT_PATH='/Users/julius/Development/Work/codething-mvp/apps/server/dist/bin.mjs'",
    );
    assert.include(script, 'exec node "$RAS_NODE_SCRIPT_PATH" "$@"');
  });

  it("uses the remote ras runner for launch and pairing scripts", () => {
    const target = {
      alias: "devbox",
    ...
    assert.include(
      buildRemoteLaunchScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE }),
      `RAS_NODE_ENGINE_RANGE='${TEST_NODE_ENGINE_RANGE}'`,
    );
    assert.include(
      buildRemoteLaunchScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE }),
    ...
    assert.include(buildRemoteLaunchScript(), "Remote RAS Code server did not become ready");
