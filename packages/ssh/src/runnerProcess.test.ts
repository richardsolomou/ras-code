...
import { buildRemoteRasCodeRunnerScript } from "./tunnel.ts";
...
server.listen(Number(process.env.RAS_TEST_PORT ?? 0), "127.0.0.1", () => {
...
fs.appendFileSync(process.env.RAS_TEST_CALLS, JSON.stringify(args) + "\\n");
if (args.includes("--package")) {
  process.stdout.write(process.env.RAS_TEST_CLI + "\\n");
} else {
  const child = childProcess.spawn(process.execPath, [process.env.RAS_TEST_CLI, ...args], { stdio: "inherit" });
            ...
            Effect.gen(function* () {
              const child = yield* spawner.spawn(
                ChildProcess.make("/bin/sh", ["-s", "--", "serve", "a path with spaces"], {
                  ...
                  env: {
                    PATH: bin,
                    RAS_TEST_CLI: cliPath,
                    RAS_TEST_CALLS: callsPath,
                    RAS_TEST_PORT: String(port),
                  },
                  detached: false,
                  stdin: Stream.make(
                    new TextEncoder().encode(buildRemoteRasCodeRunnerScript({ packageSpec })),
                  ),
                }),
              );
