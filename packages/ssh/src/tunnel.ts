...
} from "@ras-code/shared/httpReadiness";
import * as NetService from "@ras-code/shared/Net";
import { extractJsonObject, fromLenientJson } from "@ras-code/shared/schemaJson";
...
export interface RemoteRasCodeRunnerOptions {
...
export interface SshEnvironmentManagerOptions {
  ...
  readonly resolveCliRunner?: Effect.Effect<RemoteRasCodeRunnerOptions>;
}
...
function sshRunnerLogFields(runner: RemoteRasCodeRunnerOptions | undefined) {
...
remote_node_satisfies_engine() {
  RAS_NODE_ENGINE_RANGE=@@RAS_NODE_ENGINE_RANGE@@
  if [ -z "$RAS_NODE_ENGINE_RANGE" ]; then
    return 0
  fi
  node - "$RAS_NODE_ENGINE_RANGE" <<'NODE'
@@RAS_NODE_ENGINE_CHECK_SCRIPT@@
NODE
}
...
ensure_remote_node_path() {
  ...
  if ! command -v node >/dev/null 2>&1 && [ -d "$NVM_DIR/versions/node" ]; then
    for RAS_NODE_BIN in "$NVM_DIR"/versions/node/*/bin; do
      if [ -x "$RAS_NODE_BIN/node" ]; then
        PATH="$RAS_NODE_BIN:$PATH"
        export PATH
      fi
    done
  fi
...
RAS_NODE_SCRIPT_PATH=@@RAS_NODE_SCRIPT_PATH@@
if [ -n "$RAS_NODE_SCRIPT_PATH" ]; then
  ...
  exec node "$RAS_NODE_SCRIPT_PATH" "$@"
fi
if command -v ras >/dev/null 2>&1; then
  exec ras "$@"
fi
# npm extracts a package before it runs the native builds of its dependencies,
# so a failed build (ras depends on node-pty, which needs a C toolchain) leaves
# the npx cache without a ras executable. \`npx --yes\` then exits 0 without
...
require_installed_ras_cli() {
  RAS_CLI_PATH="$("$@" -- sh -c 'command -v ras' || true)"
  if [ -n "$RAS_CLI_PATH" ]; then
    return 0
  fi
...
@@RAS_NODE_ENV_SCRIPT@@
STATE_KEY="$1"
STATE_DIR="$HOME/.ras-code/ssh-launch/$STATE_KEY"
DEFAULT_SERVER_HOME="$HOME/.ras-code"
DEFAULT_RUNTIME_FILE="$DEFAULT_SERVER_HOME/userdata/server-runtime.json"
PORT_FILE="$STATE_DIR/port"
PID_FILE="$STATE_DIR/pid"
MANAGED_FILE="$STATE_DIR/managed"
LOG_FILE="$STATE_DIR/server.log"
RUNNER_FILE="$STATE_DIR/run-ras.sh"
RUNNER_NEXT="$STATE_DIR/run-ras.next.$$"
mkdir -p "$STATE_DIR"
cleanup_runner_next() {
  rm -f "$RUNNER_NEXT"
}
...
pick_port() {
  node - "$PORT_FILE" "@@RAS_DEFAULT_REMOTE_PORT@@" "@@RAS_REMOTE_PORT_SCAN_WINDOW@@" <<'NODE'
@@RAS_PICK_PORT_SCRIPT@@
NODE
}
wait_ready() {
  node - "$REMOTE_PORT" "$1" "@@RAS_READY_PROBE_TIMEOUT_MS@@" <<'NODE'
@@RAS_WAIT_READY_SCRIPT@@
NODE
}
wait_for_pid_exit() {
  PID_TO_WAIT="$1"
...
if [ -n "$DEFAULT_REMOTE_PORT" ]; then
  ...
  if wait_ready "@@RAS_REUSE_READY_TIMEOUT_MS@@"; then
...
if [ "$REMOTE_MANAGED" = "external" ]; then
  if [ -z "$REMOTE_PORT" ] || ! wait_ready "@@RAS_REUSE_READY_TIMEOUT_MS@@"; then
    REMOTE_PID=""
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  fi
  ...
  if [ "$RUNNER_CHANGED" -eq 1 ]; then
    ...
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  elif ! wait_ready "@@RAS_REUSE_READY_TIMEOUT_MS@@"; then
    kill "$REMOTE_PID" 2>/dev/null || true
    wait_for_pid_exit "$REMOTE_PID"
    REMOTE_PID=""
    REMOTE_PORT=""
...
if [ -z "$REMOTE_PORT" ]; then
  ...
  nohup env RAS_CODE_NO_BROWSER=1 "$RUNNER_FILE" serve --host 127.0.0.1 --port "$REMOTE_PORT" --base-dir "$DEFAULT_SERVER_HOME" >>"$LOG_FILE" 2>&1 < /dev/null &
  REMOTE_PID="$!"
  printf '%s\\n' "$REMOTE_PID" >"$PID_FILE"
  printf '%s\\n' "$REMOTE_PORT" >"$PORT_FILE"
  printf 'managed\\n' >"$MANAGED_FILE"
  if ! wait_ready "@@RAS_READY_TIMEOUT_MS@@"; then
    printf 'Remote RAS Code server did not become ready on 127.0.0.1:%s.\\n' "$REMOTE_PORT" >&2
    if [ -s "$LOG_FILE" ]; then
...
STATE_DIR="$HOME/.ras-code/ssh-launch/@@RAS_STATE_KEY@@"
DEFAULT_SERVER_HOME="$HOME/.ras-code"
RUNNER_FILE="$STATE_DIR/run-ras.sh"
mkdir -p "$STATE_DIR"
cat >"$RUNNER_FILE" <<'SH'
...
export function buildRemoteRasCodeRunnerScript(input?: RemoteRasCodeRunnerOptions): string {
  const packageSpec = shellSingleQuote(input?.packageSpec?.trim() || "ras-code@latest");
  const nodeScriptPath = input?.nodeScriptPath?.trim() || "";
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_RUNNER_SCRIPT, {
      RAS_PACKAGE_SPEC: packageSpec,
      RAS_NODE_SCRIPT_PATH: shellSingleQuote(nodeScriptPath),
      RAS_NODE_ENV_SCRIPT: buildRemoteNodeEnvScript(input),
    }),
  );
}

export function buildRemoteNodeEnvScript(input?: RemoteRasCodeRunnerOptions): string {
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_NODE_ENV_SCRIPT, {
      RAS_NODE_ENGINE_RANGE: shellSingleQuote(input?.nodeEngineRange?.trim() || ""),
      RAS_NODE_ENGINE_CHECK_SCRIPT: stripTrailingNewlines(buildRemoteNodeEngineCheckScript()),
    }),
  );
}

export function buildRemoteLaunchScript(input?: RemoteRasCodeRunnerOptions): string {
  return applyScriptPlaceholders(REMOTE_LAUNCH_SCRIPT, {
    RAS_NODE_ENV_SCRIPT: buildRemoteNodeEnvScript(input),
    RAS_RUNNER_SCRIPT: stripTrailingNewlines(buildRemoteRasCodeRunnerScript(input)),
    RAS_PICK_PORT_SCRIPT: stripTrailingNewlines(REMOTE_PICK_PORT_SCRIPT),
    RAS_WAIT_READY_SCRIPT: stripTrailingNewlines(REMOTE_WAIT_READY_SCRIPT),
    RAS_DEFAULT_REMOTE_PORT: String(DEFAULT_REMOTE_PORT),
    RAS_REMOTE_PORT_SCAN_WINDOW: String(REMOTE_PORT_SCAN_WINDOW),
    RAS_READY_TIMEOUT_MS: String(REMOTE_READY_TIMEOUT_MS),
    RAS_REUSE_READY_TIMEOUT_MS: String(REMOTE_REUSE_READY_TIMEOUT_MS),
    RAS_READY_PROBE_TIMEOUT_MS: String(SSH_READY_PROBE_TIMEOUT_MS),
  });
}

export function buildRemotePairingScript(
  target: DesktopSshEnvironmentTarget,
  input?: RemoteRasCodeRunnerOptions,
): string {
  return applyScriptPlaceholders(REMOTE_PAIRING_SCRIPT, {
    RAS_STATE_KEY: remoteStateKey(target),
    RAS_RUNNER_SCRIPT: stripTrailingNewlines(buildRemoteRasCodeRunnerScript(input)),
  });
}
...
export const launchOrReuseRemoteServer = Effect.fn("ssh/tunnel.launchOrReuseRemoteServer")(
  function* (
    ...
    runner?: RemoteRasCodeRunnerOptions,
  ): Effect.fn.Return<
...
const makeSshEnvironmentManager = Effect.fn("ssh/tunnel.SshEnvironmentManager.make")(function* (
  ...
  const createTunnelEntry = Effect.fn("ssh/tunnel.ensureTunnelEntry.create")(function* (input: {
    readonly key: string;
    readonly resolvedTarget: DesktopSshEnvironmentTarget;
    readonly runner?: RemoteRasCodeRunnerOptions;
...
>()("@ras-code/ssh/tunnel/SshEnvironmentManager") {
