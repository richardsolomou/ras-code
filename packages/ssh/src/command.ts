...
import type { DesktopSshEnvironmentTarget, DesktopUpdateChannel } from "@ras-code/contracts";
...
const PUBLISHABLE_RAS_VERSION_PATTERN = /^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/u;
...
export function resolveRemoteRasCodeCliPackageSpec(input: {
  readonly appVersion: string;
  readonly updateChannel: DesktopUpdateChannel;
  readonly isDevelopment?: boolean;
}): string {
  const appVersion = input.appVersion.trim();
  if (!input.isDevelopment && PUBLISHABLE_RAS_VERSION_PATTERN.test(appVersion)) {
    return `ras-code@${appVersion}`;
  }
