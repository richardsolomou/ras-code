...
export class SshPasswordPrompt extends Context.Service<SshPasswordPrompt, SshPasswordPromptShape>()(
  "@ras-code/ssh/auth/SshPasswordPrompt",
) {
...
const SSH_ASKPASS_DIR_NAME = "ras-code-ssh-askpass";
...
# Invoked by ssh via SSH_ASKPASS when RAS Code re-runs ssh with a cached password
# from the renderer's in-app prompt. We never expose a native dialog here - if
# RAS_SSH_AUTH_SECRET is missing, that's a caller bug and we fail loudly.
if [ "\${RAS_SSH_AUTH_SECRET+x}" = "x" ]; then
  printf "%s\\n" "$RAS_SSH_AUTH_SECRET"
  exit 0
fi
printf 'RAS Code ssh-askpass invoked without RAS_SSH_AUTH_SECRET.\\n' >&2
exit 1
`;
...
export const ASKPASS_WINDOWS_SCRIPT = `# Invoked by ssh via SSH_ASKPASS (through ssh-askpass.cmd) when RAS Code re-runs\r
# ssh with a cached password from the renderer's in-app prompt. We never expose\r
# a native dialog here - if RAS_SSH_AUTH_SECRET is missing, that's a caller bug\r
# and we fail loudly.\r
if ($null -ne $env:RAS_SSH_AUTH_SECRET) {\r
  [Console]::Out.WriteLine($env:RAS_SSH_AUTH_SECRET)\r
  exit 0\r
}\r
[Console]::Error.WriteLine("RAS Code ssh-askpass invoked without RAS_SSH_AUTH_SECRET.")\r
exit 1\r
`;
...
export const getDefaultSshAskpassDirectory = Effect.fn("ssh/auth.getDefaultSshAskpassDirectory")(
  function* () {
    ...
    const parentDirectory = yield* fs.makeTempDirectory({ prefix: "ras-code-ssh-runtime-" });
...
> {
  ...
  return {
    ...
    ...(input.authSecret === undefined ? {} : { RAS_SSH_AUTH_SECRET: input.authSecret ?? "" }),
    ...(platform === "win32" || baseEnv.DISPLAY || hostDisplay ? {} : { DISPLAY: "ras-code" }),
  };
});
